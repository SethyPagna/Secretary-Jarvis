...
sidebar_position: 11
title: Model Catalog
description: Remotely-hosted manifest driving curated model picker lists for OpenRouter and JARVIS Managed.
---

# Model Catalog

Jarvis fetches curated model lists for **OpenRouter** and **JARVIS Managed** from a JSON manifest hosted alongside the docs site. This lets maintainers update picker lists without shipping a new `jarvis-agent` release.
...
- **`description`** — OpenRouter-only. Drives picker badge text (`"recommended"`, `"free"`, or empty). JARVIS Managed doesn't use this — free-tier gating is determined live from the Portal's pricing endpoint.
...
model_catalog:
  enabled: true
  url: https://jarvis-agent.jarvis.local/docs/api/model-catalog.json
  ttl_hours: 24
  providers: {}
```
...
# editing OPENROUTER_MODELS or _PROVIDER_MODELS["jarvis_managed"] in jarvis_cli/models.py).
