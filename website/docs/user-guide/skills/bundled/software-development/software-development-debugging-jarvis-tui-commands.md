---
title: "Debugging Jarvis Tui Commands — Debug Jarvis TUI slash commands: Python, gateway, Ink UI"
sidebar_label: "Debugging Jarvis Tui Commands"
description: "Debug Jarvis TUI slash commands: Python, gateway, Ink UI"
---

{/* This page is auto-generated from the skill's SKILL.md by website/scripts/generate-skill-docs.py. Edit the source SKILL.md, not this page. */}

# Debugging Jarvis Tui Commands

Debug Jarvis TUI slash commands: Python, gateway, Ink UI.
...
| Path | `skills/software-development/debugging-jarvis-tui-commands` |
| Version | `1.0.0` |
| Author | JARVIS |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `debugging`, `jarvis-agent`, `tui`, `slash-commands`, `typescript`, `python` |
...
# Debugging Jarvis TUI Slash Commands
...
Use this skill when you encounter issues with slash commands in the Jarvis TUI, particularly when commands aren't showing in autocomplete, aren't working properly in the TUI, or need to be added/updated.
...
<!-- ascii-guard-ignore -->
```
Python backend (jarvis_cli/commands.py)     <- canonical COMMAND_REGISTRY
       │
       ▼
...
3. **Check if the command exists in the Python backend:**
   ```bash
   search_files --pattern "CommandDef" --file_glob "*.py" --path jarvis_cli/
   search_files --pattern "commandname" --path jarvis_cli/commands.py --context 3
   ```
...
4. If the command runs server-side, add a handler in `JarvisCLI.process_command()` in `cli.py`:
   ```python
...
1. **Command shows in TUI but not in autocomplete.** The command is defined in the TUI codebase but missing from `COMMAND_REGISTRY` in `jarvis_cli/commands.py`. Autocomplete data ships from Python.
...
1. Rebuild the TUI:
   ```bash
   cd /home/bb/jarvis-agent && npm --prefix ui-tui run build
   ```
...
4. Execute the command and confirm:
   ...
   - Any persisted config updates correctly (`read_file ~/.jarvis/config.yaml`)
