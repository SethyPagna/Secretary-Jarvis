...
The `jarvis honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). Run `jarvis memory setup` and pick Honcho first; the subcommand appears on the next invocation.

```bash
jarvis honcho status          # Connection status, config, and key settings
jarvis honcho setup           # Redirects to `jarvis memory setup`
