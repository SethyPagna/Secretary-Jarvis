...
| `--tui` | off | Expose the in-browser Chat tab (embedded `jarvis --tui` via PTY/WebSocket). Alternatively set `JARVIS_DASHBOARD_TUI=1`. |
...
The Chat tab is intentionally off for a plain `jarvis dashboard` launch. Start the dashboard with `jarvis dashboard --tui` or set `JARVIS_DASHBOARD_TUI=1` when you want the embedded browser chat pane.
...
Browse, search, and toggle skills and toolsets. Skills are loaded from `~/.jarvis/skills/` and grouped by category.
