...
If you'd rather edit the YAML directly, add a `fallback_model` section to `~/.jarvis/config.yaml`:
...
`fallback_model` (singular) is the legacy single-fallback key — Jarvis still honors it for back-compat. `fallback_providers` (plural, list) supports multiple fallbacks tried in order; `jarvis fallback` writes to this key. When both are set, Jarvis merges them with `fallback_providers` taking priority.
:::
...
| JARVIS Managed | `jarvis_managed` | `jarvis auth` (OAuth) |
...
**JARVIS Managed as fallback for OpenRouter:**
```yaml
model:
  provider: openrouter
...
When a task's provider is set to `"auto"` (the default), Jarvis tries providers in order until one works:

**For text tasks (compression, web extract, etc.):**

```text
OpenRouter → JARVIS Managed → Custom endpoint → Codex OAuth →
...
```text
Main provider (if vision-capable) → OpenRouter → JARVIS Managed →
Codex OAuth → Anthropic → Custom endpoint → give up
```

If the resolved provider fails at call time, Jarvis also has an internal retry: if the provider is not OpenRouter and no explicit `base_url` is set, it tries OpenRouter as a last-resort fallback.
...
auxiliary:
  vision:
    provider: "auto"              # auto | openrouter | jarvis_managed | codex | main | anthropic
...
auxiliary:
  compression:
    provider: "auto"                              # auto | openrouter | jarvis_managed | main
    model: "google/gemini-3-flash-preview"
```
