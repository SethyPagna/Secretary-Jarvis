...
[Adding Tools](/docs/developer-guide/adding-tools) page is for built-in Jarvis
core tools that live in `tools/` and `toolsets.py`.

**→ [Build a Jarvis Plugin](/docs/guides/build-a-jarvis-plugin)** — step-by-step guide with a complete working example.
...
Sub-category plugins surface in `jarvis plugins list` and the interactive `jarvis plugins` UI under their **path-derived key** — e.g. `observability/langfuse`, `image_gen/openai`, `platforms/teams`. That key (not the bare manifest `name:`) is the value you pass to `jarvis plugins enable …` / `disable …` and the string to add under `plugins.enabled` in `config.yaml`.
...
|---|---|---|
| A **tool** the LLM can call | Python plugin — `ctx.register_tool()` | [Build a Jarvis Plugin](/docs/guides/build-a-jarvis-plugin) · [Adding Tools](/docs/developer-guide/adding-tools) |
| A **lifecycle hook** (pre/post LLM, session start/end, tool filter) | Python plugin — `ctx.register_hook()` | [Hooks reference](/docs/user-guide/features/hooks) · [Build a Jarvis Plugin](/docs/guides/build-a-jarvis-plugin) |
| A **slash command** for the CLI / gateway | Python plugin — `ctx.register_command()` | [Build a Jarvis Plugin](/docs/guides/build-a-jarvis-plugin) · [Extending the CLI](/docs/developer-guide/extending-the-cli) |
| A **subcommand** for `jarvis <thing>` | Python plugin — `ctx.register_cli_command()` | [Extending the CLI](/docs/developer-guide/extending-the-cli) |
...
| An **STT backend** (custom whisper binary, local ASR CLI) | Config-driven — set `JARVIS_LOCAL_STT_COMMAND` env var to a shell template | [Voice Message Transcription (STT)](/docs/user-guide/features/tts#voice-message-transcription-stt) |
| **External tools via MCP** (filesystem, GitHub, Linear, Notion, any MCP server) | Config-driven — declare `mcp_servers.<name>` with `command:` / `url:` in `config.yaml`. Jarvis auto-discovers the server's tools and registers them alongside built-ins. | [MCP](/docs/user-guide/features/mcp) |
| **Additional skill sources** (custom GitHub repos, private skill indexes) | CLI — `jarvis skills tap add <repo>` | [Skills Hub](/docs/user-guide/features/skills#skills-hub) · [Publishing a custom tap](/docs/user-guide/features/skills#publishing-a-custom-skill-tap) |
| **Gateway event hooks** (fire on `gateway:startup`, `session:start`, `agent:end`, `command:*`) | Drop `HOOK.yaml` + `handler.py` into `~/.jarvis/hooks/<name>/` | [Event Hooks](/docs/user-guide/features/hooks#gateway-event-hooks) |
...
On NixOS, plugins can be installed declaratively via the module options — no `jarvis plugins install` needed. See the **[Nix Setup guide](/docs/getting-started/nix-setup#plugins)** for full details.
...
jarvis plugins enable my-plugin                      # add to allow-list (flat plugin)
jarvis plugins enable observability/langfuse         # add to allow-list (sub-category plugin)
jarvis plugins disable my-plugin                     # remove from allow-list + add to disabled
```

For plugins under a sub-category directory (e.g. `plugins/observability/langfuse/`, `plugins/image_gen/openai/`), use the full `<category>/<plugin>` key — that's exactly what `jarvis plugins list` shows in the **Name** column.
...
See the **[full guide](/docs/guides/build-a-jarvis-plugin)** for handler contracts, schema format, hook behavior, error handling, and common mistakes.
