...
Lists the agent as an available model. The advertised model name defaults to the [profile](/docs/user-guide/profiles) name (or `jarvis-agent` for the default profile). Required by most frontends for model discovery.
...
:::warning Security
The API server gives full access to jarvis-agent's toolset, **including terminal commands**. When binding to a non-loopback address like `0.0.0.0`, `API_SERVER_KEY` is **required**. Also keep `API_SERVER_CORS_ORIGINS` narrow to control browser access.
...
To give multiple users their own isolated Jarvis instance (separate config, memory, skills), use [profiles](/docs/user-guide/profiles):
...
In Open WebUI, add each as a separate connection. The model dropdown shows `alice` and `bob` as distinct models, each backed by a fully isolated Jarvis instance. See the [Open WebUI guide](/docs/user-guide/messaging/open-webui#multi-user-setup-with-profiles) for details.
