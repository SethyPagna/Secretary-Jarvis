...
The built-in web dashboard runs as an optional side-process inside the same container as the gateway. Set `JARVIS_DASHBOARD=1` and expose port `9119` alongside the gateway's `8642`:
...
|---------------------|-------------|---------|
| `JARVIS_DASHBOARD` | Set to `1` (or `true` / `yes`) to launch the dashboard alongside the main command | *(unset — dashboard not started)* |
...
| `JARVIS_DASHBOARD_TUI` | Set to `1` to expose the in-browser Chat tab (embedded `jarvis --tui` via PTY/WebSocket) | *(unset)* |

The default `JARVIS_DASHBOARD_HOST=0.0.0.0` is required for the host to reach the dashboard through the published port; the entrypoint automatically passes `--insecure` to `jarvis dashboard` in that case. Override to `127.0.0.1` if you want to restrict the dashboard to in-container access only (e.g. behind a reverse proxy in a sidecar).
...
Jarvis supports [multiple profiles](../reference/profile-commands.md) — separate `~/.jarvis/` directories that let you run independent agents (different SOUL, skills, memory, sessions, credentials) from a single installation. **When running under Docker, using Jarvis' built-in multi-profile feature is not recommended.**
...
- **Independent lifecycle** — upgrade, restart, pause, or roll back each agent separately (`docker restart jarvis-work` leaves `jarvis-personal` untouched).
...
- Optionally launches `jarvis dashboard` as a background side-process when `JARVIS_DASHBOARD=1` (see [Running the dashboard](#running-the-dashboard))
- Then runs `jarvis` with whatever arguments you pass

:::warning
Do not override the image entrypoint unless you keep `/opt/jarvis/docker/entrypoint.sh` in the command chain. The entrypoint drops root privileges to the `jarvis` user before gateway state files are created. Starting `jarvis gateway run` as root inside the official image is refused by default because it can leave root-owned files in `/opt/data` and break later dashboard or gateway starts. Set `JARVIS_ALLOW_ROOT_GATEWAY=1` only when you intentionally accept that risk.
:::
...
The container's entrypoint drops privileges to the non-root `jarvis` user (UID 10000) via `gosu`. If your host `~/.jarvis/` is owned by a different UID, set `JARVIS_UID`/`JARVIS_GID` to match your host user, or ensure the data directory is writable:
