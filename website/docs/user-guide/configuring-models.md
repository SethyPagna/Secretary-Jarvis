...
Every auxiliary task defaults to `auto` — meaning Jarvis uses your main model for that job too. Override a specific task when you want a cheaper or faster model for a side-job.
...
`provider: auto` with `model: ''` tells Jarvis to use the main model for that task.
...
To list providers/models without launching the picker, use the dashboard or the REST endpoints below. To inspect what the CLI will actually use right now: `jarvis config get model` and `jarvis status`.
