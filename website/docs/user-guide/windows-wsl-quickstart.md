...
- Using the **OpenAI-compatible API server** (exposed by `jarvis gateway` when `API_SERVER_ENABLED=true`) from a Windows-side tool. See the [API Server feature page](/docs/user-guide/features/api-server).
...
The Jarvis [Tool Gateway](/docs/user-guide/features/tool-gateway) and the API server are long-lived processes. In WSL2 you have a few options for keeping them up.
...
- **[MCP guide → WSL → Windows Chrome](/docs/guides/use-mcp-with-jarvis#wsl2-bridge-jarvis-in-wsl-to-windows-chrome)** — controlling your signed-in Windows Chrome from Jarvis in WSL.
