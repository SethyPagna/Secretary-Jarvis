...
sidebar_position: 2
title: "Installation"
description: "Install JARVIS on Linux, macOS, WSL2, native Windows (early beta), or Android via Termux"
---

# Installation

Get JARVIS up and running in under two minutes with the one-line installer.
...
:::warning Early BETA
Native Windows support is **early beta**. It installs and works for the common paths, but hasn't been road-tested as broadly as our POSIX installers. Please [file issues](https://github.com/JARVISProject/jarvis-agent/issues) when you hit rough edges. For the most battle-tested setup on Windows today, use the Linux/macOS one-liner above inside **WSL2** instead.
:::

Open PowerShell and run:

```powershell
iex (irm https://raw.githubusercontent.com/JARVISProject/jarvis-agent/main/scripts/install.ps1)
```

The installer handles **everything**: `uv`, Python 3.11, Node.js 22, `ripgrep`, `ffmpeg`, **and a portable Git Bash** (PortableGit — a self-contained Git-for-Windows distribution that ships `bash.exe` and the full POSIX toolchain Jarvis uses for shell commands; on 32-bit Windows the installer falls back to MinGit, which lacks bash and disables terminal-tool / agent-browser features).  It clones the repo under `%LOCALAPPDATA%\jarvis\jarvis-agent`, creates a virtualenv, and adds `jarvis` to your **User PATH**.  Restart your terminal (or open a new PowerShell window) after the install so PATH picks up.

**How Git is handled:**
1. If `git` is already on your PATH, the installer uses your existing install.
2. Otherwise it downloads portable **PortableGit** (~50MB, from the official `git-for-windows` GitHub release) and unpacks it to `%LOCALAPPDATA%\jarvis\git`.  No admin rights required.  Completely isolated — it won't interfere with any system Git install, broken or otherwise.  (On 32-bit Windows it falls back to MinGit because PortableGit ships only 64-bit and ARM64 assets; bash-dependent Jarvis features won't work on 32-bit hosts.)

**Why not use winget?**  Earlier designs auto-installed Git via `winget install Git.Git`, but winget fails badly when a system Git install is in a partial or broken state (exactly when users need the installer to just work).  The portable Git approach sidesteps winget, the Windows installer registry, and any existing system Git entirely.  If the Jarvis Git install itself ever breaks, `Remove-Item %LOCALAPPDATA%\jarvis\git` and re-run the installer — no system impact, no uninstall drama.

The installer also sets `JARVIS_GIT_BASH_PATH` to the located `bash.exe` so Jarvis resolves it deterministically in fresh shells.

If you prefer WSL2, the Linux installer above works inside it; both native and WSL installs can coexist without conflict (native data lives under `%LOCALAPPDATA%\jarvis`, WSL data lives under `~/.jarvis`).

**Desktop installer (alternative):** A thin GUI installer is also available — download Jarvis Desktop, run the `.exe`, and on first launch it calls `install.ps1` under the hood to provision Python (via `uv`), Node, PortableGit, and the rest of the dependencies. The desktop app and the PowerShell-installed CLI share the same install and data directories, so you can use either or both. See the [Windows (Native) guide](../user-guide/windows-native#desktop-installer-alternative) for details.

### Android / Termux

Jarvis now ships a Termux-aware installer path too:
...
- **CLI (`jarvis chat`, `jarvis setup`, `jarvis gateway`, …)** — native, uses your default terminal
...
Set `JARVIS_DISABLE_WINDOWS_UTF8=1` in your environment if you hit an encoding-related bug and want to fall back to the legacy cp1252 stdio path (useful for bisecting).
:::
...
| Per-user (git installer) | `~/.jarvis/jarvis-agent/` | `~/.local/bin/jarvis` (symlink) | `~/.jarvis/` |
| Root-mode (`sudo curl … \| sudo bash`) | `/usr/local/lib/jarvis-agent/` | `/usr/local/bin/jarvis` | `/root/.jarvis/` (or `$JARVIS_HOME`) |
...
2. **As the unprivileged service user**, run the regular installer. It will detect the missing sudo, skip `--with-deps`, and install Chromium into the user's local Playwright cache:
   ...
   ```bash
   curl -fsSL https://raw.githubusercontent.com/JARVISProject/jarvis-agent/main/scripts/install.sh | bash -s -- --skip-browser
   ```
...
3. **Make `jarvis` available to the service user's shells.** The installer writes the launcher to `~/.local/bin/jarvis`. System service accounts often have a minimal PATH that doesn't include `~/.local/bin`. Either add it to the user's environment, or symlink the launcher into a system location:
   ...
   sudo ln -s /home/jarvis/.jarvis/jarvis-agent/venv/bin/jarvis /usr/local/bin/jarvis
   ```

4. **Verify:** `jarvis doctor` should now run cleanly. If you get `ModuleNotFoundError: No module named 'dotenv'`, you're invoking the repo source `jarvis` file (`~/.jarvis/jarvis-agent/jarvis`) with system Python instead of the venv launcher (`~/.jarvis/jarvis-agent/venv/bin/jarvis`) — fix step 3.
