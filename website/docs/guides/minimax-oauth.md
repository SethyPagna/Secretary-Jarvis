...
```bash
jarvis auth add minimax-cn --type oauth   # if OAuth is supported on your CN account
# or simpler:
...
Jarvis implements a PKCE device-code flow against the MiniMax OAuth endpoints:
...
```bash
JARVIS_INFERENCE_PROVIDER=minimax-oauth jarvis
```
