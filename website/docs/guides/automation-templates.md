...
Copy-paste recipes for common automation patterns. Each template uses Jarvis's built-in [cron scheduler](/docs/user-guide/features/cron) for time-based triggers and [webhook platform](/docs/user-guide/messaging/webhooks) for event-driven triggers.
...
jarvis cron create "0 2 * * *" \
  "You are a project manager triaging the JARVISProject/jarvis-agent GitHub repo.

1. Run: gh issue list --repo JARVISProject/jarvis-agent --state open --json number,title,labels,author,createdAt --limit 30
...
jarvis cron create "0 9 * * 1" \
  "Scan the JARVISProject/jarvis-agent repo for documentation drift.

1. Run: gh pr list --repo JARVISProject/jarvis-agent --state merged --json number,title,files,mergedAt --limit 30
