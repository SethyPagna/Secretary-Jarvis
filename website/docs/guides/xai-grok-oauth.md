...
See [OAuth over SSH / Remote Hosts](./oauth-over-ssh.md#browser-only-remote-cloud-shell--codespaces--ec2-instance-connect) for the full walkthrough. Regression fix for [#26923](https://github.com/JARVISProject/jarvis-agent/issues/26923).
...
```bash
jarvis config set model.default grok-4.3
...
| `JARVIS_INFERENCE_PROVIDER` | Force the active provider at runtime, e.g. `JARVIS_INFERENCE_PROVIDER=xai-oauth jarvis`. |
...
This is **not** a stale-token problem — re-running `jarvis model` won't change it. xAI's backend has been seen to restrict OAuth API access to specific SuperGrok tiers despite the in-app subscription being active (issue [#26847](https://github.com/JARVISProject/jarvis-agent/issues/26847)).
