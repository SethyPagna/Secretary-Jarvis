...
[PR #12550](https://github.com/JARVISProject/jarvis-agent/pull/12550) proposed a
...
(`jarvis-dialog-bridge.invalid`). `Fetch.enable` intercepts those XHRs
...
- `toolsets.py` — register `browser_dialog` in `browser`, `jarvis-acp`, `jarvis-api-server`, core toolsets (gated on CDP reachability)
...
- `jarvis_cli/config.py` — add `browser.dialog_policy` and `browser.dialog_timeout_s` to `DEFAULT_CONFIG`
