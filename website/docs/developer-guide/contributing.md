...
- **Profile-safe paths**: Never hardcode `~/.jarvis` — use `get_jarvis_home()` from `jarvis_constants` for code paths and `display_jarvis_home()` for user-facing messages. See [AGENTS.md](https://github.com/JARVISProject/jarvis-agent/blob/main/AGENTS.md#profiles-multi-instance-support) for full rules.

## Cross-Platform Compatibility

Jarvis officially supports **Linux, macOS, WSL2, and native Windows (early beta — via PowerShell install)**.  Native Windows uses Git Bash (from [Git for Windows](https://git-scm.com/download/win)) for shell commands.  A few features require POSIX kernel primitives and are gated: the dashboard's embedded PTY terminal pane (`/chat` tab) is WSL2-only. The native-Windows path is new and moves fast — if you're doing Windows-heavy dev, expect to hit and fix rough edges.
...
- **Discord**: [discord.gg/JARVISProject](https://discord.gg/JARVISProject)
- **GitHub Discussions**: For design proposals and architecture discussions
- **Skills Hub**: Upload specialized skills and share with the community

## License

By contributing, you agree that your contributions will be licensed under the [MIT License](https://github.com/JARVISProject/jarvis-agent/blob/main/LICENSE).
