...
- **Profile-safe paths**: Never hardcode `~/.jarvis` — use `get_jarvis_home()` from `jarvis_constants` for code paths and `display_jarvis_home()` for user-facing messages. See [AGENTS.md](https://github.com/NousResearch/jarvis-agent/blob/main/AGENTS.md#profiles-multi-instance-support) for full rules.
...
By contributing, you agree that your contributions will be licensed under the [MIT License](https://github.com/NousResearch/jarvis-agent/blob/main/LICENSE).
