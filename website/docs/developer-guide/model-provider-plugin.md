...
Model provider plugins declare an inference backend — an OpenAI-compatible endpoint, an Anthropic Messages server, a Codex-style Responses API, or a Bedrock-native surface — that Jarvis can route `AIAgent` calls through. Every built-in provider (OpenRouter, Anthropic, GMI, DeepSeek, Nvidia, …) ships as one of these plugins. Third parties can add their own by dropping a directory under `$JARVIS_HOME/plugins/model-providers/` with zero changes to the repo.
...
| Field | Type | Purpose |
|---|---|---|
| `name` | str | Canonical id — matches `--provider` choices and `JARVIS_INFERENCE_PROVIDER` |
...
- [Building a Jarvis Plugin](/docs/guides/build-a-jarvis-plugin) — general plugin authoring
