...
| `requires_env` | Interactive credential prompt during `jarvis plugins install` (see [Build a Jarvis Plugin](/docs/guides/build-a-jarvis-plugin#gate-on-environment-variables) for the rich format) |
...
`my_backend_web_package` must expose a top-level `register` function. See [Distribute via pip](/docs/guides/build-a-jarvis-plugin#distribute-via-pip) in the general plugin guide for the full setup.
