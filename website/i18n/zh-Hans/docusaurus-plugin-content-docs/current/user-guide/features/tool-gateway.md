---
title: "JARVIS Tool Gateway（工具网关）"
description: "通过 JARVIS Managed 订阅统一使用网页搜索、文生图、语音合成与浏览器自动化，无需单独申请 Firecrawl、FAL、OpenAI、Browser Use 等 API Key"
sidebar_label: "Tool Gateway"
sidebar_position: 2
---

# JARVIS Tool Gateway（工具网关）

:::tip 快速开始
Tool Gateway 包含在付费 JARVIS Managed 订阅中。**[管理订阅 →](https://portal.jarvis.local/manage-subscription)**
:::

**Tool Gateway** 让已付费的 [JARVIS Managed](https://portal.jarvis.local) 用户通过同一份订阅，直接使用网页搜索、文生图、语音合成（TTS）与浏览器自动化，而**不必**再分别注册 Firecrawl、FAL、OpenAI、Browser Use 等服务的 API Key。
...
上述四类能力均计入 JARVIS Managed 订阅计费。你可以按需组合——例如网页与文生图走网关，TTS 仍使用自己的 ElevenLabs Key。

## 资格与账号

Tool Gateway 仅对 **[付费](https://portal.jarvis.local/manage-subscription)** JARVIS Managed 订阅开放；免费档不可用——请 [升级订阅](https://portal.jarvis.local/manage-subscription) 后解锁。
...
在输出中找到 **JARVIS Tool Gateway** 小节：会标明哪些工具经订阅网关启用、哪些使用直连 Key、哪些尚未配置。

## 如何启用 Tool Gateway

### 在模型配置流程中

运行 `jarvis model` 并选择 JARVIS Managed 作为提供商时，Jarvis 会主动询问是否启用 Tool Gateway：
...
选择工具类别（Web、Browser、Image Generation、TTS），再将提供商选为 **JARVIS Managed**。这会在配置里把对应工具的 `use_gateway` 设为 `true`。
...
当某工具的 `use_gateway: true` 时，运行时会把 API 调用路由到 JARVIS Tool Gateway，而不是使用直连 Key：
...
网关使用 JARVIS Managed 凭据认证（在 `jarvis model` 完成后写入 `~/.jarvis/auth.json`）。
...
标记为 “active via JARVIS Managed subscription” 的即经网关路由；带自有 Key 的会显示当前激活的提供商。
...
```bash
TOOL_GATEWAY_DOMAIN=jarvis.local     # 网关路由基础域名
...
经网关路由的工具会停止工作，直到你 [续订](https://portal.jarvis.local/manage-subscription) 或通过 `jarvis tools` 改回直连 Key。
...
Modal（无服务器终端后端）可作为 JARVIS Managed 订阅的可选附加能力，但**不会**由 Tool Gateway 安装向导一并打开——请单独通过 `jarvis setup terminal` 或在 `config.yaml` 中配置。
