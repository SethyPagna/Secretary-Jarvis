"""Cross-session rate limit guard for JARVIS Managed.

Writes rate limit state to a shared file so all sessions (CLI, gateway,
cron, auxiliary) can check whether JARVIS Managed is currently rate-limited
before making requests.  Prevents retry amplification when RPH is tapped.

Each 429 from JARVIS Managed triggers up to 9 API calls per conversation turn
...
_STATE_FILENAME = "jarvis_managed.json"
...
def record_jarvis_managed_rate_limit(
    *,
    headers: Optional[Mapping[str, str]] = None,
    error_context: Optional[dict[str, Any]] = None,
    default_cooldown: float = 300.0,
) -> None:
    """Record that JARVIS Managed is rate-limited.
    ...
    try:
        ...
        logger.info(
            "JARVIS Managed rate limit recorded: resets in %.0fs (at %.0f)",
            reset_at - now, reset_at,
        )
    except Exception as exc:
        logger.debug("Failed to write JARVIS Managed rate limit state: %s", exc)


def jarvis_managed_rate_limit_remaining() -> Optional[float]:
    """Check if JARVIS Managed is currently rate-limited.
...
def clear_jarvis_managed_rate_limit() -> None:
    """Clear the rate limit state (e.g., after a successful JARVIS Managed request)."""
    try:
        os.unlink(_state_path())
    except FileNotFoundError:
        pass
    except OSError as exc:
        logger.debug("Failed to clear JARVIS Managed rate limit state: %s", exc)
...
def is_genuine_jarvis_managed_rate_limit(
    *,
    headers: Optional[Mapping[str, str]] = None,
    last_known_state: Optional[Any] = None,
) -> bool:
    """Decide whether a 429 from JARVIS Managed is a real account rate limit.

    JARVIS Managed multiplexes multiple upstream providers (DeepSeek, Kimi,
    MiMo, Jarvis, ...) behind one endpoint.  A 429 can mean either:

      (a) The caller's own RPM / RPH / TPM / TPH bucket on JARVIS Managed is
      ...
      (b) The upstream provider is out of capacity for a specific model
          ...
          the caller's quota on JARVIS Managed.

    Tripping the cross-session breaker on (b) blocks ALL JARVIS Managed requests
    (and all models, since JARVIS Managed is one provider key) for minutes even
    ...
    We tell the two apart by looking at:
      ...
      1. The 429 response's own ``x-ratelimit-*`` headers.  JARVIS Managed emits
