...
the shared design system for react ui @ JARVIS. components, hooks, utils, fonts, overlays, etc.

## setup

`npm i @jarvis_managed-research/ui` or `pnpm i @jarvis_managed-research/ui`

import the global styles in your app's root css file:

```css
@import 'tailwindcss';
@import '@jarvis_managed-research/ui/styles/globals.css';
```
...
```css
@import 'tailwindcss';
@import '@jarvis_managed-research/ui/styles/fonts.css';
@import '@jarvis_managed-research/ui/styles/globals.css';
```

Next.js apps should prefer `next/font/local` with the exported
`@jarvis_managed-research/ui/fonts/*` files and map them to the same `--font-*`
...
import {
  ...
  Grid
  // whatever else!
} from '@jarvis_managed-research/ui'
```
...
creates a [GitHub Release](https://github.com/JARVISResearch/design-language/releases)
