...
def _start_agent_build(sid: str, session: dict) -> None:
    ...
    Classic `jarvis` shows the prompt before constructing AIAgent; the TUI used
...
def _apply_model_switch(sid: str, session: dict, raw_input: str) -> dict:
    from jarvis_cli.model_switch import parse_model_flags, switch_model
    ...
    os.environ["JARVIS_MODEL"] = result.new_model
    os.environ["JARVIS_INFERENCE_MODEL"] = result.new_model
    ...
    # in config or env.
    #
    # JARVIS_TUI_PROVIDER is the canonical "explicit-this-process" carrier
    ...
    if result.target_provider:
        os.environ["JARVIS_INFERENCE_PROVIDER"] = result.target_provider
        os.environ["JARVIS_TUI_PROVIDER"] = result.target_provider
    if persist_global:
...
def _(rid, params: dict) -> dict:
    ...
    filename = os.path.abspath(
        f"jarvis_conversation_{_time.strftime('%Y%m%d_%H%M%S')}.json"
    )
    try:
