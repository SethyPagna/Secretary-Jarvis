...
- **Three-section layout**: Setup row, main loop container, and detail container â€” each visually distinct
...
- **Semantic color for outcomes**: Green = improvement (keep), Red = no improvement (discard) â€” not arbitrary decoration
- **Highlighted key step**: "Run training" uses `c-coral` to visually distinguish the most important step from other `c-teal` actions
- **Horizontal pipeline flow**: Training details section uses left-to-right arrow-connected nodes (GPT â†’ MuonAdamW â†’ Evaluation)
...
<svg width="100%" viewBox="0 0 680 920" xmlns="http://www.w3.org/2000/svg">
  ...
  <!-- ========================================== -->
  <!-- SECTION 1: SETUP (Human â†’ program.md â†’ AI) -->
  <!-- ========================================== -->
  ...
  <!-- Arrow: Setup row â†’ Loop (from program.md center down) -->
  ...
  <g>
    ...
    <text class="ts" x="66" y="188">~12 experiments/hour â€” runs until manually stopped</text>
  </g>
  ...
  <!-- Step 3: Run training (highlighted â€” key step) -->
  ...
  <!-- Step 4: Decision â€” val_bpb improved? -->
  <g class="node c-gray">
  ...
  <!-- Converge arrows: Keep â†’ Log, Discard â†’ Log -->
  ...
  <!-- Connection arrow: Loop â†’ Training details -->
  ...
  <g>
    <rect x="40" y="710" width="600" height="170" rx="16"
          stroke-width="1" stroke-dasharray="6 4"
          fill="var(--bg-secondary)" stroke="var(--border)"/>
    <text class="th" x="66" y="738">train.py â€” modifiable training pipeline</text>
    <text class="ts" x="66" y="756">Runs during each training step â€” single GPU, single file</text>
  </g>
  ...
  <text class="ts" x="340" y="856" text-anchor="middle" opacity=".5">climbmix-400b data Â· 8K BPE vocab Â· 300s budget Â· 2048 context</text>
...
| Run training | `c-coral` | Highlighted key step â€” the 5-min training run |
| Decision check | `c-gray` | Neutral evaluation checkpoint |
| Keep (improved) | `c-green` | Semantic success â€” val_bpb decreased |
| Discard (not improved) | `c-red` | Semantic failure â€” no improvement |
| Training pipeline nodes | `c-coral` | Training infrastructure components |
| Evaluation node | `c-amber` | Distinct from training â€” measurement/metric role |
| Containers | Neutral (dashed) | Subtle grouping that recedes behind content |

## Layout Notes

- **ViewBox**: 680Ã—920 (standard width, tall for 3 sections)
- **Three sections**: Setup row (y=30â€“98), loop container (y=142â€“670), training details (y=710â€“880)
- **Container style**: Dashed border (`stroke-dasharray="6 4"`), neutral fill (`var(--bg-secondary)`), `stroke-width="1"` â€” not colored, so inner nodes pop
...
- **Horizontal sub-flow**: Training pipeline uses left-to-right arrow-connected nodes (GPT model â†’ MuonAdamW â†’ Evaluation)
