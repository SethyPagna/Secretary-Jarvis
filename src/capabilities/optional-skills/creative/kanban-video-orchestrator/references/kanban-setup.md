...
> [JARVIS Project kanban video pipeline](https://github.com/JARVIS Project kanban video pipeline).
...
├── src/tools/                         ← optional project-local helpers
└── output/
...
`~/.jarvis/.env` and macOS Keychain (if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    if grep -q "^${var}=" ~/.jarvis/.env 2>/dev/null && \
       [ -n "$(grep "^${var}=" ~/.jarvis/.env | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ~/.jarvis/.env or Keychain (${kc_account}/${kc_service})"
    return 1
}
