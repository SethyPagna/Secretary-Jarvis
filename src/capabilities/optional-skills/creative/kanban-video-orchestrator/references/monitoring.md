...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `jarvis profile list` (profile exists?), `jarvis status` (src/gateway/dispatcher up?), and verify tenant. |
