...
### Visual / rendering skills (`jarvis-agent/src/capabilities/skills/creative/`)
...
### Design / pre-production skills (`jarvis-agent/src/capabilities/skills/creative/`)
...
### Audio / media skills (`jarvis-agent/src/capabilities/skills/creative/` + `src/capabilities/skills/media/`)
...
### Kanban infrastructure (`jarvis-agent/src/capabilities/skills/devops/`)
...
key is present in `~/.jarvis/.env` (or macOS Keychain) before firing the kanban.
...
preference: macOS Keychain → `~/.jarvis/.env` → environment variable.
