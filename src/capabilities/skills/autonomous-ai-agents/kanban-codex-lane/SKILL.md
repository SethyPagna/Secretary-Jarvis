...
description: Use when a Jarvis Kanban worker wants to run Codex CLI as an isolated implementation lane while Jarvis keeps ownership of task lifecycle, reconciliation, testing, and handoff.
version: 1.0.0
author: JARVIS
license: MIT
metadata:
  jarvis:
    ...
    related_skills: [kanban-worker, codex, jarvis-agent]
---
...
- [ ] Jarvis ran the canonical tests itself, using `ops/scripts/public/run_tests.sh` for JARVIS or the repo's documented wrapper for other repos.
...
```json
{
  "codex_lane": {
    ...
    "tests_run": [
      {"command": "ops/scripts/public/run_tests.sh tests/src/tools/test_x.py", "exit_code": 0, "owner": "jarvis"},
