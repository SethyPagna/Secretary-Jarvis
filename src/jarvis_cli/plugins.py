...
1. **Bundled plugins** – ``<repo>/src/plugins/<name>/`` (shipped with jarvis-agent;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.jarvis/src/plugins/<name>/``
3. **Project plugins** – ``./.jarvis/src/plugins/<name>/`` (opt-in via
...
def get_bundled_plugins_dir() -> Path:
    """Locate the bundled ``src/plugins/`` directory.
...
# stderr in addition to ~/.jarvis/logs/agent.log. Aimed at plugin authors
...
VALID_HOOKS: Set[str] = {
    ...
    # Approval lifecycle hooks. Fired by src/tools/approval.py when a dangerous
    # command needs user approval -- fires BOTH for CLI-interactive prompts
    # and for src/gateway/ACP approvals (Telegram, Discord, Slack, TUI, etc.).
...
class PluginManifest:
    ...
    #              in ~/.jarvis/src/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``jarvis plugins list``. For a flat plugin at
    # ``src/plugins/disk-cleanup/`` the key is ``disk-cleanup``; for a nested
    # category plugin at ``src/plugins/image_gen/openai/`` the key is
...
class PluginManager:
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        # 1. Bundled plugins (<repo>/src/plugins/<name>/)
        #
        # Repo-shipped plugins live next to src/jarvis_cli/. Two layouts are
        # supported (see ``_scan_directory`` for details):
        #
        #   - flat: ``src/plugins/disk-cleanup/plugin.yaml`` (standalone)
        #   - category: ``src/plugins/image_gen/openai/plugin.yaml`` (backend)
        #
        # ``memory/``, ``context_engine/``, and ``model-providers/`` are
        # skipped at the top level — they have their own discovery systems
        # (src/plugins/memory/__init__.py, src/providers/__init__.py). ``platforms/``
        ...
        # 2. User plugins (~/.jarvis/src/plugins/)
        ...
        # 3. Project plugins (./.jarvis/src/plugins/)
        ...
        for manifest in winners.values():
            ...
            # Model provider plugins are loaded by src/providers/__init__.py
            ...
            if manifest.kind == "model-provider":
                ...
                self._plugins[lookup_key] = loaded
                logger.debug(
                    "Skipping '%s' (model-provider, handled by src/providers/ discovery)",
                    lookup_key,
                )
                continue
    ...
    def _parse_manifest(
        ...
        try:
            ...
            # so they're routed to src/plugins/memory discovery instead of being
            # loaded by the general PluginManager (which has no
            # register_memory_provider on PluginContext). Mirrors the
            # heuristic in src/plugins/memory/__init__.py:_is_memory_provider_dir.
                ...
                if init_file.exists():
                    try:
                        ...
                        if (
                            ...
                            # to src/providers/__init__.py discovery.
                            kind = "model-provider"
                            logger.debug(
