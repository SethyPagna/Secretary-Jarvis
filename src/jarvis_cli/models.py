...
# ---------------------------------------------------------------------------
# JARVIS Managed free-model helper
# ---------------------------------------------------------------------------
# The JARVIS Managed models endpoint is the source of truth for which models
...
# ---------------------------------------------------------------------------
# JARVIS Managed recommended models
#
...
CANONICAL_PROVIDERS: list[ProviderEntry] = [
    ...
    ProviderEntry("stepfun",        "StepFun Step Plan",       "StepFun Step Plan (src/agent/coding models via Step Plan API)"),
...
# Auto-extend CANONICAL_PROVIDERS with any provider registered in src/providers/
# that is not already in the list above.  Adding src/plugins/model-providers/<name>/
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model a user would be offered first in the ``jarvis model`` picker.

    Used as a fallback when the user has configured a provider but never
    selected a model (e.g. ``jarvis auth add openai-codex`` without
    ``jarvis model``).
    """
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    **Permissive when the field is missing.** Some OpenRouter-compatible gateways
    (JARVIS Managed, private mirrors, older catalog snapshots) don't populate
...
def get_curated_jarvis_managed_model_ids() -> list[str]:
    ...
    ``ops/docs/api/model-catalog.json``); falls back to the in-repo
...
def fetch_models_with_pricing(
    ...
    *,
    ...
    Works with any OpenRouter-compatible endpoint (OpenRouter, JARVIS Managed).
    """
...
def fetch_ai_gateway_pricing(
    ...
    *,
    ...
    """Fetch Vercel AI Gateway /v1/models and return jarvis-shaped pricing.

    Vercel uses ``input`` / ``output`` field names; jarvis's picker expects
...
# _is_third_party_anthropic_endpoint in src/agent/anthropic_adapter.py), so
...
def resolve_fast_mode_overrides(model_id: Optional[str]) -> dict[str, Any] | None:
    ...
    ``_build_api_kwargs`` in src/agent/runtime.py — each API path handles its own
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    # Handles any provider registered in src/providers/ with auth_type="api_key".
