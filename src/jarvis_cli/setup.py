...
_DOCS_BASE = "https://github.com/SethyPagna/Secretary-Jarvis"
...
def _print_setup_summary(config: dict, jarvis_home):
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {jarvis_home}/src/cron/, sessions/, logs/"
    )
    print()
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            print_info(
                "Jarvis can keep multiple credentials for one provider and rotate between"
            )
            print_info(
    ...
    if _vision_needs_setup:
        ...
        print_info("doesn't currently provide one Jarvis can auto-use for vision,")
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``jarvis model`` -> xAI Grok OAuth (SuperGrok Subscription).
    """
    try:
...
def setup_terminal_backend(config: dict):
    ...
    if selected_backend == "local":
        ...
        if use_managed_modal:
            ...
            print_info("Modal execution will use the JARVIS Managed gateway and your subscription.")
        ...
        print_info("Requires the optional SDK: pip install 'jarvis-agent[vercel]'")
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                print_warning("Install failed — run manually: pip install 'jarvis-agent[vercel]'")
                if result.stderr:
                    print_info(f"  Error: {result.stderr.strip().splitlines()[-1]}")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # bridge in src/gateway/run.py now unconditionally derives
            # JARVIS_MAX_ITERATIONS from agent.max_turns at startup.
...
def _setup_slack():
    ...
    print()
    print_info("   Full guide: https://github.com/SethyPagna/Secretary-Jarvis")
    print()
...
def _setup_webhooks():
    ...
    print_info("   https://github.com/SethyPagna/Secretary-Jarvis")
    print()
    print_info("   Open config in your editor:  jarvis config edit")
    print_info("   Open config in your editor:  jarvis config edit")
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      ...
      * ``active_provider`` in the auth store — covers OAuth device-code /
        external-OAuth providers (JARVIS Managed, Codex, Qwen, Gemini CLI, ...).
...
_OPENCLAW_SCRIPT = (
    get_optional_skills_dir(PROJECT_ROOT / "capabilities" / "optional-skills")
    / "migration"
    / "openclaw-migration"
    / "scripts"
...
def run_setup_wizard(args):
    ...
    if is_existing:
        ...
        setup_mode = prompt_choice("How would you like to set up Jarvis?", [
...
def _run_first_time_quick_setup(config: dict, jarvis_home, is_existing: bool):
    ...
    the user can customize later via ``jarvis setup <section>``.
    """
