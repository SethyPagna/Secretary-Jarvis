...
# These map to keys in src/tools/toolsets.py TOOLSETS dict.
...
def _get_effective_configurable_toolsets():
    ...
    plugins (e.g. ``src/plugins/spotify``) share their toolset key with the
...
TOOL_CATEGORIES = {
    ...
    "image_gen": {
        ...
        "icon": "🎨",
        # Per-provider rows for FAL.ai (`src/plugins/image_gen/fal`), OpenAI,
    ...
    "browser": {
        ...
        "providers": [
            {
                "name": "JARVIS Managed (Browser Use cloud)",
                "badge": "subscription",
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_jarvis_home()}/jarvis-agent && npm install")
                if result.stderr:
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      docker pull ghcr.io/sethypagna/secretary-jarvis:latest"
            )
            return
        ...
        if not camofox_dir.exists() and _npm_bin:
            ...
            # package.json so that `jarvis update` does not silently pull
...
def _get_platform_tools(
    ...
    *,
    ...
    from tools.toolsets import resolve_toolset, TOOLSETS
...
# ("JARVIS Managed" managed-gateway entry, "Firecrawl Self-Hosted")
...
# src/plugins/browser/<vendor>/; this helper is the sole source of provider rows
...
# ("JARVIS Managed", "Local Browser", "Camofox") — see the comment block
...
def _visible_providers(cat: dict, config: dict) -> list[dict]:
    ...
    # later) so the picker lists them alongside FAL / JARVIS Managed.
    ...
    # remaining hardcoded rows ("JARVIS Managed", "Firecrawl
    ...
    # the hardcoded "JARVIS Managed" / "Local Browser" / "Camofox" rows
...
def _configure_provider(provider: dict, config: dict):
    ...
    if not env_vars:
        ...
        if managed_feature:
            _print_info("  Requests for this tool will use your JARVIS Managed subscription.")
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    ...
    print(color("  Guide: https://github.com/SethyPagna/Secretary-Jarvis", Colors.DIM))
    print()
