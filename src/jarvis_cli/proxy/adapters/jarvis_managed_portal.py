...
:func:`jarvis_cli.auth.resolve_jarvis_managed_runtime_credentials`.
"""
...
from jarvis_cli.auth import (
    ...
    _load_auth_store,
    _auth_store_lock,
    _is_terminal_jarvis_managed_refresh_error,
...
class JarvisManagedPortalAdapter(UpstreamAdapter):
    ...
    def __init__(self) -> None:
        ...
        # and persistence are handled by resolve_jarvis_managed_runtime_credentials().
    ...
    def get_retry_credential(
        ...
        *,
        ...
        if failed_credential.bearer.count(".") != 2:
            return None
        logger.info("proxy: JARVIS Managed upstream rejected bearer; retrying with legacy session key")
        return self._get_credential(
    ...
    def _get_credential(self, *, inference_auth_mode: str) -> UpstreamCredential:
        with self._lock:
            ...
            if state is None:
                raise RuntimeError(
                    "Not logged into JARVIS Managed. Run `jarvis login jarvis_managed` first."
                )

            try:
                refreshed = resolve_jarvis_managed_runtime_credentials(
            ...
            if not agent_key:
                raise RuntimeError(
                    ...
                    "Try `jarvis login jarvis_managed` to re-authenticate."
                )

            base_url = (
                _validate_jarvis_managed_inference_url_from_network(refreshed.get("base_url"))
    ...
    def _read_state(self) -> Optional[Dict[str, Any]]:
        ...
        state = providers.get("jarvis_managed")
        ...
        *,
        ...
        try:
            with _auth_store_lock():
                ...
                providers["jarvis_managed"] = state
                _save_auth_store(store)
            _write_shared_jarvis_managed_state(state)
        except Exception as exc:
            logger.warning("proxy: failed to persist JARVIS Managed quarantine state: %s", exc)


__all__ = ["JarvisManagedPortalAdapter"]
