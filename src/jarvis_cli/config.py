...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``jarvis logs``
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    ...
    Resolution order:
    1. Stamped ``~/.jarvis/.install_method`` file (written by installers)
    2. JARVIS_MANAGED env / .managed marker (NixOS, Homebrew)
    3. Container detection (/.dockerenv, /run/.containerenv, cgroup)
    4. .git directory presence -> 'git'
    5. Fallback -> 'pip'
    """
    stamp = get_jarvis_home() / ".install_method"
    try:
...
def stamp_install_method(method: str) -> None:
    """Write the install method to ~/.jarvis/.install_method."""
    stamp = get_jarvis_home() / ".install_method"
    try:
...
DEFAULT_CONFIG = {
    ...
    "terminal": {
        ...
        # bundled Jarvis image, which drops to the `jarvis` user via gosu).
    ...
    # a plugin in src/plugins/context_engine/<name>/ or ~/.jarvis/src/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    "delegation": {
        ...
        # Orchestrator role controls (see src/tools/delegate_tool.py:_get_max_spawn_depth
    ...
    # Skills — external skill directories for sharing skills across src/tools/agents.
    ...
    "security": {
        ...
        # ``src/jarvis_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
    ...
    "kanban": {
        ...
        # falls back to the default profile (the one `jarvis` launches with
    ...
    "sessions": {
        ...
        # per (roughly) min_interval_hours at CLI/src/gateway/cron startup.
    ...
    # Contextual first-touch onboarding hints (see src/agent/onboarding.py).
    ...
    "updates": {
        ...
        # can be restored with ``jarvis import <path>``.  Off by default —
        # on large JARVIS_HOME directories the zip can add minutes to every
...
# selection step (JARVIS Managed / OpenRouter / Custom endpoint), so this
...
OPTIONAL_ENV_VARS = {
    ...
    # env blocklist in src/tools/environments/local.py does NOT rewrite them —
    # skills legitimately need these passed through to curl via
    # src/tools/env_passthrough.py when the user's skill calls out.
    "NOTION_API_KEY": {
...
def get_custom_provider_context_length(
    ...
    Before this helper existed, the lookup was duplicated in ``src/agent/runtime.py``'s
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    # Old configs and the packaged example template had a flat `stt.model` key
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$JARVIS_HOME/src/plugins/`` for currently installed user plugins.
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    that appears in src/gateway/run.py, src/cron/scheduler.py, main.py, etc.
...
def set_config_value(key: str, value: str):
    ...
    _config_to_env_sync = {
        ...
        # gateway bridges it in src/gateway/run.py. Persisting to .env causes
...
# Any provider registered in src/providers/ with auth_type="api_key" automatically
...
# Bundled platform plugins under ``src/plugins/platforms/*/plugin.yaml`` declare
