...
Usage:
    python src/jarvis_cli/terminal.py                          # Start interactive mode with all tools
    python src/jarvis_cli/terminal.py --toolsets web,terminal  # Start with specific toolsets
    python src/jarvis_cli/terminal.py --skills jarvis-agent-dev,github-auth
    python src/jarvis_cli/terminal.py --list-tools             # List available tools and exit
"""

# IMPORTANT: jarvis_cli.bootstrap must be the very first import — UTF-8 stdio
# on Windows.  No-op on POSIX.  See src/jarvis_cli/bootstrap.py for full rationale.
try:
...
try:
    from jarvis_cli.pt_input_extras import install_shift_enter_alias, install_ctrl_enter_alias
...
def _strip_reasoning_tags(text: str) -> str:
    ...
    ``src/agent/runtime.py::_strip_think_blocks`` and the stream consumer's
...
def load_cli_config() -> Dict[str, Any]:
    ...
    defaults = {
        ...
        "onboarding": {
            # First-touch hint flags (see src/agent/onboarding.py).  Each hint is
    ...
    if config_path.exists():
        try:
            ...
            # location is model.provider (written by `jarvis model`).
    ...
    # Normalize config key: the new config system (src/jarvis_cli/config.py) and all
    ...
    # src/gateway/run.py but may lazily import jarvis_cli.terminal as src/jarvis_cli/terminal.py (triggering this code).
    ...
    # where it was already set correctly by src/gateway/run.py's config bridge.
    ...
    # Compression config is read directly from config.yaml by src/agent/runtime.py and
...
# Initialize centralized logging early — agent.log + errors.log in ~/.jarvis/logs/.
...
# prompt; the full src/agent/tool registry is initialized on first use.
...
class JarvisCLI:
    ...
    def __init__(
        ...
        Args:
            ...
            provider: Inference provider ("auto", "openrouter", "jarvis_managed", "openai-codex", "zai", "kimi-coding", "minimax", "minimax-cn")
        ...
        # by `jarvis chat --ignore-rules` in src/jarvis_cli/main.py. When true we
    ...
    def _build_status_bar_text(self, width: Optional[int] = None) -> str:
        ...
        try:
            ...
            yolo_active = bool(os.getenv("JARVIS_YOLO_MODE"))
            if width < 52:
    ...
    def show_banner(self):
        ...
        if ctx_len and ctx_len <= 8192:
            ...
            self._console_print(
                "[dim]   Jarvis needs 16k–32k minimum. Tool schemas + system prompt alone use ~4k–8k.[/]"
            )
    ...
    def _handle_tools_command(self, cmd: str):
        ...
        def _run_capture(ns: Namespace) -> None:
            ...
            # Buffer reports isatty()=True so color() in src/jarvis_cli/colors.py
    ...
    def show_toolsets(self):
        ...
        print("  Example: python src/jarvis_cli/terminal.py --toolsets web,terminal")
        print()
    ...
    def _handle_model_switch(self, cmd_original: str):
        ...
        """
        from jarvis_cli.model_switch import switch_model, parse_model_flags
    ...
    def _show_gateway_status(self):
        ...
        try:
            ...
            print("    python src/jarvis_cli/terminal.py --gateway")
            print()
    ...
    def process_command(self, command: str) -> bool:
        ...
        # change in src/jarvis_cli/commands.py instead of touching every dispatch site.
        ...
        if canonical in {"quit", "exit"}:
            ...
            try:
                ...
                if not plugins:
                    ...
                    print(f"Drop plugin directories into {display_jarvis_home()}/src/plugins/ to get started.")
                else:
                ...
                if qcmd.get("type") == "exec":
                    ...
                    if exec_cmd:
                        try:
                            ...
                            # shell snippets from config.yaml — not src/agent/LLM controlled.
                            result = subprocess.run(
    ...
    def _maybe_continue_goal_after_turn(self) -> None:
        ...
        ``_handle_message`` in ``src/gateway/run.py``.
        """
        mgr = self._get_goal_manager()
    ...
    def _manual_compress(self, cmd_original: str = ""):
        ...
        with self._busy_command("Compressing context..."):
            try:
                ...
                # session on the agent (src/agent/runtime.py::_compress_context). Sync the
    ...
    def _show_usage(self):
        ...
        if self.verbose:
            ...
            # src/tools/run_agent/etc. in quiet mode. Setting logger.setLevel
            ...
            # Console quietness is enforced by jarvis_cli.logging_config not
    ...
    def _reload_skills(self) -> None:
        ...
        try:
            ...
            # Sync src/jarvis_cli/terminal.py's module-level _skill_commands so all consumers
    ...
    def chat(self, message, images: list = None) -> Optional[str]:
        ...
        # See src/agent/image_routing.py for the decision table.
        if images:
            try:
        ...
        try:
            ...
            def run_agent():
                ...
                # by src/acp_adapter/server.py for ACP sessions.
                set_sudo_password_callback(self._sudo_password_callback)
                set_approval_callback(self._approval_callback)
                try:
                    set_secret_capture_callback(self._secret_capture_callback)
            ...
            # (src/gateway/run.py around line 9983).
            if (
                self.agent
    ...
    def _print_exit_summary(self):
        ...
        if msg_count > 0:
            ...
            print(f"  jarvis --resume {self.session_id}")
            if session_title:
                print(f"  jarvis -c \"{session_title}\"")
            print()
...
def main(
    ...
    Examples:
        python src/jarvis_cli/terminal.py                            # Start interactive mode
        python src/jarvis_cli/terminal.py --toolsets web,terminal    # Use specific toolsets
        python src/jarvis_cli/terminal.py --skills jarvis-agent-dev,github-auth
        python src/jarvis_cli/terminal.py -q "What is Python?"       # Single query mode
        python src/jarvis_cli/terminal.py -q "Describe this" --image ~/storage/shared/Pictures/cat.png
        python src/jarvis_cli/terminal.py --list-tools               # List tools and exit
        python src/jarvis_cli/terminal.py --resume 20260225_143052_a1b2c3  # Resume session
        python src/jarvis_cli/terminal.py -w                         # Start in isolated git worktree
        python src/jarvis_cli/terminal.py -w -q "Fix issue #123"     # Single query in worktree
    """
    global _active_worktree
