...
Both the CLI (src/jarvis_cli/terminal.py) and gateway (src/gateway/run.py) /model handlers
...
def is_legacy_jarvis_non_agentic(model_name: str) -> bool:
    ...
    Callers in :mod:`src/jarvis_cli/terminal.py` and here should go through this single helper
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from src/jarvis_cli/models.py (OPENROUTER_MODELS,
    ...
    # both (src/gateway/CLI invoke ``get_compatible_custom_providers()`` which
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # (see src/jarvis_cli/main.py::_save_custom_provider); older
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            # downstream readers (src/agent/models_dev.py, src/gateway/run.py,
            # src/agent/runtime.py, src/jarvis_cli/config.py) already consume that dict.
