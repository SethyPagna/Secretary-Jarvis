...
def get_jarvis_home() -> Path:
    ...
    template in ``src/jarvis_cli/gateway.py`` and the kanban dispatcher in
    ``src/jarvis_cli/kanban_db.py``).  See https://github.com/JARVISProject/jarvis-agent/issues/18594.
    """
    ...
    if not _profile_fallback_warned:
        try:
            # Inline the default-root resolution from get_default_jarvis_root()
            # to stay import-safe (this function is called from module scope
            # in 30+ files; we cannot afford to trigger logging setup here).
            active_path = (Path.home() / ".jarvis" / "active_profile")
...
def get_subprocess_home() -> str | None:
    ...
    When ``{JARVIS_HOME}/home/`` exists on disk, subprocesses should use it
...
def get_config_path() -> Path:
    ...
    in 7+ files (skill_utils.py, src/jarvis_cli/logging_config.py, src/jarvis_cli/time_utils.py, etc.).
    """
