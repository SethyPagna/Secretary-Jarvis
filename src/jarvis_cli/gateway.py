        ...
        for scope_args in [["systemctl", "--user"], ["systemctl"]]:
            try:
                result = subprocess.run(
                    scope_args + ["list-units", "jarvis-gateway*",
...
def _graceful_restart_via_sigusr1(pid: int, drain_timeout: float) -> bool:
    ...
    SIGUSR1 is wired in src/gateway/run.py to ``request_restart(via_service=True)``
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    patterns = [
        ...
        "jarvis_cli.main --profile",
        "jarvis_cli.main -p",
        "src/jarvis_cli/main.py gateway",
        "src/jarvis_cli/main.py --profile",
        "src/jarvis_cli/main.py -p",
        "jarvis gateway",
        "src/gateway/run.py",
    ]
    ...
    def _matches_current_profile(command: str) -> bool:
        if current_profile_name:
            return (
                ...
                or f"JARVIS_HOME={current_home}" in command
            )
        ...
        if "--profile " in command or " -p " in command:
            return False
        if "JARVIS_HOME=" in command and f"JARVIS_HOME={current_home}" not in command:
            return False
        return True
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}jarvis gateway restart{scope_flag}")
...
def install_linux_gateway_from_setup(force: bool = False, enable_on_startup: bool = True) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            print_warning("  System service install requires sudo, so Jarvis can't create it from this user session.")
            if run_as_user:
                print_info(f"  After setup, run: sudo jarvis gateway install --system --run-as-user {run_as_user}")
            else:
                print_info("  After setup, run: sudo jarvis gateway install --system --run-as-user <your-user>")
...
User={username}
Group={group_name}
ExecStart={python_path} -m jarvis_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run --replace
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Jarvis install")
    return True
...
def systemd_install(
    ...
    print()
    print("Next steps:")
    print(f"  {'sudo ' if system else ''}jarvis gateway start{scope_flag}              # Start the service")
    print(f"  {'sudo ' if system else ''}jarvis gateway status{scope_flag}             # Check status")
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {'sudo ' if system else ''}jarvis gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(f"  Run after the start-limit window expires: {'sudo ' if system else ''}jarvis gateway restart{scope_flag}")
        ...
        print(f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}jarvis gateway start{scope_flag}")
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print("↻ Updated gateway launchd service definition to match the current Jarvis install")
    return True
...
def _guard_official_docker_root_gateway() -> None:
    ...
    print("  Set JARVIS_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk.")
    sys.exit(1)
...
_PLATFORMS = [
    {
        ...
        "vars": [
            ...
            {"name": "TELEGRAM_HOME_CHANNEL", "prompt": "Home channel ID (for src/cron/notification delivery, or empty to set later with /set-home)", "password": False,
             "help": "For DMs, this is your user ID. You can set it later by typing /set-home in chat."},
        ],
    },
    # Discord moved to src/plugins/platforms/discord/ — its setup metadata is
    # discovered dynamically via _all_platforms() from the platform registry
    # entry registered by src/plugins/platforms/discord/adapter.py::register().
    {
        "key": "slack",
        "label": "Slack",
        "emoji": "💼",
    ...
    },
    {
        ...
        "vars": [
            ...
            {"name": "MATRIX_HOME_ROOM", "prompt": "Home room ID (for src/cron/notification delivery, or empty to set later with /set-home)", "password": False,
    ...
    },
    {
        ...
        "vars": [
            ...
            {"name": "MATTERMOST_HOME_CHANNEL", "prompt": "Home channel ID (for src/cron/notification delivery, or empty to set later with /set-home)", "password": False,
    ...
    },
    {
        ...
        "vars": [
            ...
            {"name": "SMS_HOME_CHANNEL", "prompt": "Home channel phone number (for src/cron/notification delivery, or empty)", "password": False,
    ...
    },
    {
        ...
        "vars": [
            ...
            {"name": "FEISHU_HOME_CHANNEL", "prompt": "Home chat ID (optional, for src/cron/notifications)", "password": False,
    ...
    },
    {
        ...
        "vars": [
            ...
            {"name": "WECOM_HOME_CHANNEL", "prompt": "Home chat ID (optional, for src/cron/notifications)", "password": False,
             "help": "Chat ID for scheduled results and notifications."},
        ],
    },
    {
    ...
    },
    {
        ...
        "vars": [
            ...
            {"name": "BLUEBUBBLES_HOME_CHANNEL", "prompt": "Home channel (phone number or iMessage ID for src/cron/notifications, or empty)", "password": False,
...
def _all_platforms() -> list[dict]:
    ...
    # User-installed platform plugins under ~/.jarvis/src/plugins/ still require
...
def _setup_wecom():
    ...
    home = prompt("  Home chat ID (optional, for src/cron/notifications)", password=False)
    if home:
...
def _setup_weixin():
    ...
    print_info("  3. Jarvis will store the returned account_id/token in ~/.jarvis/.env.")
    ...
    if access_idx == 0:
        ...
        print_info("  Unknown DM users can request access and you approve them with `jarvis pairing approve`.")
    elif access_idx == 1:
...
def _setup_feishu():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info("  Unknown users can request access; approve with `jarvis pairing approve`.")
    elif access_idx == 1:
    ...
    # ── Home channel ──
    print()
    home_channel = prompt("  Home chat ID (optional, for src/cron/notifications)", password=False)
    if home_channel:
...
def _setup_qqbot():
    ...
    if user_openid:
        ...
        print()
        home_channel = prompt("  Home channel OpenID (for src/cron/notifications, or empty)", password=False)
        if home_channel:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info("    signal-cli link -n \"JarvisAgent\"")
...
def _builtin_setup_fn(key: str):
    ...
    return {
        ...
        # src/plugins/platforms/discord/adapter.py::register() and dispatched
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.jarvis/src/plugins/
    ...
    if subcmd == "install":
        ...
        if supports_systemd_services():
            if is_wsl():
                ...
                print_info("  Consider running in foreground instead: jarvis gateway run")
                print_info("  Or use tmux/screen for persistence: tmux new -s jarvis 'jarvis gateway run'")
                print()
            ...
            print()
            print("  jarvis gateway run                              # direct foreground")
            print("  tmux new -s jarvis 'jarvis gateway run'         # persistent via tmux")
            print("  nohup jarvis gateway run > ~/.jarvis/logs/gateway.log 2>&1 &  # background")
            sys.exit(1)
        elif is_container():
        ...
        if restart_all:
            ...
            if supports_systemd_services() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
                ...
                # when invoked from a gateway-hosted src/agent/tool call, foreground
        ...
        if supports_systemd_services() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                if is_termux():
                    print("  nohup jarvis gateway run > ~/.jarvis/logs/gateway.log 2>&1 &  # Best-effort background start")
                elif is_wsl():
                    print("  tmux new -s jarvis 'jarvis gateway run'         # persistent via tmux")
                    print("  nohup jarvis gateway run > ~/.jarvis/logs/gateway.log 2>&1 &  # background")
                elif is_windows():
                    print("  jarvis gateway install  # Install as Windows Scheduled Task (auto-start on login)")
                else:
                    print("  jarvis gateway install  # Install as user service")
                    print("  sudo jarvis gateway install --system  # Install as boot-time system service")
