...
Bypasses src/jarvis_cli/terminal.py entirely.  No banner, no spinner, no session_id line,
...
Env var fallbacks (used when the corresponding arg is not passed):
    ...
    - JARVIS_INFERENCE_PROVIDER  (already read by resolve_runtime_provider)
"""
...
def run_oneshot(
    ...
    Args:
        ...
        provider: Optional provider override. Falls back to
            JARVIS_INFERENCE_PROVIDER env var, then config.yaml's model.provider,
            then "auto".
...
def _run_agent(
    ...
    # Mirrors the same normalization in src/jarvis_cli/terminal.py so oneshot workers (e.g. kanban
