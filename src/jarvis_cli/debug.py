...
def _pending_file() -> Path:
    ...
    (``src/gateway/run.py::_start_cron_ticker``) which calls
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "jarvis-agent/debug-share"},
    )
...
def _delete_hint(url: str) -> str:
    ...
    paste_id = _extract_paste_id(url)
    if paste_id:
        return f"jarvis debug delete {url}"
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "jarvis-agent/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
