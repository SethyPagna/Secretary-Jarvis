...
def _is_profile_api_key_provider(provider_id: str) -> bool:
    ...
    declared in src/plugins/model-providers/<name>/ automatically dispatch to _model_flow_api_key_provider
...
_UPDATE_CRITICAL_FILES = (
    "src/jarvis_cli/main.py",
    "src/jarvis_cli/config.py",
    "src/jarvis_cli/__init__.py",
    "src/jarvis_cli/terminal.py",
    "src/agent/runtime.py",
    "src/tools/model_tools.py",
    "src/tools/toolsets.py",
    "src/jarvis_cli/constants.py",
)
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    The Vite build outputs to ``src/jarvis_cli/web_dist/`` (per config/vite.config.ts
    outDir: "../../src/jarvis_cli/web_dist"), NOT to ``desktop/web/dist/``.  Uses the Vite
...
def _find_stale_dashboard_pids() -> list[int]:
    ...
    patterns = [
        ...
        "src/jarvis_cli/main.py dashboard",
    ]
    self_pid = os.getpid()
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("? Your fork is not tracking the official Jarvis repository.")
...
def _refresh_active_lazy_features() -> None:
    ...
    optional backends moved to ``src/tools/lazy_deps.py`` and only install on
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    merges and ships in a release. ``ops/scripts/public/install_psutil_android.py``
    contains the same logic for ``ops/scripts/public/install.sh`` (fresh installs).
...
def _ensure_fhs_path_guard() -> None:
    ...
    Mirrors the post-symlink probe added to ``ops/scripts/public/install.sh`` so that
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        # orphan merge-conflict markers in src/jarvis_cli/config.py bricked
        ...
        if needs_migration:
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ? API keys require manual entry: jarvis config migrate")
            else:
                print()
    ...
    if action is None:
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} ? jarvis -p {p.name}")
                break
        print()
        return
    ...
    if plan.existing:
        ...
        if existing_is_distribution:
            ...
            print(
                ...
                "    overwrite its SOUL.md, skills/, src/cron/, and mcp.json.\n"
...
# the top-level flags declared in ``src/jarvis_cli/_parser.py``.
#
...
def _prepare_agent_startup(args) -> None:
    """Discover src/plugins/MCP/hooks for commands that can run an agent turn."""
    ...
    try:
        ...
        # so inline is safe.  Moved here from src/tools/model_tools.py module scope
...
""",
    ...
    profile_update = profile_subparsers.add_parser(
        ...
        description=(
            ...
            "distribution-owned files (SOUL.md, skills/, src/cron/, mcp.json). "
...
""",
    ...
    # response only, nothing else. Bypasses src/jarvis_cli/terminal.py entirely.
    if getattr(args, "oneshot", None):
        from jarvis_cli.oneshot import run_oneshot
