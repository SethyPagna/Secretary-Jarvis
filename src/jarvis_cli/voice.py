...
# Modifier aliases shared by desktop voice shortcuts and CLI helpers.
...
# (``_register_voice_handler`` in src/jarvis_cli/terminal.py) logs a warning when that
...
def normalize_voice_record_key_for_prompt_toolkit(raw: Any) -> str:
    ...
    Mirrors the desktop shortcut parser contract.
...
def _play_beep(frequency: int, count: int = 1) -> None:
    """Audible cue matching src/jarvis_cli/terminal.py's record/stop beeps.

    880 Hz single-beep on start (src/jarvis_cli/terminal.py:_voice_start_recording line 7532),
    660 Hz double-beep on stop (src/jarvis_cli/terminal.py:_voice_stop_and_transcribe line 7585).
...
# This Event mirrors src/jarvis_cli/terminal.py:_voice_tts_done: cleared while speak_text is
...
def stop_and_transcribe() -> Optional[str]:
    ...
    # — matches src/jarvis_cli/terminal.py:_voice_stop_and_transcribe's result.get("transcript").
...
def start_continuous(
    ...
    # (src/jarvis_cli/terminal.py:7528 comment).
...
def _continuous_on_silence() -> None:
    ...
    # CLI parity (src/jarvis_cli/terminal.py:10619-10621): wait for any in-flight TTS to
...
def speak_text(text: str) -> None:
    ...
    Mirrors src/jarvis_cli/terminal.py:_voice_speak_response exactly — same markdown strip
