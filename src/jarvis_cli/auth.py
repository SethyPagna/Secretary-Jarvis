...
# src/providers/ that is not already declared above.  New providers only need a
# src/plugins/model-providers/<name>/ plugin — no edits to this file required.
try:
...
def _save_auth_store(auth_store: Dict[str, Any]) -> Path:
    ...
    try:
        ...
        # Mirrors src/agent/google_oauth.py (#19673) and src/tools/mcp_oauth.py (#21148).
        fd = os.open(
            str(tmp_path),
...
def resolve_provider(
    ...
    *,
    ...
    # Extend with aliases declared in src/plugins/model-providers/<name>/ that aren't already mapped.
    # This keeps src/providers/ as the single source for new aliases while the
...
# The `base_url` here is the marker "cloudcode-pa://google" that src/agent/runtime.py
...
def _read_xai_oauth_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    ...
    if not state:
        raise AuthError(
            "No xAI OAuth credentials stored. Select xAI Grok OAuth (SuperGrok Subscription) in `jarvis model`.",
            provider="xai-oauth",
