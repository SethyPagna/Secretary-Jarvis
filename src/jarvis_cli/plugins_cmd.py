...
Plugins are installed from Git repositories into ``~/.jarvis/src/plugins/``.
    ...
    if user_dir.is_dir():
        ...
        for child in user_dir.iterdir():
            ...
            if manifest.get("name") == name:
                return True
    # Bundled: <repo>/src/plugins/<name>/ (or JARVIS_BUNDLED_PLUGINS on Nix).
...
def _discover_all_plugins() -> list:
    ...
    user types into ``jarvis plugins enable <key>``. For category-namespaced
...
def _user_installed_plugin_dir(name: str) -> Optional[Path]:
    """Resolved path under ``~/.jarvis/src/plugins/<name>`` if it exists."""
    plugins_dir = _plugins_dir()
    try:
        target = _sanitize_plugin_name(name, plugins_dir, allow_subdir=True)
    except ValueError:
...
def dashboard_update_user_plugin(name: str) -> dict[str, Any]:
    """``git pull`` inside ``~/.jarvis/src/plugins/<name>``."""
...
def dashboard_remove_user_plugin(name: str) -> dict[str, Any]:
    """Delete a plugin tree under ``~/.jarvis/src/plugins/`` only."""
    plugins_dir = _plugins_dir()
