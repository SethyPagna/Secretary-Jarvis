...
_PUBLIC_API_PATHS: frozenset = frozenset({
    ...
    "/api/dashboard/src/plugins/rescan",
})
...
def _skill_asset_count() -> int:
    ...
    roots = [
        ...
        Path(__file__).resolve().parents[1] / "capabilities" / "skills",
...
def _bundled_skill_catalog(*, include_disabled: bool = True) -> list[dict[str, Any]]:
    ...
    roots = [repo_root / "capabilities" / "skills", repo_root / "capabilities" / "optional-skills"]
    resource_root = Path(os.environ.get("JARVIS_RESOURCE_ROOT", "")).expanduser()
...
@app.post("/api/src/gateway/restart")
...
# in src/jarvis_cli/config.py; listed here for deterministic ordering in the UI.
...
@app.get("/api/src/providers/oauth")
...
@app.delete("/api/src/providers/oauth/{provider_id}")
...
#
#   PKCE (Anthropic):
#     1. POST /api/src/providers/oauth/anthropic/start
...
#     3. POST /api/src/providers/oauth/anthropic/submit { session_id, code }
#          ? server exchanges (code + verifier) ? tokens at console.anthropic.com
#          ? persists to ~/.jarvis/.anthropic_oauth.json AND credential pool
#          ? returns { ok: true, status: "approved" }
#
#   Device code (JARVIS Managed, OpenAI Codex):
#     1. POST /api/src/providers/oauth/{jarvis_managed|openai-codex}/start
...
#     3. UI polls GET /api/src/providers/oauth/{provider}/poll/{session_id}
...
@app.post("/api/src/providers/oauth/{provider_id}/start")
...
@app.post("/api/src/providers/oauth/{provider_id}/submit")
...
@app.get("/api/src/providers/oauth/{provider_id}/poll/{session_id}")
...
@app.delete("/api/src/providers/oauth/sessions/{session_id}")
...
@app.get("/api/src/cron/jobs/{job_id}")
...
async def create_cron_job(body: CronJobCreate, profile: str = "default"):
    try:
        ...
        _log.exception("POST /api/src/cron/jobs failed")
        raise HTTPException(status_code=400, detail=str(e))


@app.put("/api/src/cron/jobs/{job_id}")
...
@app.post("/api/src/cron/jobs/{job_id}/pause")
...
@app.post("/api/src/cron/jobs/{job_id}/resume")
...
@app.post("/api/src/cron/jobs/{job_id}/trigger")
...
@app.delete("/api/src/cron/jobs/{job_id}")
...
@app.get("/api/src/tools/toolsets")
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Returns a string like ``"/jarvis"`` (no trailing slash) or ``""`` when
...
def _discover_dashboard_plugins() -> list:
    """Scan src/plugins/*/dashboard/manifest.json for dashboard extensions.

    Checks three plugin sources (same as jarvis_cli.plugins):
    1. User plugins:    ~/.jarvis/src/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/src/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.jarvis/src/plugins/  (only if JARVIS_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()
    ...
    if os.environ.get("JARVIS_ENABLE_PROJECT_PLUGINS"):
...
@app.get("/api/dashboard/src/plugins/rescan")
...
@app.get("/api/dashboard/src/plugins/hub")
async def get_plugins_hub(request: Request):
    ...
    try:
        ...
        _log.warning("src/plugins/hub failed: %s", exc)
...
@app.post("/api/dashboard/src/plugins/{name:path}/visibility")
    ...
    for plugin in _get_dashboard_plugins():
        ...
        try:
            ...
            if router is None:
                ...
                continue
            app.include_router(router, prefix=f"/api/src/plugins/{plugin['name']}")
            _log.info("Mounted plugin API routes: /api/src/plugins/%s/", plugin["name"])
        except Exception as exc:
