...
TIPS = [
    ...
    "jarvis logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    ...
    "Desktop TTS prefers Kokoro, then OmniVoice-style local voices, then system TTS.",
    ...
    # --- Batch & Data ---
    "src/tools/batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    ...
    # --- Plugins ---
    "Three plugin types: general (src/tools/hooks), memory providers, and context engines.",
    ...
    "MCP servers send notifications/src/tools/list_changed to trigger automatic tool re-registration without restart.",
    ...
    'The packaged desktop app streams chat, terminal commands, stats, and voice through the same local backend.',
    'Home voice capture can transcribe speech, send it to chat, and speak the same response text back aloud.',
    ...
    # --- Batch ---
    "src/tools/batch_runner.py --resume content-matches completed prompts by text so dataset reorders don't re-run finished work.",
    ...
    'JARVIS_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.jarvis/src/plugins/ — trust-gated by design.',
