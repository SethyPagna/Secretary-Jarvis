...
Jobs are stored in ~/.jarvis/src/cron/jobs.json
Output is saved to ~/.jarvis/src/cron/output/{job_id}/{timestamp}.md
"""
...
def _normalize_profile(profile: Optional[str]) -> Optional[str]:
    ...
    is canonicalized with the same rules as ``jarvis -p`` and must refer to an
    ...
    # so profile directories can move with the Jarvis root.
...
def create_job(
    ...
    Args:
        ...
        workdir: Optional absolute path.  When set, the job runs as if launched
                ...
                predictably.
        profile: Optional Jarvis profile name. When set, the job runs with
                that profile's JARVIS_HOME so profile-specific config,
...
def get_due_jobs() -> List[Dict[str, Any]]:
    ...
    For recurring jobs (src/cron/interval), if the scheduled time is stale
...
def rewrite_skill_refs(
    ...
    scheduled to follow. See src/cron/scheduler.py where ``skill_view`` is
