...
Uses a file-based lock (~/.jarvis/src/cron/.tick.lock) so only one tick
...
# Platforms that support a configured src/cron/notification home target, mapped to
...
def _job_profile_context(job_id: str, profile: Optional[str]):
    """Temporarily run a job under a specific Jarvis profile.
...
def _run_job_impl(job: dict) -> tuple[bool, str, str, Optional[str]]:
    ...
    # and discoverable via session_search (same pattern as src/gateway/run.py).
    _session_db = None
    try:
    ...
    try:
        ...
        blocked_doc = (
            ...
            "the threat pattern (`src/tools/cronjob_tools.py::_CRON_THREAT_PATTERNS`)."
        )
    ...
    #   - src/tools/terminal_tool.py: background-process notification routing
    ...
    #   - src/tools/tts_tool.py: picks Opus vs MP3 based on
    #     JARVIS_SESSION_PLATFORM == "telegram".
    #   - src/tools/skills_tool.py + src/agent/prompt_builder.py: per-platform
    #     skill-disable lists and the system-prompt cache key both consume
    #     JARVIS_SESSION_PLATFORM.
    #   - src/tools/send_message_tool.py: mirror source labelling and the
    ...
    try:
        ...
        prefill_file = os.getenv("JARVIS_PREFILL_MESSAGES_FILE", "") or _cfg.get("prefill_messages_file", "")
        if prefill_file:
