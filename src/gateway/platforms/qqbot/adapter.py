...
class QQAdapter(BasePlatformAdapter):
    ...
    # ------------------------------------------------------------------
    #
    # These mirror the signatures that src/gateway/run.py detects on the adapter
    ...
    async def send_exec_approval(
        ...
        Called by ``src/gateway/run.py``'s ``_approval_notify_sync`` when the
    ...
    async def send_update_prompt(
        ...
        ``src/gateway/run.py``'s ``jarvis update --gateway`` watcher. Button
