...
Create a plugin directory in `~/.jarvis/src/plugins/` (or under `src/plugins/platforms/`
...
- `apply_yaml_config_fn: (yaml_cfg, platform_cfg) -> Optional[dict]` —
  ...
  instead of growing core `src/gateway/config.py` boilerplate per platform.
...
- `cron_deliver_env_var: str` — name of the `*_HOME_CHANNEL` env var.  When
  ...
  `src/cron/scheduler.py`'s hardcoded sets.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `src/jarvis_cli/config.py` so the setup
...
and tear down your side task in `finally`. See `src/plugins/platforms/line/`
...
See `src/plugins/platforms/irc/`, `src/plugins/platforms/teams/`, and
`src/plugins/platforms/google_chat/` for complete working examples, and
...
## 1. Core Adapter (`src/gateway/platforms/<platform>.py`)

The adapter is a subclass of `BasePlatformAdapter` from `src/gateway/platforms/base.py`.
...
## 2. Platform Enum (`src/gateway/config.py`)
...
## 3. Adapter Factory (`src/gateway/run.py`)
...
## 4. Authorization Maps (`src/gateway/run.py`)
...
## 5. Session Source (`src/gateway/session.py`)
...
## 6. System Prompt Hints (`src/agent/prompt_builder.py`)
...
## 7. Toolset (`src/tools/toolsets.py`)
...
## 8. Cron Delivery (`src/cron/scheduler.py`)
...
## 9. Send Message Tool (`src/tools/send_message_tool.py`)
...
## 10. Cronjob Tool Schema (`src/tools/cronjob_tools.py`)
...
## 11. Channel Directory (`src/gateway/channel_directory.py`)
...
## 12. Status Display (`src/jarvis_cli/status.py`)
...
## 13. Gateway Setup Wizard (`src/jarvis_cli/gateway.py`)
...
## 14. Phone/ID Redaction (`src/agent/redact.py`)

If your platform uses sensitive identifiers (phone numbers, etc.), add a
regex pattern and redaction function to `src/agent/redact.py`. This ensures
...
## 16. Tests (`tests/src/gateway/test_<platform>.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" src/gateway/ src/tools/ src/agent/ src/cron/ src/jarvis_cli/ src/tools/toolsets.py \
  --include="*.py" -l | sort -u
