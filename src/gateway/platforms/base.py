...
from jarvis_cli.utils import normalize_proxy_url

logger = logging.getLogger(__name__)

# Audio file extensions Jarvis recognizes for native audio delivery.
# Kept in sync with src/tools/send_message_tool.py and src/cron/scheduler.py via
        ...
        for attempt in range(retries + 1):
            try:
                response = await client.get(
                    url,
                    headers={
                        "User-Agent": "Mozilla/5.0 (compatible; JarvisAgent/1.0)",
                        "Accept": "image/*,*/*;q=0.8",
                    },
                )
...
class EphemeralReply(str):
    ...
    Slash-command handlers in ``src/gateway/run.py`` can return this wrapper
...
class BasePlatformAdapter(ABC):
    ...
    def extract_local_files(content: str) -> Tuple[List[str], str]:
        ...
        partition lives in ``src/gateway/run.py``.
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        if session_key in self._active_sessions:
            ...
            # Same shape as the Telegram bursty-grace path in src/gateway/run.py.
