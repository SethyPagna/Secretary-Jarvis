...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self.gateway_runner = None  # Set by src/gateway/run.py for cross-platform delivery
    ...
    def _register_slash_commands(self) -> None:
        ...
        @discord.app_commands.choices(mode=[
            ...
            # src/gateway/run.py — expose both in the slash UI so autocomplete
        ...
        # src/jarvis_cli/commands.py automatically appear as Discord slash
                ...
                if not in_allowlist and not allow_any_attachment:
                    ...
                    if max_doc_bytes and att.size and att.size > max_doc_bytes:
                        ...
                        try:
                            ...
                            # do NOT inject a path string here. ``src/gateway/run.py``
...
# Used by ``src/tools/send_message_tool._send_via_adapter`` when the gateway runner
...
#
# This block was previously hosted in ``src/tools/send_message_tool.py`` as
...
def _apply_yaml_config(yaml_cfg: dict, discord_cfg: dict) -> dict | None:
    ...
    ``src/gateway/config.py::load_gateway_config()`` before this migration.
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'jarvis-agent[discord]'",
        # Interactive setup wizard — replaces the central
        # src/jarvis_cli/setup.py::_setup_discord function.  Same shape as Teams.
        ...
        # that used to live in ``src/gateway/config.py``.  Hook contract: #24836.
