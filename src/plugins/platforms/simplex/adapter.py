...
``src/plugins/platforms/simplex/``. The Jarvis plugin loader scans the
directory at startup, calls ``register(ctx)``, and the platform
becomes available to ``src/gateway/run.py`` and ``src/tools/send_message_tool``
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        payload = {
            "corrId": f"jarvis-snd-{int(time.time() * 1000)}",
            "cmd": cmd_str,
        }
...
def interactive_setup() -> None:
    ...
    Writes to ``~/.jarvis/.env`` via ``jarvis_cli.config``.
    """
    print()
    ...
    try:
        ...
        print("jarvis_cli.config not available; set SIMPLEX_* vars manually in ~/.jarvis/.env")
        return
