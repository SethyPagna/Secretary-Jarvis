...
def _redact_sensitive(text: str) -> str:
    ...
    email addresses. src/agent/redact.py handles log-level redaction elsewhere;
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    Required environment (see src/gateway/config.py Google Chat block):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        #       src/gateway/session.py:build_session_key).
    ...
    async def edit_message(
        ...
        *,
        ...
        on this method being overridden (see src/gateway/run.py:10199 and
        src/gateway/stream_consumer.py). Without it, Google Chat shows no
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Upstream code (src/gateway/run.py and src/gateway/platforms/base.py) calls
    ...
    # ``src/plugins/platforms/google_chat/oauth.py`` for the helper
...
def _validate_config(config: PlatformConfig) -> bool:
    ...
    Mirrors the legacy dispatch entry in ``src/gateway/config.py`` so the
...
def _check_for_registry() -> bool:
    ...
    The registry pass at ``src/gateway/config.py:_apply_env_overrides`` adds
...
def interactive_setup() -> None:
    ...
    The setup wizard at ``src/jarvis_cli/gateway.py`` calls this for plugin
    ...
    print_info("")
    print_info("Full guide: ops/docs/jarvis/social-media-and-platforms.md")
    print_info("")
...
async def _standalone_send(
    ...
    *,
    ...
    Used by ``src/tools/send_message_tool._send_via_adapter`` when the gateway
