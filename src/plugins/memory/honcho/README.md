...
```bash
jarvis honcho setup    # full interactive wizard (cloud or local)
jarvis memory setup    # generic picker, also works
```
...
Host key is derived from the active Jarvis profile: `jarvis` (default) or `jarvis.<profile>`.
...
If `sessionPeerPrefix` is `true`, the peer name is prepended: `eri-jarvis-agent`.
...
Host key is derived from the active Jarvis profile: `jarvis` (default) or `jarvis.<profile>` (e.g. `jarvis -p coder` → host key `jarvis.coder`).
...
| Command | Description |
|---------|-------------|
| `jarvis honcho setup` | Full interactive setup wizard |
