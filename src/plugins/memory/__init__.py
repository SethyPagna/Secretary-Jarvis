...
1. Bundled providers: ``src/plugins/memory/<name>/`` (shipped with jarvis-agent)
2. User-installed providers: ``$JARVIS_HOME/src/plugins/<name>/``
...
def _get_user_plugins_dir() -> Optional[Path]:
    """Return ``$JARVIS_HOME/src/plugins/`` or None if unavailable."""
    try:
...
def _iter_provider_dirs() -> List[Tuple[str, Path]]:
    ...
    # 1. Bundled providers (src/plugins/memory/<name>/)
...
def load_memory_provider(name: str) -> Optional["MemoryProvider"]:
    ...
    Checks both bundled (``src/plugins/memory/<name>/``) and user-installed
    (``$JARVIS_HOME/src/plugins/<name>/``) directories.  Bundled takes
...
def _load_provider_from_dir(provider_dir: Path) -> Optional["MemoryProvider"]:
    ...
    module_name = f"plugins.memory.{name}" if _is_bundled else f"_jarvis_user_memory.{name}"
...
def discover_plugin_cli_commands() -> List[dict]:
    ...
    module_name = f"plugins.memory.{active_provider}.cli" if _is_bundled else f"_jarvis_user_memory.{active_provider}.cli"
    try:
