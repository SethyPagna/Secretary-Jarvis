...
Mounted at /api/src/plugins/kanban/ by the dashboard plugin system.
...
``/api/src/plugins/...`` request must present the session bearer token (or the
...
the in-browser PTY bridge in ``src/jarvis_cli/web_server.py``.
...
def specify_task_endpoint(
    ...
    try:
        os.environ["JARVIS_KANBAN_BOARD"] = board or kanban_db.DEFAULT_BOARD
...
async def stream_events(ws: WebSocket):
    ...
    # authenticates in src/jarvis_cli/web_server.py.
