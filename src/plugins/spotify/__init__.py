...
Why a plugin instead of a top-level ``src/tools/`` file?

- ``src/plugins/`` is where third-party service integrations live (see
  ``src/plugins/image_gen/`` for the backend-provider pattern, ``src/plugins/
  disk-cleanup/`` for the standalone pattern). ``src/tools/`` is reserved
  for foundational capabilities (terminal, read_file, web_search, etc.).
- Mirroring the image_gen plugin layout (``src/plugins/<category>/<backend>/``
  for categories, flat ``src/plugins/<name>/`` for standalones) makes new
