...
credentials. Managed JARVIS Managed gateway support has been removed — the JARVIS Managed
subscription now routes through Browser Use instead (see
``src/plugins/browser/browser_use/``).
...
class BrowserbaseBrowserProvider(BrowserProvider):
    ...
    Direct credentials only — managed-JARVIS Managed-gateway support lives on the
