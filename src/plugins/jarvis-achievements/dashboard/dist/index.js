(function () {
  ...
  // Originally authored by @PCinkusz (MIT).
  // Bundled into jarvis-agent. This local copy is the staging ground for new
  ...
  async function api(path, options) {
    const url = "/api/src/plugins/jarvis-achievements" + path;
    const token = window.__JARVIS_SESSION_TOKEN__ || "";
    const headers = { ...((options && options.headers) || {}) };
    if (token) headers["X-Jarvis-Session-Token"] = token;
  ...
  async function buildShareImage(achievement) {
    ...
    ctx.fillText("JARVIS  ·  github.com/SethyPagna/Secretary-Jarvis", 70, H - 40);
  ...
  function ShareDialog({ achievement, onClose }) {
    ...
    function tweetText() {
      ...
      return tmpl + "\n\n@SethyPagna · https://github.com/SethyPagna/Secretary-Jarvis";
    }
