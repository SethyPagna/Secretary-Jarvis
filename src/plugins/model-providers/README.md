...
directory layout mirrors `src/plugins/platforms/`:
...
`src/providers/__init__.py._discover_providers()` scans this directory (and
`$JARVIS_HOME/src/plugins/model-providers/`) the first time anything calls
...
User plugins at `$JARVIS_HOME/src/plugins/model-providers/<name>/` override
bundled plugins of the same name — last-writer-wins in
`register_provider()`. Drop a file there to replace a built-in.

## Adding a new provider

1. Create `src/plugins/model-providers/<your_provider>/__init__.py`:
...
2. Create `src/plugins/model-providers/<your_provider>/plugin.yaml`:
...
quirks — see `src/plugins/model-providers/openrouter/__init__.py` for
`build_extra_body` and `build_api_kwargs_extras` examples, and
`src/plugins/model-providers/gemini/__init__.py` for `thinking_config`
translation.
