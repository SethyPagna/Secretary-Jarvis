"""Assorted AIAgent runtime helpers — moved out of src/agent/runtime.py for clarity.
...
from jarvis_cli.utils import base_url_host_matches, base_url_hostname, env_var_enabled, atomic_json_write
...
def strip_think_blocks(agent, content: str) -> str:
    ...
    Handles four cases:
      ...
      2. Unterminated open tag at a block boundary (start of text or
         ...
         ``src/gateway/stream_consumer.py``'s filter so models that mention
...
def restore_primary_runtime(agent) -> bool:
    ...
    ``src/gateway/run.py``), so this restoration IS needed there too.
    """
