...
Providers live in ``<repo>/src/plugins/web/<name>/`` (built-in, auto-loaded as
``kind: backend``) or ``~/.jarvis/src/plugins/web/<name>/`` (user, opt-in via
``plugins.enabled``).
...
in ``src/tools/web_tools.py``; the response-shape contract documented below
...
class WebSearchProvider(abc.ABC):
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        ...
        Used by ``src/jarvis_cli/tools_config.py`` to inject this provider as a
