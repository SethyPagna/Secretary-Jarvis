...
# See tests/src/agent/test_auxiliary_client.py for patch patterns this supports.
if TYPE_CHECKING:
...
from jarvis_cli.utils import base_url_host_matches, base_url_hostname, normalize_proxy_env_vars
...
_OR_HEADERS_BASE = {
    "HTTP-Referer": "https://github.com/SethyPagna/Secretary-Jarvis",
    "X-Title": "JARVIS",
...
_NVIDIA_NIM_CLOUD_HEADERS = {
    "X-BILLING-INVOKE-ORIGIN": "JarvisAgent",
}
...
# see src/agent/portal_tags.py for the rationale.
...
class _CodexCompletionsAdapter:
    ...
    def create(self, **kwargs) -> Any:
        ...
        # src/agent/transports/codex.py::build_kwargs() so auxiliary callers
        ...
        if isinstance(extra_body, dict):
            ...
            if isinstance(reasoning_cfg, dict):
                if reasoning_cfg.get("enabled") is False:
                    ...
                    # Truthy-only check mirrors src/agent/transports/codex.py
...
def _resolve_custom_runtime() -> Tuple[Optional[str], Optional[str], Optional[str]]:
    ...
    # local servers ignore the Authorization header.  Same fix as src/jarvis_cli/terminal.py
    ...
    if provider == "xai-oauth":
        ...
        if client is None:
            logger.warning(
                ...
                "OAuth token found (run: jarvis model -> xAI Grok OAuth — SuperGrok Subscription)"
            )
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
...
def _build_call_kwargs(
    ...
    if tools:
        ...
        # injection paths (src/agent/runtime.py) already dedup, but this guard
