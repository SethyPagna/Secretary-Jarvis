...
This is the biggest single chunk pulled out of ``src/agent/runtime.py``: the
...
def run_conversation(
    ...
    # are reused.  The system prompt is Jarvis's territory; plugins
    ...
    # See src/agent/transports/codex_app_server_session.py for the adapter
                ...
                if hasattr(response, 'usage') and response.usage:
                    ...
                    if agent._session_db and agent.session_id:
                        try:
                            ...
                            # Under concurrent load (src/cron/kanban), the initial
                    ...
                    if classified.is_auth or classified.reason == FailoverReason.billing:
                        if _provider in {"openai-codex", "xai-oauth"} and status_code == 401:
                            if _provider == "openai-codex":
                                ...
                                agent._vprint(f"{agent.log_prefix}      re-authenticate with xAI Grok OAuth (SuperGrok Subscription) from `jarvis model`.", force=True)
                        else:
