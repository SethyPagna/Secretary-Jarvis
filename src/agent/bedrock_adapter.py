...
def normalize_converse_response(response: Dict) -> SimpleNamespace:
    ...
    The agent loop in ``src/agent/runtime.py`` expects responses shaped like
...
# Used by src/agent/runtime.py to trigger context compression instead of retrying.
...
# context window sizes.  Used by src/agent/model_metadata.py when dynamic
