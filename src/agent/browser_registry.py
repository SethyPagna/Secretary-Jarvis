...
2. Legacy preference order — ``browser-use`` → ``browserbase`` — filtered by
   ...
   because it covers both the managed JARVIS Managed gateway and direct API key path;
...
def _resolve(configured: Optional[str]) -> Optional[BrowserProvider]:
    ...
    the *web* extract plugin (``src/plugins/web/firecrawl/``), so users who set
    ...
    plugins added under ``~/.jarvis/src/plugins/browser/<vendor>/`` are subject
