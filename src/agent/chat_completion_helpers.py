...
def build_api_kwargs(agent, api_messages: list) -> dict:
    ...
    # completely unknown provider not in src/providers/ registry.
    _ephemeral_out = getattr(agent, "_ephemeral_max_output_tokens", None)
    if _ephemeral_out is not None:
        agent._ephemeral_max_output_tokens = None
...
def build_assistant_message(agent, assistant_message, finish_reason: str) -> dict:
    ...
    if reasoning_text and agent.reasoning_callback:
        ...
        # CLI post-response display fallback (src/jarvis_cli/terminal.py _reasoning_shown_this_turn).
...
def try_activate_fallback(agent, reason: "FailoverReason | None" = None) -> bool:
    ...
    try:
        ...
        if not fb_api_key_hint:
            ...
            # _normalize_custom_provider_entry in src/jarvis_cli/config.py).
...
def interruptible_streaming_api_call(agent, api_kwargs: dict, *, on_first_delta=None):
    ...
    def _call():
        ...
        _max_stream_retries = int(os.getenv("JARVIS_STREAM_RETRIES", 2))
