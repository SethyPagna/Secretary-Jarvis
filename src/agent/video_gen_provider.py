...
Providers live in ``<repo>/src/plugins/video_gen/<name>/`` (built-in, auto-loaded
as ``kind: backend``) or ``~/.jarvis/src/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).

Mirrors the ``image_gen`` provider design (``src/agent/image_gen_provider.py``) so
