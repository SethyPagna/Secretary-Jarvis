...
Keeping it in ``src/agent/runtime.py`` bloats that file with code that's mostly
...
# capture our warnings.  (src/agent/runtime.py also does
...
def init_agent(
    ...
    from jarvis_cli.logging_config import setup_logging, setup_verbose_logging
    ...
    if agent.verbose_logging:
        ...
        # on the fact that jarvis_cli.logging_config.setup_logging() does not
        ...
        # handler level inside src/jarvis_cli/logging_config.py, not here.
        pass
    ...
    # In-memory todo list for task planning (one per src/agent/session)
    ...
    # 2. Check src/plugins/context_engine/<name>/ directory (repo-shipped)
    ...
    if _engine_name != "compressor":
        # Try loading from src/plugins/context_engine/<name>/
        try:
            from plugins.context_engine import load_context_engine
            _selected_engine = load_context_engine(_engine_name)
        except Exception as _ce_load_err:
            _ra().logger.debug("Context engine load from src/plugins/context_engine/: %s", _ce_load_err)
