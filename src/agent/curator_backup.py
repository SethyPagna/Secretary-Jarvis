...
``~/.jarvis/src/cron/jobs.json`` as ``cron-jobs.json`` when it exists. Cron
...
def _cron_jobs_file() -> Path:
    """Source path for the live cron jobs store (``~/.jarvis/src/cron/jobs.json``)."""
...
def _backup_cron_jobs_into(dest: Path) -> Dict[str, Any]:
    ...
    if not src.exists():
        info["reason"] = "no src/cron/jobs.json present"
        return info
    try:
        raw = src.read_text(encoding="utf-8")
    except OSError as e:
        logger.debug("Failed to read src/cron/jobs.json for backup: %s", e)
...
def snapshot_skills(reason: str = "manual") -> Optional[Path]:
    ...
    try:
        ...
        # Capture src/cron/jobs.json alongside the tarball. Never fails the
...
def _restore_cron_skill_links(snapshot_dir: Path) -> Dict[str, Any]:
    """Reconcile backed-up cron skill links into the live ``src/cron/jobs.json``.
