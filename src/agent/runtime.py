...
#   (b) `patch("agent.runtime.OpenAI", ...)` test patterns used by ~28 test files.
#
# NOTE: `fire` is ONLY used in the `__main__` block below (for running
# src/agent/runtime.py directly as a CLI) — it is NOT needed for library usage.
...
# OpenAI lazy proxy + safe stdio + proxy URL helpers — see src/agent/process_bootstrap.py.
# `OpenAI` is re-exported here so `patch("agent.runtime.OpenAI", ...)` in tests works.
...
# Agent internals extracted to src/agent/ package for modularity
...
from jarvis_cli.utils import atomic_json_write, base_url_host_matches, base_url_hostname, env_var_enabled, normalize_proxy_url
...
class AIAgent:
    ...
    def steer(self, text: str) -> bool:
        ...
        Thread-safe: callable from src/gateway/CLI/TUI threads. Multiple calls
    ...
    def _prepare_anthropic_messages_for_api(self, api_messages: list) -> list:
        ...
        # The Anthropic adapter (src/agent/anthropic_adapter.py:_convert_content_part_to_anthropic)
    ...
    def _supports_reasoning_extra_body(self) -> bool:
        ...
        Some src/providers/routes reject `reasoning` with 400s, so gate it to
...
def main(
    ...
    if list_tools:
        ...
        print("  python src/agent/runtime.py --enabled_toolsets=research --query='search for Python news'")
        print("  python src/agent/runtime.py --enabled_toolsets=development --query='debug this code'")
        print("  python src/agent/runtime.py --enabled_toolsets=safe --query='analyze without terminal'")
        print("  ")
        print("  # Combine multiple toolsets")
        print("  python src/agent/runtime.py --enabled_toolsets=web,vision --query='analyze website'")
        print("  ")
        print("  # Disable toolsets")
        print("  python src/agent/runtime.py --disabled_toolsets=terminal --query='no command execution'")
        print("  ")
        print("  # Run with trajectory saving enabled")
        print("  python src/agent/runtime.py --save_trajectories --query='your question here'")
        return
