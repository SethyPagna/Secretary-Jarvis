...
Providers live in ``<repo>/src/plugins/browser/<name>/`` (built-in, auto-loaded as
``kind: backend``) or ``~/.jarvis/src/plugins/browser/<name>/`` (user, opt-in via
``plugins.enabled``).
...
``src/tools/browser_providers/``; the lifecycle contract documented below is
...
class BrowserProvider(abc.ABC):
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        ...
        expose API key prompts, badges, managed-JARVIS Managed gating, and the
