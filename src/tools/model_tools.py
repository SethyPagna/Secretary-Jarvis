...
Thin orchestration layer over the tool registry. Each tool file in src/tools/
self-registers its schema, handler, and metadata via tools.registry.register().
This module triggers discovery (by importing all tool modules), then provides
the public API that src/agent/runtime.py, src/jarvis_cli/terminal.py, src/tools/batch_runner.py, and the RL
environments consume.

Public API (signatures preserved from the original 2,400-line version):
    get_tool_definitions(enabled_toolsets, disabled_toolsets, quiet_mode) -> list
    handle_function_call(function_name, function_args, task_id, user_task) -> str
    TOOL_TO_TOOLSET_MAP: dict          (for src/tools/batch_runner.py)
    TOOLSET_REQUIREMENTS: dict         (for src/jarvis_cli/terminal.py, doctor.py)
...
#   - src/gateway/run.py            -> start_gateway() uses run_in_executor
#   - src/jarvis_cli/terminal.py, src/jarvis_cli/*      -> inline on startup (no event loop)
#   - desktop/gateway startup   -> inline on startup (no event loop)
#   - src/acp_adapter/server.py     -> asyncio.to_thread on session init
...
# Tools whose execution is intercepted by the agent loop (src/agent/runtime.py)
