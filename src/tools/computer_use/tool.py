...
For captures / actions with `capture_after=True`:
  ...
  src/agent/runtime.py's tool-message builder inspects `_multimodal` and emits a
  ...
  `src/agent/anthropic_adapter.py`). Every provider that supports multi-part
...
def handle_computer_use(args: Dict[str, Any], **kwargs) -> Any:
    ...
    (image + summary) which src/agent/runtime.py wraps into the tool message.
    """
