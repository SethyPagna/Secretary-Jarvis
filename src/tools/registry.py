...
schema, handler, toolset membership, and availability check.  ``src/tools/model_tools.py``
queries the registry instead of maintaining its own parallel data structures.

Import chain (circular-import safe):
    src/tools/registry.py  (no imports from model_tools or tool files)
           ^
    src/tools/*.py  (import from tools.registry at module level)
           ^
    src/tools/model_tools.py  (imports tools.registry + all tool modules)
           ^
    src/agent/runtime.py, src/jarvis_cli/terminal.py, src/tools/batch_runner.py, etc.
"""
...
# check_fn TTL cache
#
# check_fn callables like src/tools/terminal_tool.check_terminal_requirements
...
class ToolRegistry:
        ...
        with self._lock:
            ...
            if existing and existing.toolset != toolset:
                ...
                if both_mcp:
                    ...
                    # Reject shadowing — prevent src/plugins/MCP from overwriting
    ...
    def deregister(self, name: str) -> None:
        ...
        when a server sends ``notifications/src/tools/list_changed``.
        """
        with self._lock:
    ...
    # ------------------------------------------------------------------
    # Query helpers  (replace redundant dicts in src/tools/model_tools.py)
    # ------------------------------------------------------------------
