...
- ``src/agent/video_gen_provider.py`` defines the :class:`VideoGenProvider` ABC.
- ``src/agent/video_gen_registry.py`` holds the active providers (populated by
  plugins at import time).
- Each provider lives under ``src/plugins/video_gen/<name>/``.
