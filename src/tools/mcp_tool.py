...
try:
    ...
    # Notification types for dynamic tool discovery (src/tools/list_changed)
    try:
        from mcp.types import (
            ServerNotification,
...
class MCPServerTask:
    ...
    # ----- Dynamic tool discovery (notifications/src/tools/list_changed) -----
                ...
                if _MCP_NOTIFICATION_TYPES and isinstance(message, ServerNotification):
                    match message.root:
                        case ToolListChangedNotification():
                            logger.info(
                                "MCP server '%s': received src/tools/list_changed notification",
                                self.name,
                            )
                            # Some servers (notably mongodb-mcp-server) emit
                            # src/tools/list_changed immediately after initialize,
    ...
    async def _refresh_tools(self):
        ...
        Called when the server sends ``notifications/src/tools/list_changed``.
