...
# Singularity helpers (scratch dir, SIF cache) now live in src/tools/environments/singularity.py
...
# Dangerous command detection + approval now consolidated in src/tools/approval.py
...
def _transform_sudo_command(command: str | None) -> tuple[str | None, str | None]:
    ...
    If SUDO_PASSWORD is not set and in interactive mode (JARVIS_INTERACTIVE=1):
    ...
    if not has_configured_password and not sudo_password and env_var_enabled("JARVIS_INTERACTIVE"):
...
# Environment classes now live in src/tools/environments/
