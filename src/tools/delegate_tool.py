...
# approval callback is stored in src/tools/terminal_tool.py's threading.local(),
...
#   true            → _subagent_auto_approve (opt-in YOLO for src/cron/batch)
# Both emit a logger.warning for audit; gateway sessions are unaffected
# because they resolve approvals via src/tools/approval.py's per-session queue,
...
def _build_child_agent(
    ...
    # Extract parent's API key so subagents inherit auth (e.g. JARVIS Managed).
    ...
    # Inheriting acp_command unconditionally causes src/agent/runtime.py to initialize
    ...
    if override_acp_command:
        ...
        # so src/agent/runtime.py initializes the CopilotACPClient.
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Available providers: openrouter, jarvis_managed, zai, kimi-coding, minimax."
        ) from exc
...
def _load_config() -> dict:
    ...
    Checks the runtime config (src/jarvis_cli/terminal.py CLI_CONFIG) first, then falls back
    to the persistent config (src/jarvis_cli/config.py load_config()) so that
        ...
        "properties": {
            ...
            "acp_command": {
                ...
                "description": (
                    ...
                    "See src/agent/copilot_acp_client.py for the implementation. "
