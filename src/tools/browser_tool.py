...
# (Browserbase / Browser Use / Firecrawl) out of ``src/tools/browser_providers/``
# and into ``src/plugins/browser/<vendor>/``. The dispatcher consults the
...
# plugins under ``src/plugins/browser/<vendor>/`` and self-register through
...
# plugins drop in under ``~/.jarvis/src/plugins/browser/<vendor>/``.
...
def _get_cloud_provider() -> Optional[CloudBrowserProvider]:
    ...
    browser plugins (``~/.jarvis/src/plugins/browser/<vendor>/``) participate
...
def _extract_relevant_content(
    ...
    # secrets to the extraction model before src/agent/runtime.py's general
