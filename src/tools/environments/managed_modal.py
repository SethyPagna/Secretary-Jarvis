...
class ManagedModalEnvironment(BaseModalExecutionEnvironment):
    ...
    def __init__(
        ...
        if gateway is None:
            raise ValueError("Managed Modal requires a configured tool gateway and JARVIS Managed user token")

        self._gateway_origin = gateway.gateway_origin.rstrip("/")
        self._jarvis_managed_user_token = gateway.jarvis_managed_user_token
    ...
    def _request(self, method: str, path: str, *,
        ...
        headers = {
            "Authorization": f"Bearer {self._jarvis_managed_user_token}",
