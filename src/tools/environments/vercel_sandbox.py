...
Uses the Vercel Python SDK to run commands in cloud sandboxes through Jarvis'
shared ``BaseEnvironment`` shell contract. When persistence is enabled, the
backend stores task-scoped snapshot metadata under ``JARVIS_HOME`` and restores
...
def _snapshot_store_path() -> Path:
    return get_jarvis_home() / _SNAPSHOT_STORE_NAME
...
class VercelSandboxEnvironment(BaseEnvironment):
    ...
    def _configure_attached_sandbox(self, *, requested_cwd: str) -> None:
        ...
        if self._remote_home == "/":
            container_base = "/.jarvis"
        else:
            container_base = f"{self._remote_home.rstrip('/')}/.jarvis"
    ...
    def _vercel_bulk_download(self, dest_tar_path: Path) -> None:
        remote_jarvis = (
            "/.jarvis"
            if self._remote_home == "/"
            else f"{self._remote_home.rstrip('/')}/.jarvis"
        )
        archive_member = remote_jarvis.lstrip("/")
