...
* :class:`_ManagedFalSyncClient` — wrapper that drives a JARVIS Managed-managed
...
existing test suites (``tests/src/tools/test_image_generation.py``,
``tests/src/tools/test_managed_media_gateways.py``) and for the
``src/plugins/image_gen/fal/`` plugin's ``_it`` indirection — moving the
