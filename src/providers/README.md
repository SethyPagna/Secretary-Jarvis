...
`src/plugins/model-providers/<name>/` (bundled in this repo) and
`$JARVIS_HOME/src/plugins/model-providers/<name>/` (per-user overrides). The
registry in `src/providers/__init__.py` lazily discovers them the first time any
consumer calls `get_provider_profile()` or `list_providers()`. See
`src/plugins/model-providers/README.md` for the plugin contract and examples.
...
- `src/jarvis_cli/auth.py` extends `PROVIDER_REGISTRY` with every api-key
  profile it sees (skipping `copilot`, `kimi-coding`, `kimi-coding-cn`,
  `zai`, `openrouter`, `custom` — those need bespoke token resolution).
- `src/jarvis_cli/models.py` extends `CANONICAL_PROVIDERS` and calls
  `profile.fetch_models()` inside `provider_model_ids()`.
- `src/jarvis_cli/doctor.py` adds a `/models` health check for each
  `auth_type="api_key"` profile.
- `src/jarvis_cli/config.py` injects every `env_var` into
  `OPTIONAL_ENV_VARS` so the setup wizard knows about it.
- `src/jarvis_cli/runtime_provider.py` reads `profile.api_mode` as a fallback
  when URL detection finds nothing.
- `src/agent/model_metadata.py` maps hostname → provider via
  `profile.get_hostname()`.
- `src/agent/auxiliary_client.py` reads `profile.default_aux_model` first
  before falling back to the legacy hardcoded dict.
- `src/agent/transports/chat_completions.py::_build_kwargs_from_profile()`
  invokes `profile.prepare_messages()`, `profile.build_extra_body()`,
  and `profile.build_api_kwargs_extras()` on every call.
- `src/agent/runtime.py` passes `provider_profile=<ProviderProfile>` so the
...
See `src/plugins/model-providers/README.md` — drop a new directory there (or
under `$JARVIS_HOME/src/plugins/model-providers/` for a private plugin).
...
Full reference in `src/providers/base.py` dataclass definition.
