...
1. Bundled plugins: ``src/plugins/model-providers/<name>/`` (shipped with jarvis-agent)
2. User plugins: ``$JARVIS_HOME/src/plugins/model-providers/<name>/``
...
For backward compatibility, ``src/providers/*.py`` files (other than ``base.py``
...
# Repo-root ``src/plugins/model-providers/`` — populated at discovery time.
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$JARVIS_HOME/src/plugins/model-providers/`` can override
...
def _user_plugins_dir() -> Path | None:
    """Return ``$JARVIS_HOME/src/plugins/model-providers/`` if it exists."""
    try:
...
def _discover_providers() -> None:
    ...
    Order:
      1. Bundled plugins at ``<repo>/src/plugins/model-providers/<name>/``
      2. User plugins at ``$JARVIS_HOME/src/plugins/model-providers/<name>/``
      3. Legacy per-file modules at ``src/providers/<name>.py`` (back-compat)
    ...
    # 2. User plugins — under $JARVIS_HOME/src/plugins/model-providers/<name>/.
    ...
    # 3. Legacy single-file profiles at src/providers/<name>.py. Kept for
    #    back-compat — if someone drops a ``src/providers/foo.py`` into an
