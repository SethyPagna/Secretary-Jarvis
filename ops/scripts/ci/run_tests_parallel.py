...
def _format_file(file: Path, repo_root: Path) -> str:
    ...
    ``/home/runner/work/jarvis-agent/jarvis-agent/tests/acp/test_auth.py``.
...
def _load_durations(repo_root: Path) -> dict[str, float]:
    ...
    ``tests/src/tools/test_code_execution.py``) to wall-clock seconds from
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Default: 600 (10 min), env: JARVIS_TEST_FILE_TIMEOUT."
        ),
    )
    parser.add_argument(
        "--slice",
