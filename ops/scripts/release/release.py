...
Usage:
    ...
    python ops/scripts/release/release.py

    # Preview with semver bump
    python ops/scripts/release/release.py --bump minor

    # Create the release
    python ops/scripts/release/release.py --bump minor --publish

    # First release (no previous tag)
    python ops/scripts/release/release.py --bump minor --publish --first-release

    # Override CalVer date (e.g. for a belated release)
    python ops/scripts/release/release.py --bump minor --publish --date 2026.3.15
"""
...
ACP_REGISTRY_MANIFEST = REPO_ROOT / "ops" / "config" / "acp_registry" / "agent.json"
...
AUTHOR_MAP = {
    ...
    "mind-dragon@jarvis_managed.research": "Mind-Dragon",
    ...
    "shannon@jarvis.local": "shannonsands",
    ...
    "jarvis@jarvis.local": "JARVISProject",
    ...
    "emozilla@jarvis.local": "emozilla",
    ...
    "clement@jarvis.local": "lemassykoi",  # PR #27042 (model-switch probe keyless providers)
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "jarvis@jarvis.local"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/JARVISProject/jarvis-agent",
...
def main():
    ...
    if args.publish:
        ...
        print(f"  Example: python ops/scripts/release/release.py --bump minor --publish")
        print(f"{'='*60}")
