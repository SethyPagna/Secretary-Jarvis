...
| JARVIS docs import pack | Done | This checkpoint adds `ops/docs/jarvis/` sections for skills, MCP, tools, memory, scheduling, platforms, security, context, architecture, and env vars |
...
| Packaged backend smoke gate | Done | `ops/scripts/build/build-desktop.ps1` builds the PyInstaller backend and `ops/scripts/checks/smoke-desktop-backend.ps1` verifies `/api/status`, `/api/shutdown`, and process cleanup |
...
- `python -m py_compile jarvis_constants.py jarvis_cli/__init__.py ops/scripts/maintenance/rebrand.py`
...
- `ops/scripts/build/build-desktop.ps1` is the Windows build entrypoint.
