...
- [x] 43.3 Add venv lifecycle to voice setup assistant - commits `5133b65`, `d1d8ed7`
- [x] 43.4 Start Brain with local venv Python and update attention previews - commit `c9ef9de`
- [x] 43.5 Verify tests/builds/setup doctor/runtime live-test and push - commit `21b6859`
...
- [x] 48.2 Add a separate Electron floating orb BrowserWindow - commit `7c81ee4`
- [x] 48.3 Add compact `shell=orb` renderer mode - commit `7c81ee4`
- [x] 48.4 Bridge orb quick actions to the main app via IPC/tray actions - commit `7c81ee4`
...
- [x] 58.2 Upgrade the orb mini-HUD with online/offline status, model, voice/TTS readiness, task count, and a System quick action - commit 5422227
- [x] 58.3 Keep the floating orb shell compact and pass-through-safe while proving quick actions call the Electron bridge - commit 5422227
- [x] 58.4 Verify HUD build/UI tests, restart app, run live test, commit, and push - verified HUD build, HUD UI tests 27 passed / 1 skipped, supervisor restart, runtime status 4 core services online / 0 degraded, self-test ready, live test ready (commit 5422227)

## Phase 59 - Voice Push-To-Talk Activation And Audio Event Bridge
- [x] 59.1 Add Phase 59 tracking to `PLAN.md` and `PROGRESS.md` - pending commit
- [ ] 59.2 Start a Gateway live voice session when Voice is opened from the main orb/sidebar
- [ ] 59.3 Make floating orb/tray `open-voice` call the same Gateway voice-session start path
- [ ] 59.4 Subscribe HUD to `audio` SSE events for voice state/capsule updates
