...
const dumpNotice = (snap: MemorySnapshot, dump: HeapDumpResult | null) =>
  `jarvis-tui: ${snap.level} memory (${formatBytes(snap.heapUsed)}) — auto heap dump → ${dump?.heapPath ?? '(failed)'}\n`
...
setupGracefulExit({
  ...
  onError: (scope, err) => {
    ...
    process.stderr.write(`jarvis-tui ${scope}: ${message.slice(0, 2000)}\n`)
  },
  onSignal: signal => {
    resetTerminalModes()
    process.stderr.write(`jarvis-tui: received ${signal}\n`)
  }
})
