...
_TUI_BUILD_INPUT_FILES = (
    ...
    "packages/jarvis-ink/package-lock.json",
...
def cmd_chat(args):
    ...
    use_tui = getattr(args, "tui", False) or os.environ.get("JARVIS_TUI") == "1"
...
# it to the model-selection handlers so plain `jarvis --tui` does not pay for
...
def cmd_dashboard(args):
    ...
    embedded_chat = args.tui or os.environ.get("JARVIS_DASHBOARD_TUI") == "1"
    start_server(
        host=args.host,
        port=args.port,
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    wants_tui = os.environ.get("JARVIS_TUI") == "1" or "--tui" in sys.argv[1:]
    if not wants_tui:
        return False
    ...
    if getattr(args, "command", None) not in {None, "chat"}:
        return False
    if not (getattr(args, "tui", False) or os.environ.get("JARVIS_TUI") == "1"):
        return False
...
""",
    ...
    dashboard_parser.add_argument(
        ...
        action="store_true",
        help=(
            "Expose the in-browser Chat tab (embedded `jarvis --tui` via PTY/WebSocket). "
            "Alternatively set JARVIS_DASHBOARD_TUI=1."
        ),
    )
    dashboard_parser.add_argument(
        "--skip-build",
