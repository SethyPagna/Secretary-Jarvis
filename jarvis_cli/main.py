...
# IMPORTANT: jarvis_bootstrap must be the very first import ó it sets up
...
def cmd_whatsapp(args):
    ...
    print("-" * 50)
    if wa_mode == "bot":
        print("üì± Open WhatsApp (or WhatsApp Business) on the")
        print("   phone with the BOT's number, then scan:")
    else:
        print("üì± Open WhatsApp on your phone, then scan:")
    print()
...
def _save_custom_provider(
    ...
    cfg["custom_providers"] = providers
    save_config(cfg)
    print(f'  üíæ Saved to custom providers as "{name}" (edit in config.yaml)')
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        try:
            ...
            if auto_stash_ref is not None:
                ...
                if not update_succeeded:
                    print(
                        f"  ?Ô∏è  Local changes preserved in stash (ref: {auto_stash_ref})"
                    )
        ...
        if needs_migration:
            ...
            if missing_env:
                print(
                    f"  ?Ô∏è  {len(missing_env)} new required setting(s) need configuration"
                )
            if missing_config:
                print(f"  ?Ô∏è  {len(missing_config)} new config option(s) available")
