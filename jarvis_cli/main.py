    ...
    try:
        ...
        def _curses_browse(stdscr):
            ...
            while True:
                ...
                # Header line
                if search_text:
                    header = f"  Browse sessions — filter: {search_text}▁ˆ"
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        try:
            ...
            if auto_stash_ref is not None:
                ...
                if not update_succeeded:
                    print(
                        f"  ℹï¸  Local changes preserved in stash (ref: {auto_stash_ref})"
                    )
        ...
        if needs_migration:
            ...
            if missing_env:
                print(
                    f"  ⚠ï¸  {len(missing_env)} new required setting(s) need configuration"
                )
            if missing_config:
                print(f"  ℹï¸  {len(missing_config)} new config option(s) available")
