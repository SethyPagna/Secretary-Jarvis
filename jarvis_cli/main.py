...
Usage:
    ...
    jarvis honcho sessions                 # List directory â†’ session name mappings
    ...
    jarvis honcho migrate                  # Step-by-step migration guide: OpenClaw native â†’ Jarvis + Honcho
...
# IMPORTANT: jarvis_bootstrap must be the very first import â€” it sets up
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive commands (jarvis tools, jarvis setup, jarvis model) use
...
# ---------------------------------------------------------------------------
# Profile override â€” MUST happen before any jarvis module import.
#
...
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set JARVIS_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
...
# Bridge security.redact_secrets from config.yaml â†’ JARVIS_REDACT_SECRETS env
...
# Initialize centralized file logging early â€” all `jarvis` subcommands
...
def _has_any_provider_configured() -> bool:
    ...
    # Only count these if Jarvis has been explicitly configured â€” Claude Code
...
def _pin_kanban_board_env() -> None:
    ...
    (``jarvis kanban â€¦``) resolve the board on different paths: the env-pin if
...
def cmd_whatsapp(args):
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            ...
            print()
            print("  Tip: Agent responses are prefixed with 'âš• JARVIS'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  jarvis gateway")
        ...
        print("âš  Pairing may not have completed. Run 'jarvis whatsapp' to try again.")
...
def cmd_postinstall(args):
    ...
    print("âš• Jarvis post-install bootstrap")
    print()
...
# `jarvis model` provider picker. It does NOT re-run credential setup â€” it
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    # Route through _prompt_api_key so users can replace a stale/broken key
    # in-flow (K/R/C) instead of having to edit ~/.jarvis/.env by hand.
    ...
    from jarvis_cli.models import ai_gateway_model_ids, get_pricing_for_provider
...
def _model_flow_xai_oauth(_config, current_model="", *, args=None):
    ...
    if status.get("logged_in"):
        print("  xAI Grok OAuth (SuperGrok Subscription) credentials: âœ“")
        print()
        print("    1. Use existing credentials")
        print("    2. Reauthenticate (new OAuth login)")
        print("    3. Cancel")
    ...
    if selected:
        ...
        print(f"Default model set to: {selected} (via xAI Grok OAuth â€” SuperGrok Subscription)")
    else:
        print("No change.")
...
# it to the model-selection handlers so startup stays light on mobile shells.
if not _is_termux_startup_environment():
    from jarvis_cli.models import _PROVIDER_MODELS
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``jarvis update`` is a high-attention surface â€” surface the
...
def _restore_stashed_changes(
    ...
    if stash_selector is None:
        print(
            "âš  Local changes were restored, but Jarvis couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "âš  Local changes were restored, but Jarvis couldn't drop the saved stash entry."
            )
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("â„¹ Your fork is not tracking the official Jarvis repository.")
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  âœ“ Added upstream: https://github.com/SethyPagna/Secretary-Jarvis.git"
                )
                has_upstream = True
            else:
...
def _format_concurrent_instances_message(
    ...
    lines = ["âœ— Another jarvis.exe is running:"]
...
def _update_node_dependencies() -> None:
    ...
    for label, path in paths:
        ...
        # streamed output to ``~/.jarvis/logs/update.log`` so nothing is lost.
        ...
        print(f"  âš  npm install failed in {label}")
...
def _ensure_fhs_path_guard() -> None:
    ...
    path_comment = (
        "# JARVIS â€” ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _cmd_update_pip(args):
    ...
    print("âœ“ Update complete! Restart jarvis to use the new version.")
    ...
    try:
        ...
        if needs_migration:
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  â„¹ API keys require manual entry: jarvis config migrate")
            else:
                print()
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            if _graceful_ok:
                                ...
                                # WILL respawn the unit â€” but only after
        ...
        try:
            ...
            if supports_systemd_services() and has_legacy_jarvis_units():
                print()
                print("âš  Legacy Jarvis gateway unit(s) detected:")
...
def cmd_profile(args):
    ...
    if action is None:
        # Bare `jarvis profile` â€” show current profile status
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} â†’ jarvis -p {p.name}")
                break
        print()
        return
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from jarvis_cli.models import CANONICAL_PROVIDERS as _cp
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``jarvis`` or only flags â†’ defaults to ``chat``.
        return False
...
def _prepare_agent_startup(args) -> None:
    ...
    try:
        # MCP tool discovery â€” no event loop running in CLI startup,
...
def main():
    ...
    setup_parser.add_argument(
        ...
        "compatibility â€” a bare 'jarvis setup' now does this.",
    )
    setup_parser.add_argument(
        "--quick",
        action="store_true",
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for Jarvis CLI",
    )
    ...
    cron_create.add_argument(
        "--profile",
        help="Jarvis profile name to run the job under. Use 'default' for the root profile. Named profiles must already exist. Omit to preserve the scheduler's existing profile.",
    )
    ...
    cron_edit.add_argument(
        "--profile",
        help="Jarvis profile name to run the job under. Use 'default' for the root profile. Pass empty string to clear.",
    )
...
""",
    ...
    # subcommand â€” ``jarvis --help``, ``jarvis version``, ``jarvis logs``,
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            if not existing:
                print(
                    f"\n  Nothing to reset â€” no memory files found in {display_jarvis_home()}/memories/\n"
                )
                return
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles â€” multiple isolated Jarvis instances",
    )
    ...
    # for `jarvis dashboard` cmdlines and SIGTERM them directly â€” the same
    ...
    logs_parser = subparsers.add_parser(
        ...
        epilog="""\
Examples:
    jarvis logs                    Show last 50 lines of agent.log
    jarvis logs -f                 Follow agent.log in real time
...
""",
    ...
    # e.g. ``jarvis -c Pokemon Agent Dev`` â†’ ``jarvis -c 'Pokemon Agent Dev'``
