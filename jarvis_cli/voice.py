...
# (``_register_voice_handler`` in jarvis_cli/terminal.py) logs a warning when that
...
def _play_beep(frequency: int, count: int = 1) -> None:
    """Audible cue matching jarvis_cli/terminal.py's record/stop beeps.

    880 Hz single-beep on start (jarvis_cli/terminal.py:_voice_start_recording line 7532),
    660 Hz double-beep on stop (jarvis_cli/terminal.py:_voice_stop_and_transcribe line 7585).
...
# This Event mirrors jarvis_cli/terminal.py:_voice_tts_done: cleared while speak_text is
...
def stop_and_transcribe() -> Optional[str]:
    ...
    # — matches jarvis_cli/terminal.py:_voice_stop_and_transcribe's result.get("transcript").
...
def start_continuous(
    ...
    # (jarvis_cli/terminal.py:7528 comment).
...
def _continuous_on_silence() -> None:
    ...
    # CLI parity (jarvis_cli/terminal.py:10619-10621): wait for any in-flight TTS to
...
def speak_text(text: str) -> None:
    ...
    Mirrors jarvis_cli/terminal.py:_voice_speak_response exactly — same markdown strip
