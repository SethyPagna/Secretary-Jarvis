...
class JarvisManagedFeatureState:
    key: str
    label: str
    included_by_default: bool
    available: bool
    active: bool
    managed_by_jarvis_managed: bool
...
@dataclass(frozen=True)
class JarvisManagedSubscriptionFeatures:
    subscribed: bool
    jarvis_managed_auth_present: bool
    provider_is_jarvis_managed: bool
    features: Dict[str, JarvisManagedFeatureState]

    @property
    def web(self) -> JarvisManagedFeatureState:
        return self.features["web"]

    @property
    def image_gen(self) -> JarvisManagedFeatureState:
        return self.features["image_gen"]

    @property
    def tts(self) -> JarvisManagedFeatureState:
        return self.features["tts"]

    @property
    def browser(self) -> JarvisManagedFeatureState:
        return self.features["browser"]

    @property
    def modal(self) -> JarvisManagedFeatureState:
        return self.features["modal"]

    def items(self) -> Iterable[JarvisManagedFeatureState]:
...
def get_jarvis_managed_subscription_features(
    config: Optional[Dict[str, object]] = None,
) -> JarvisManagedSubscriptionFeatures:
    if config is None:
        config = load_config() or {}
    config = dict(config)
    model_cfg = _model_config_dict(config)
    provider_is_jarvis_managed = str(model_cfg.get("provider") or "").strip().lower() == "jarvis_managed"
    ...
    managed_tools_flag = managed_jarvis_managed_tools_enabled()
    jarvis_managed_auth_present = bool(jarvis_managed_status.get("logged_in"))
    subscribed = provider_is_jarvis_managed or jarvis_managed_auth_present
    ...
    managed_web_available = managed_tools_flag and jarvis_managed_auth_present and is_managed_tool_gateway_ready("firecrawl")
    managed_image_available = managed_tools_flag and jarvis_managed_auth_present and is_managed_tool_gateway_ready("fal-queue")
    managed_tts_available = managed_tools_flag and jarvis_managed_auth_present and is_managed_tool_gateway_ready("openai-audio")
    managed_browser_available = managed_tools_flag and jarvis_managed_auth_present and is_managed_tool_gateway_ready("browser-use")
    managed_modal_available = managed_tools_flag and jarvis_managed_auth_present and is_managed_tool_gateway_ready("modal")
    ...
    features = {
        "web": JarvisManagedFeatureState(
            key="web",
            label="Web tools",
            included_by_default=True,
            available=web_available,
            active=web_active,
            managed_by_jarvis_managed=web_managed,
        ...
        "image_gen": JarvisManagedFeatureState(
            key="image_gen",
            label="Image generation",
            included_by_default=True,
            available=image_available,
            active=image_active,
            managed_by_jarvis_managed=image_managed,
        ...
        "tts": JarvisManagedFeatureState(
            key="tts",
            label="OpenAI TTS",
            included_by_default=True,
            available=tts_available,
            active=tts_active,
            managed_by_jarvis_managed=tts_managed,
        ...
        "browser": JarvisManagedFeatureState(
            key="browser",
            label="Browser automation",
            included_by_default=True,
            available=browser_available,
            active=browser_active,
            managed_by_jarvis_managed=browser_managed,
        ...
        "modal": JarvisManagedFeatureState(
            key="modal",
            label="Modal execution",
            included_by_default=False,
            available=modal_available,
            active=modal_active,
            managed_by_jarvis_managed=modal_managed,
    ...
    return JarvisManagedSubscriptionFeatures(
        subscribed=subscribed,
        jarvis_managed_auth_present=jarvis_managed_auth_present,
        provider_is_jarvis_managed=provider_is_jarvis_managed,
        features=features,
    )
...
def apply_jarvis_managed_managed_defaults(
    ...
    *,
    ...
    if not features.provider_is_jarvis_managed:
        return set()
...
def get_gateway_eligible_tools(
    ...
    # Quick provider check without the heavy get_jarvis_managed_subscription_features call
    model_cfg = config.get("model")
    if not isinstance(model_cfg, dict) or str(model_cfg.get("provider") or "").strip().lower() != "jarvis_managed":
        return [], [], []

    direct = _get_gateway_direct_credentials()

    # Check which tools the user has explicitly opted into the gateway for.
    # This is distinct from managed_by_jarvis_managed which fires implicitly when
