...
describe("feature plugin slot manifest", () => {
  ...
  it("separates ready, partial, missing, and optional plug-in slots", () => {
    const manifest = buildFeaturePluginSlotManifest({
      ...
      pathExists: (path) => path.includes("piper") || path.includes("yolo") || path.includes("Kokoro"),
      listFiles: (path) =>
        path.includes("piper")
          ...
          : path.includes("Kokoro")
            ? ["C:/jarvis/models/huggingface/snapshots/hexgrad__Kokoro-82M/config.json", "C:/jarvis/models/huggingface/snapshots/hexgrad__Kokoro-82M/model.safetensors"]
          : [],
    });

    expect(manifest.summary).toEqual({ ready: 2, partial: 1, missing: 0, optional: 1 });
    expect(manifest.slots.find((slot) => slot.id === "feature-kokoro-82m")?.validationHint).toContain("HF_TOKEN");
