...
def test_gate_off_returns_once_without_prompting():
    ...
    with patch(
        "jarvis_cli.terminal.load_cli_config",
...
def test_gate_on_choice_always_persists_and_returns_always():
    ...
    with patch(
        "jarvis_cli.terminal.load_cli_config",
        return_value={"approvals": {"destructive_slash_confirm": True}},
    ), patch("jarvis_cli.terminal.save_config_value", _fake_save):
        result = _bound(JarvisCLI._confirm_destructive_slash, self_)(
            "clear", "detail",
        )
...
def test_gate_default_true_when_config_missing():
    ...
    with patch("jarvis_cli.terminal.load_cli_config", side_effect=Exception("boom")):
        result = _bound(JarvisCLI._confirm_destructive_slash, self_)(
            "clear", "detail",
        )
