...
def cli_obj(_isolate):
    ...
    with patch("jarvis_cli.terminal.load_cli_config", return_value={
        "display": {"tool_progress": "new"},
        "terminal": {},
    }), patch("jarvis_cli.terminal.get_tool_definitions", return_value=[]), \
         patch("jarvis_cli.terminal.build_welcome_banner"):
...
class TestLowContextWarning:
    ...
    def test_no_warning_for_normal_context(self, cli_obj):
        ...
        with patch("jarvis_cli.terminal.get_tool_definitions", return_value=[]), \
             patch("jarvis_cli.terminal.build_welcome_banner"):
            cli_obj.show_banner()
    ...
    def test_compact_banner_does_not_crash_on_narrow_terminal(self, cli_obj):
        ...
        with patch("shutil.get_terminal_size", return_value=os.terminal_size((70, 40))), \
             patch("jarvis_cli.terminal._build_compact_banner", return_value="compact banner"):
            cli_obj.show_banner()
