...
class TestCLIQuickCommands:
    ...
    def test_exec_command_uses_chat_console_when_tui_is_live(self):
        ...
        with patch("jarvis_cli.terminal.ChatConsole", return_value=live_console):
            result = cli.process_command("/dn")
    ...
    def test_quick_command_takes_priority_over_skill_commands(self):
        ...
        with patch("jarvis_cli.terminal._skill_commands", {"/mygif": {"name": "gif-search"}}):
    ...
    def test_unknown_command_still_shows_error(self):
        cli = self._make_cli({})
        with patch("jarvis_cli.terminal._cprint") as mock_cprint:
