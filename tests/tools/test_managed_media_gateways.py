...
def _enable_managed_jarvis_managed_tools(monkeypatch):
    """Patch the source modules so managed_jarvis_managed_tools_enabled() returns True
...
def test_managed_fal_submit_uses_gateway_origin_and_jarvis_managed_token(monkeypatch):
    captured = {}
    _install_fake_tools_package()
    _install_fake_fal_client(captured)
    monkeypatch.delenv("FAL_KEY", raising=False)
    monkeypatch.setenv("FAL_QUEUE_GATEWAY_URL", "http://127.0.0.1:3009")
    monkeypatch.setenv("TOOL_GATEWAY_USER_TOKEN", "jarvis_managed-token")
    ...
    assert captured["client_key"] == "jarvis_managed-token"
...
def test_openai_tts_uses_managed_audio_gateway_when_direct_key_absent(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("TOOL_GATEWAY_DOMAIN", "jarvis.local")
    monkeypatch.setenv("TOOL_GATEWAY_USER_TOKEN", "jarvis_managed-token")
    ...
    assert captured["api_key"] == "jarvis_managed-token"
    assert captured["base_url"] == "https://openai-audio-gateway.jarvis.local/v1"
...
def test_transcription_uses_model_specific_response_formats(monkeypatch, tmp_path):
    ...
    assert whisper_capture["base_url"] == "https://openai-audio-gateway.jarvis.local/v1"
