...
class TestVprintForceParameter:
    ...
    def test_error_messages_use_force_in_run_agent(self):
        """Verify that critical error _vprint calls in src/agent/runtime.py
        include force=True."""
...
class TestCtrlCResetsContinuousMode:
    ...
    def test_ctrl_c_handler_resets_voice_continuous(self):
        ...
        with open("src/jarvis_cli/terminal.py") as f:
            source = f.read()
...
class TestHandleVoiceCommandReal:
    ...
    @patch("jarvis_cli.terminal._cprint")
...
class TestVoiceBeepConfigReal:
    ...
    @patch("jarvis_cli.terminal._cprint")
    @patch("jarvis_cli.terminal.threading.Thread")
...
class TestVoiceSpeakResponseReal:
    ...
    def test_async_scheduling_clears_done_before_thread_start(self):
        ...
        with patch("jarvis_cli.terminal.threading.Thread", FakeThread):
    ...
    @patch("jarvis_cli.terminal._cprint")
    @patch("jarvis_cli.terminal.os.unlink")
    @patch("jarvis_cli.terminal.os.path.getsize", return_value=1000)
    @patch("jarvis_cli.terminal.os.path.isfile", return_value=True)
    @patch("jarvis_cli.terminal.os.makedirs")
