...
class TestFirecrawlClientConfig:
    ...
    def setup_method(self):
        ...
        self._managed_patchers = [
            patch("tools.web_tools.managed_jarvis_managed_tools_enabled", return_value=True),
            patch("tools.managed_tool_gateway.managed_jarvis_managed_tools_enabled", return_value=True),
        ]
    ...
    def test_no_config_raises_with_helpful_message(self):
        ...
        with patch("tools.web_tools.Firecrawl"):
            with patch("tools.web_tools._read_jarvis_managed_access_token", return_value=None):
    ...
    def test_tool_gateway_domain_builds_firecrawl_gateway_origin(self):
        ...
        with patch.dict(os.environ, {"TOOL_GATEWAY_DOMAIN": "jarvis.local"}):
            with patch("tools.web_tools._read_jarvis_managed_access_token", return_value="jarvis_managed-token"):
                with patch("tools.web_tools.Firecrawl") as mock_fc:
                    from tools.web_tools import _get_firecrawl_client
                    result = _get_firecrawl_client()
                    mock_fc.assert_called_once_with(
                        api_key="jarvis_managed-token",
                        api_url="https://firecrawl-gateway.jarvis.local",
                    )
        ...
        with patch.dict(os.environ, {
            ...
            with patch("tools.web_tools._read_jarvis_managed_access_token", return_value="jarvis_managed-token"):
                with patch("tools.web_tools.Firecrawl") as mock_fc:
                    ...
                    mock_fc.assert_called_once_with(
                        api_key="jarvis_managed-token",
                        api_url="http://firecrawl-gateway.jarvis.local",
                    )
                    assert result is mock_fc.return_value
    ...
    def test_default_gateway_domain_targets_jarvis_managed_production_origin(self):
    ...
    def test_jarvis_managed_auth_token_respects_jarvis_home_override(self, tmp_path):
        ...
        (jarvis_home / "auth.json").write_text(json.dumps({
            "providers": {
                "jarvis_managed": {
                    "access_token": "jarvis_managed-token",
                }
            }
        }))
        ...
        with patch.dict(os.environ, {
            ...
            assert tools.web_tools._read_jarvis_managed_access_token() == "jarvis_managed-token"
