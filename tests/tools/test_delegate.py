...
class TestDelegateTask(unittest.TestCase):
    ...
    def test_depth_increments(self):
        ...
        with patch("agent.runtime.AIAgent") as MockAgent:
...
class TestToolNamePreservation(unittest.TestCase):
    ...
    def test_build_child_agent_does_not_raise_name_error(self):
        ...
        with patch("agent.runtime.AIAgent"):
            try:
                _build_child_agent(
                    task_index=0,
...
class TestDelegationProviderIntegration(unittest.TestCase):
    ...
    def test_cross_provider_delegation(self, mock_creds, mock_cfg):
        """Parent on JARVIS Managed, subagent on OpenRouter — full credential switch."""
        mock_cfg.return_value = {
            "max_iterations": 45,
            "model": "google/gemini-3-flash-preview",
            "provider": "openrouter",
        ...
        parent = _make_mock_parent(depth=0)
        parent.provider = "jarvis_managed"
        parent.base_url = "https://inference-api.jarvis.local/v1"
        parent.api_key = "jarvis_managed-key-abc"
        ...
        with patch("agent.runtime.AIAgent") as MockAgent:
            ...
            _, kwargs = MockAgent.call_args
            # Child should use OpenRouter, NOT JARVIS Managed
            self.assertEqual(kwargs["provider"], "openrouter")
            self.assertEqual(kwargs["base_url"], "https://openrouter.ai/api/v1")
...
class TestOrchestratorRoleBehavior(unittest.TestCase):
    ...
    def test_batch_mode_per_task_role_override(self, mock_cfg, mock_creds):
        ...
        with patch("agent.runtime.AIAgent", side_effect=_factory):
            delegate_task(
                tasks=[
...
class TestOrchestratorEndToEnd(unittest.TestCase):
    ...
    def test_end_to_end_nested_orchestration(self, mock_cfg, mock_creds):
        ...
        with patch("agent.runtime.AIAgent", side_effect=_factory) as MockAgent:
            delegate_task(
