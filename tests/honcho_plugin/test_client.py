...
class TestResolveSessionName:
    ...
    def test_per_repo_uses_git_root(self):
        ...
        with patch.object(
            ...
            result = config.resolve_session_name("/home/user/jarvis-agent/subdir")
...
class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="jarvis.coder")
        assert config.host == "jarvis.coder"
        assert config.workspace_id == "jarvis"  # shared workspace
        assert config.ai_peer == "jarvis.coder"
    ...
    def test_from_global_config_reads_profile_host_block(self, tmp_path):
        ...
        config = HonchoClientConfig.from_global_config(
            host="jarvis.coder", config_path=config_file,
        )
        assert config.host == "jarvis.coder"
    ...
    def test_from_global_config_auto_resolves_host(self, tmp_path):
        ...
        with patch("plugins.memory.honcho.client.resolve_active_host", return_value="jarvis.dreamer"):
            config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.host == "jarvis.dreamer"
