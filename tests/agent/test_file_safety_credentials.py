...
Regression for https://github.com/JARVISProject/jarvis-agent/issues/17656 —
...
def test_identically_named_files_outside_jarvis_home_not_blocked(
    fake_home, tmp_path
):
    """A project's ``.env``, ``auth.json``, or ``mcp-tokens/`` outside
    JARVIS_HOME must remain readable — the gate is per-location, not
    per-filename."""
    from agent.file_safety import get_read_block_error
    ...
    for rel in (".env", "auth.json"):
        ...
        assert get_read_block_error(str(p)) is None, (
            f"{rel} outside JARVIS_HOME should NOT be blocked"
        )
