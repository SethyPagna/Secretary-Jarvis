...
# ── Sample headers from JARVIS Managed inference API ──────────────────────────────

JARVIS_MANAGED_HEADERS = {
...
class TestParseHeaders:
    def test_basic_parsing(self):
        state = parse_rate_limit_headers(JARVIS_MANAGED_HEADERS, provider="jarvis_managed")
        assert state is not None
        assert state.provider == "jarvis_managed"
...
class TestFormatting:
    ...
    def test_format_display_with_data(self):
        state = parse_rate_limit_headers(JARVIS_MANAGED_HEADERS, provider="jarvis_managed")
        result = format_rate_limit_display(state)
        assert "JARVIS Managed" in result
...
class TestAgentIntegration:
    ...
    def test_capture_rate_limits_from_headers(self):
        ...
        class MockResponse:
            headers = JARVIS_MANAGED_HEADERS

        # Import AIAgent minimally
        from unittest.mock import MagicMock, patch

        # Test the parsing directly
        state = parse_rate_limit_headers(MockResponse.headers, provider="jarvis_managed")
        assert state is not None
        assert state.requests_min.limit == 800
        assert state.tokens_hour.limit == 336000000
