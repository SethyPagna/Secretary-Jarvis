...
class TestAuxiliaryConfigBridge:
    ...
    def test_web_extract_bridged(self, monkeypatch):
        config = {
            "auxiliary": {
                "web_extract": {"provider": "jarvis_managed", "model": "gemini-2.5-flash"},
            }
        }
        _run_auxiliary_bridge(config, monkeypatch)
        assert os.environ.get("AUXILIARY_WEB_EXTRACT_PROVIDER") == "jarvis_managed"
    ...
    def test_all_tasks_with_overrides(self, monkeypatch):
        config = {
            "auxiliary": {
                ...
                "web_extract": {"provider": "jarvis_managed", "model": "gemini-3-flash"},
            }
        }
        _run_auxiliary_bridge(config, monkeypatch)
        assert os.environ.get("AUXILIARY_VISION_PROVIDER") == "openrouter"
