...
class TestRecordJarvisManagedRateLimit:
    """Test recording rate limit state."""

    def test_records_with_header_reset(self, rate_guard_env):
        from agent.jarvis_managed_rate_guard import record_jarvis_managed_rate_limit, _state_path

        headers = {"x-ratelimit-reset-requests-1h": "1800"}
        record_jarvis_managed_rate_limit(headers=headers)
    ...
    def test_falls_back_to_default_cooldown(self, rate_guard_env):
        ...
        record_jarvis_managed_rate_limit(headers=None)
    ...
    def test_custom_default_cooldown(self, rate_guard_env):
        ...
        record_jarvis_managed_rate_limit(headers=None, default_cooldown=120.0)
    ...
    def test_creates_directory_if_missing(self, rate_guard_env):
        ...
        record_jarvis_managed_rate_limit(headers={"retry-after": "10"})
        assert os.path.exists(_state_path())


class TestJarvisManagedRateLimitRemaining:
    ...
    def test_returns_none_when_no_file(self, rate_guard_env):
        ...
        assert jarvis_managed_rate_limit_remaining() is None

    def test_returns_remaining_seconds_when_active(self, rate_guard_env):
        from agent.jarvis_managed_rate_guard import record_jarvis_managed_rate_limit, jarvis_managed_rate_limit_remaining

        record_jarvis_managed_rate_limit(headers={"x-ratelimit-reset-requests-1h": "600"})
        remaining = jarvis_managed_rate_limit_remaining()
    ...
    def test_returns_none_when_expired(self, rate_guard_env):
        from agent.jarvis_managed_rate_guard import jarvis_managed_rate_limit_remaining, _state_path
...
class TestClearJarvisManagedRateLimit:
    ...
    def test_clears_existing_file(self, rate_guard_env):
        ...
        record_jarvis_managed_rate_limit(headers={"retry-after": "600"})
        assert jarvis_managed_rate_limit_remaining() is not None
...
class TestFormatRemaining:
    ...
    def test_seconds(self):
        from agent.jarvis_managed_rate_guard import format_remaining
...
class TestParseResetSeconds:
    ...
    def test_case_insensitive_headers(self, rate_guard_env):
        from agent.jarvis_managed_rate_guard import _parse_reset_seconds
...
class TestAuxiliaryClientIntegration:
    ...
    def test_try_jarvis_managed_skips_when_rate_limited(self, rate_guard_env, monkeypatch):
        from agent.jarvis_managed_rate_guard import record_jarvis_managed_rate_limit

        # Record a rate limit
        record_jarvis_managed_rate_limit(headers={"retry-after": "600"})

        # Mock _read_jarvis_managed_auth to return valid creds (would normally succeed)
        import agent.auxiliary_client as aux
        monkeypatch.setattr(aux, "_read_jarvis_managed_auth", lambda: {
            "access_token": "test-token",
            "inference_base_url": "https://api.jarvis_managed.test/v1",
        })

        result = aux._try_jarvis_managed()
        assert result == (None, None)

    def test_try_jarvis_managed_works_when_not_rate_limited(self, rate_guard_env, monkeypatch):
        import agent.auxiliary_client as aux

        # No rate limit recorded — _try_jarvis_managed should proceed normally
        # (will return None because no real creds, but won't be blocked
        # by the rate guard)
        monkeypatch.setattr(aux, "_read_jarvis_managed_auth", lambda: None)
        result = aux._try_jarvis_managed()
        assert result == (None, None)


class TestIsGenuineJarvisManagedRateLimit:
    """Tell a real account-level 429 apart from an upstream-capacity 429.

    JARVIS Managed multiplexes upstreams (DeepSeek, Kimi, MiMo, Jarvis).
    ...
    def test_exhausted_hourly_bucket_in_429_headers_is_genuine(self):
        from agent.jarvis_managed_rate_guard import is_genuine_jarvis_managed_rate_limit
        ...
        assert is_genuine_jarvis_managed_rate_limit(headers=headers) is True
    ...
    def test_healthy_headers_on_429_are_upstream_capacity(self):
        # Classic upstream-capacity symptom: JARVIS Managed edge reports plenty of
        ...
        assert is_genuine_jarvis_managed_rate_limit(headers=headers) is False

    def test_bare_429_with_no_headers_is_upstream(self):
        from agent.jarvis_managed_rate_guard import is_genuine_jarvis_managed_rate_limit

        assert is_genuine_jarvis_managed_rate_limit(headers=None) is False
        assert is_genuine_jarvis_managed_rate_limit(headers={}) is False
        assert is_genuine_jarvis_managed_rate_limit(
    ...
    def test_last_known_state_with_exhausted_bucket_triggers_genuine(self):
        ...
        last_state = parse_rate_limit_headers(prior_headers, provider="jarvis_managed")
        assert is_genuine_jarvis_managed_rate_limit(
