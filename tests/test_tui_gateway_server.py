...
def test_config_set_model_syncs_inference_provider_env(monkeypatch):
    """After an explicit provider switch, JARVIS_INFERENCE_PROVIDER must
    ...
    assert os.environ["JARVIS_INFERENCE_PROVIDER"] == "anthropic"


def test_config_set_model_syncs_tui_provider_unconditionally(monkeypatch):
    """Regression for #16857: /model must set JARVIS_TUI_PROVIDER even when
    ...
    # Both env vars must reflect the user's choice. JARVIS_TUI_PROVIDER is
    # the canonical explicit-this-process carrier consumed by
    # _resolve_startup_runtime() on /new.
    assert os.environ["JARVIS_TUI_PROVIDER"] == "custom:xuanji"
    assert os.environ["JARVIS_INFERENCE_PROVIDER"] == "custom:xuanji"
...
def test_config_set_model_syncs_tui_provider_env(monkeypatch):
    ...
    try:
        ...
        assert resp["result"]["value"] == "anthropic/claude-sonnet-4.6"
        assert os.environ["JARVIS_TUI_PROVIDER"] == "anthropic"
        assert os.environ["JARVIS_MODEL"] == "anthropic/claude-sonnet-4.6"
        assert os.environ["JARVIS_INFERENCE_MODEL"] == "anthropic/claude-sonnet-4.6"
    finally:
        server._sessions.clear()
