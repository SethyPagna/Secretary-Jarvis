...
def agent():
    ...
    with (
        patch(
            "agent.runtime.get_tool_definitions", return_value=_make_tool_defs("web_search")
        ),
...
def agent_with_memory_tool():
    ...
    with (
        patch(
            "agent.runtime.get_tool_definitions",
...
class TestInit:
    ...
    def test_model_max_tokens_from_config(self):
        ...
        with (
            patch("agent.runtime.get_tool_definitions", return_value=_make_tool_defs("terminal")),
            patch("agent.runtime.check_toolset_requirements", return_value={}),
            patch("agent.runtime.OpenAI"),
            patch(
                "jarvis_cli.config.load_config",
    ...
    def test_valid_tool_names_populated(self):
        ...
        with (
            patch("agent.runtime.get_tool_definitions", return_value=tools),
            patch("agent.runtime.check_toolset_requirements", return_value={}),
            patch("agent.runtime.OpenAI"),
        ):
            a = AIAgent(
...
class TestInterrupt:
    def test_interrupt_sets_flag(self, agent):
        with patch("agent.runtime._set_interrupt"):
            agent.interrupt()
...
class TestBuildSystemPrompt:
    ...
    def test_can_use_soul_identity_even_when_context_files_are_skipped(self):
        with (
            ...
            patch("agent.runtime.check_toolset_requirements", return_value={}),
            patch("agent.runtime.OpenAI"),
            patch("agent.runtime.load_soul_md", return_value="SOUL IDENTITY"),
        ):
            agent = AIAgent(
                api_key="test-k...7890",
                base_url="https://openrouter.ai/api/v1",
    ...
    def test_skills_prompt_derives_available_toolsets_from_loaded_tools(self):
        ...
        with (
            patch("agent.runtime.get_tool_definitions", return_value=tools),
            patch(
                "agent.runtime.check_toolset_requirements",
                side_effect=AssertionError("should not re-check toolset requirements"),
            ),
            patch("agent.runtime.get_toolset_for_tool", create=True, side_effect=toolset_map.get),
            patch("agent.runtime.build_skills_system_prompt", return_value="SKILLS_PROMPT") as mock_skills,
            patch("agent.runtime.OpenAI"),
        ):
            agent = AIAgent(
                api_key="test-k...7890",
...
class TestExecuteToolCalls:
    def test_single_tool_executed(self, agent):
        ...
        messages = []
        with patch(
            "agent.runtime.handle_function_call", return_value="search result"
        ) as mock_hfc:
    ...
    def test_invalid_json_args_defaults_empty(self, agent):
        ...
        mock_msg = _mock_assistant_msg(content="", tool_calls=[tc])
        messages = []
        with patch("agent.runtime.handle_function_call", return_value="ok") as mock_hfc:
            agent._execute_tool_calls(mock_msg, messages, "task-1")
    ...
    def test_result_truncation_over_100k(self, agent, tmp_path, monkeypatch):
        ...
        with patch("agent.runtime.handle_function_call", return_value=big_result):
            agent._execute_tool_calls(mock_msg, messages, "task-1")
    ...
    def test_quiet_tool_output_suppressed_when_progress_callback_present(self, agent):
        ...
        with patch("agent.runtime.handle_function_call", return_value="search result"), \
    ...
    def test_run_conversation_suppresses_retry_noise_in_parseable_quiet_mode(self, agent):
        ...
        with patch("agent.runtime.time.sleep", return_value=None):
...
class TestConcurrentToolExecution:
    ...
    def test_concurrent_executes_all_tools(self, agent):
        ...
        with patch("agent.runtime.handle_function_call", side_effect=fake_handle):
    ...
    def test_invoke_tool_dispatches_to_handle_function_call(self, agent):
        ...
        with patch("agent.runtime.handle_function_call", return_value="result") as mock_hfc:
    ...
    def test_sequential_tool_callbacks_fire_in_order(self, agent):
        ...
        with patch("agent.runtime.handle_function_call", return_value='{"success": true}'):
    ...
    def test_concurrent_tool_callbacks_fire_for_each_tool(self, agent):
        ...
        with patch("agent.runtime.handle_function_call", side_effect=['{"id":1}', '{"id":2}']):
            agent._execute_tool_calls_concurrent(mock_msg, messages, "task-1")
    ...
    def test_invoke_tool_blocked_skips_handle_function_call(self, agent, monkeypatch):
        ...
        with patch("agent.runtime.handle_function_call", side_effect=AssertionError("should not run")):
            result = agent._invoke_tool("web_search", {"q": "test"}, "task-1")
...
class TestPathsOverlap:
    ...
    def test_same_path_overlaps(self):
        from agent.runtime import _paths_overlap
...
class TestParallelScopePathNormalization:
    def test_extract_parallel_scope_path_normalizes_relative_to_cwd(self, tmp_path, monkeypatch):
        from agent.runtime import _extract_parallel_scope_path
    ...
    def test_extract_parallel_scope_path_treats_relative_and_absolute_same_file_as_same_scope(self, tmp_path, monkeypatch):
        from agent.runtime import _extract_parallel_scope_path, _paths_overlap
    ...
    def test_should_parallelize_tool_batch_rejects_same_file_with_mixed_path_spellings(self, tmp_path, monkeypatch):
        from agent.runtime import _should_parallelize_tool_batch
...
class TestRunConversation:
    ...
    def test_tool_calls_then_stop(self, agent):
        ...
        with (
            patch("agent.runtime.handle_function_call", return_value="search result") as mock_handle_function_call,
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
    ...
    def test_request_scoped_api_hooks_fire_for_each_api_call(self, agent):
        ...
        with (
            patch("agent.runtime.handle_function_call", return_value="search result"),
    ...
    def test_interrupt_breaks_loop(self, agent):
        ...
        with (
            ...
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
            patch("agent.runtime._set_interrupt"),
            patch.object(
    ...
    def test_context_compression_triggered(self, agent):
        ...
        with (
            patch("agent.runtime.handle_function_call", return_value="result"),
            patch.object(
    ...
    def test_length_with_tool_calls_returns_partial_without_executing_tools(self, agent):
        ...
        with (
            patch("agent.runtime.handle_function_call") as mock_handle_function_call,
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
    ...
    def test_truncated_tool_call_retries_once_before_refusing(self, agent):
        ...
        with (
            patch("agent.runtime.handle_function_call", return_value='{"success":true}') as mock_hfc,
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
    ...
    def test_kanban_block_called_on_iteration_exhaustion(self, agent, monkeypatch):
        ...
        with (
            patch("agent.runtime.handle_function_call", return_value="ok") as mock_hfc,
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
    ...
    def test_no_kanban_block_when_not_in_kanban_mode(self, agent, monkeypatch):
        """kanban_block must NOT be called when JARVIS_KANBAN_TASK is unset."""
        self._setup_agent(agent)
        agent.max_iterations = 2
...
class TestRetryExhaustion:
    ...
    def test_invalid_response_returns_error_not_crash(self, agent):
        ...
        with (
            ...
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
            patch("agent.runtime.time", self._make_fast_time_mock()),
...
class TestJarvisManagedCredentialRefresh:
    ...
    def test_try_refresh_jarvis_managed_client_credentials_rebuilds_client(
        ...
        with patch("agent.runtime.OpenAI", side_effect=_fake_openai):
...
class TestSafeWriter:
    ...
    def test_write_delegates_normally(self):
        ...
        from agent.runtime import _SafeWriter
...
def test_aiagent_uses_copilot_acp_client():
    with (
        patch("agent.runtime.get_tool_definitions", return_value=_make_tool_defs("web_search")),
        patch("agent.runtime.check_toolset_requirements", return_value={}),
        patch("agent.runtime.OpenAI") as mock_openai,
...
class TestMemoryNudgeCounterPersistence:
    ...
    def test_counters_initialized_in_init(self):
        ...
        with patch("agent.runtime.get_tool_definitions", return_value=[]):
            a = AIAgent(
