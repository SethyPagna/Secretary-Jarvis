...
PR #3480 capped context pressure percentage at 100% in src/agent/display.py
...
class TestMemoryToolPercentClamp:
    """src/tools/memory_tool.py — _success_response and _render_block pct"""
...
class TestCLIStatsPercentClamp:
    """jarvis_cli.terminal.py — /stats command percentage"""
...
class TestGatewayStatsPercentClamp:
    """src/gateway/run.py — _format_usage_stats percentage"""
...
class TestSourceLinesAreClamped:
    ...
    def test_gateway_run_clamped(self):
        ...
        assert "min(100, ctx.last_prompt_tokens" in src, (
            "src/gateway/run.py stats pct is not clamped with min(100, ...)"
        )

    def test_cli_clamped(self):
        src = self._read_file("src/jarvis_cli/terminal.py")
        assert "min(100, (last_prompt" in src, (
            "jarvis_cli.terminal.py /stats pct is not clamped with min(100, ...)"
        )
