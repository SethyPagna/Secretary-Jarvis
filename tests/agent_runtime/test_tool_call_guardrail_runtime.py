...
def _make_agent(*tool_names: str, max_iterations: int = 10, config: dict | None = None) -> AIAgent:
    with (
        patch("agent.runtime.get_tool_definitions", return_value=_make_tool_defs(*tool_names)),
...
def test_default_sequential_path_warns_repeated_exact_failure_without_blocking_execution():
    ...
    with patch("agent.runtime.handle_function_call", return_value=json.dumps({"error": "boom"})) as mock_hfc:
...
def test_config_enabled_hard_stop_blocks_repeated_exact_failure_before_execution():
    ...
    with patch("agent.runtime.handle_function_call", return_value="SHOULD_NOT_RUN") as mock_hfc:
        agent._execute_tool_calls_sequential(msg, messages, "task-1")
...
def test_sequential_after_call_appends_guidance_to_tool_result_without_extra_messages():
    ...
    with patch("agent.runtime.handle_function_call", return_value=json.dumps({"error": "boom"})):
        agent._execute_tool_calls_sequential(msg, messages, "task-1")
...
def test_same_tool_failure_warning_tells_model_to_recover_with_tools():
    ...
    with patch("agent.runtime.handle_function_call", return_value=json.dumps({"exit_code": 1})):
        agent._execute_tool_calls_sequential(msg, messages, "task-1")
...
def test_plugin_pre_tool_block_wins_without_counting_as_toolguard_block():
    ...
    with (
        ...
        patch("agent.runtime.handle_function_call", return_value="SHOULD_NOT_RUN") as mock_hfc,
    ):
        agent._execute_tool_calls_sequential(msg, messages, "task-1")
...
def test_default_run_conversation_warns_without_guardrail_halt():
    ...
    with (
        patch("agent.runtime.handle_function_call", return_value=json.dumps({"error": "boom"})) as mock_hfc,
