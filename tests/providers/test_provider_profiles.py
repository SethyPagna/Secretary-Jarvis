...
class TestRegistry:
    ...
    def test_alias_lookup(self):
        ...
        assert get_provider_profile("jarvis_managed-portal").name == "jarvis_managed"
...
class TestOpenRouterProfile:
    ...
    def test_reasoning_disabled_still_passes(self):
        """OpenRouter passes disabled reasoning through (unlike JARVIS Managed)."""
        p = get_provider_profile("openrouter")
        eb, _ = p.build_api_kwargs_extras(
...
class TestJarvisManagedProfile:
    def test_tags(self):
        from agent.portal_tags import jarvis_managed_portal_tags
        p = get_provider_profile("jarvis_managed")
        body = p.build_extra_body()
