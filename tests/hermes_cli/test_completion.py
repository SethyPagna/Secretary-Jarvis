...
class TestGenerateBash:
    def test_contains_completion_function_and_register(self):
        ...
        assert "complete -F _hermes_completion jarvis" in out
...
class TestGenerateZsh:
    ...
    def test_registers_compdef_instead_of_invoking_completion_function(self):
        out = generate_zsh(_make_parser())
        assert 'compdef _hermes jarvis' in out
        ...
        try:
            result = subprocess.run(
                [
                    "zsh",
                    "-fc",
                    f"autoload -Uz compinit && compinit -D; source {path}; [[ ${{_comps[jarvis]}} == _hermes ]]",
                ],
                capture_output=True,
                text=True,
            )
