...
class TestRenameProfile:
    ...
    def test_renames_root_honcho_host_without_changing_ai_peer(self, profile_env):
        ...
        assert cfg["hosts"]["jarvis.heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["jarvis.heimdall"]["peerName"] == "user-peer"
    ...
    def test_pins_ai_peer_when_absent_on_honcho_host_rename(self, profile_env):
        ...
        assert "jarvis.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["jarvis.heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["jarvis.heimdall"]["workspace"] == "jarvis"
    ...
    def test_does_not_overwrite_existing_honcho_host_on_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                ...
                "jarvis.heimdall": {"aiPeer": "heimdall"},
            }
        }))
        ...
        assert cfg["hosts"]["jarvis.heimdall"]["aiPeer"] == "heimdall"
...
class TestInternalHelpers:
    ...
    def test_default_hermes_home_docker(self, tmp_path, monkeypatch):
        """In Docker, _get_default_hermes_home() returns JARVIS_HOME itself."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("JARVIS_HOME", str(docker_home))
