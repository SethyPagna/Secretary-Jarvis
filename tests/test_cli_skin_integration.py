...
from jarvis_cli.terminal import JarvisCLI, _build_compact_banner, _rich_text_from_ansi
...
class TestCliSkinPromptIntegration:
    ...
    def test_handle_skin_command_refreshes_live_tui(self, capsys):
        ...
        with patch("jarvis_cli.terminal.save_config_value", return_value=True):
...
class TestCompactBannerSkinIntegration:
    def test_default_compact_banner_keeps_legacy_jarvis_managed_jarvis_branding(self):
        set_active_skin("default")

        with patch("jarvis_cli.terminal.shutil.get_terminal_size", return_value=SimpleNamespace(columns=90)), \
             patch.dict(_build_compact_banner.__globals__, {"format_banner_version_label": lambda: "JARVIS v0.1.0 (test)"}):
            banner = _build_compact_banner()

        assert "JARVIS" in banner

    def test_poseidon_compact_banner_uses_skin_branding_instead_of_jarvis_managed_jarvis(self):
