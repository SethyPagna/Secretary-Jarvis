...
def test_setup_same_provider_rotation_strategy_saved_for_multi_credential_pool(tmp_path, monkeypatch):
    ...
    import jarvis_cli.main as _main_mod
    import jarvis_cli.setup as _setup_mod
...
def test_setup_same_provider_fallback_can_add_another_credential(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("jarvis_cli.auth_commands.auth_add_command", fake_auth_add_command)
...
def test_setup_pool_step_shows_manual_vs_auto_detected_counts(tmp_path, monkeypatch, capsys):
    ...
    _stub_tts(monkeypatch)
    monkeypatch.setattr("jarvis_cli.setup.prompt_choice", fake_prompt_choice)
    monkeypatch.setattr("jarvis_cli.setup.prompt_yes_no", lambda *args, **kwargs: False)
    monkeypatch.setattr("jarvis_cli.setup.prompt", lambda *args, **kwargs: "")
    monkeypatch.setattr("agent.credential_pool.load_pool", lambda provider: _Pool())
    monkeypatch.setattr("agent.auxiliary_client.get_available_vision_backends", lambda: [])
...
def test_setup_summary_shows_camofox_when_browser_feature_is_camofox(tmp_path, monkeypatch, capsys):
    ...
    _clear_provider_env(monkeypatch)
    monkeypatch.setattr(
        "jarvis_cli.setup.get_jarvis_managed_subscription_features",
        lambda config: JarvisManagedSubscriptionFeatures(
            ...
            features={
                ...
                "browser": JarvisManagedFeatureState("browser", "Browser automation", True, True, True, False, True, True, "Camofox"),
                "modal": JarvisManagedFeatureState("modal", "Modal execution", False, False, False, False, False, True, "local"),
            },
        ),
    )
    monkeypatch.setattr("agent.auxiliary_client.get_available_vision_backends", lambda: [])
    ...
    monkeypatch.setattr(
        ...
        lambda config: JarvisManagedSubscriptionFeatures(
            ...
            features={
                ...
                "image_gen": JarvisManagedFeatureState("image_gen", "Image generation", True, False, False, False, False, True, ""),
                "tts": JarvisManagedFeatureState("tts", "OpenAI TTS", True, False, False, False, False, True, ""),
                "browser": JarvisManagedFeatureState("browser", "Browser automation", True, False, False, False, False, True, "Browserbase"),
                "modal": JarvisManagedFeatureState("modal", "Modal execution", False, False, False, False, False, True, "local"),
            },
        ),
    )
