...
def test_delegation_default_toolsets_removed_from_cli_config():
    ...
    Guards against accidental re-introduction in src/jarvis_cli/terminal.py's CLI_CONFIG default
    dict. If this test fails, someone re-added the key without wiring it up
    to _load_config() in src/tools/delegate_tool.py.
    ...
    the user's ~/.jarvis/config.yaml over the defaults (src/jarvis_cli/terminal.py:359-366).
    ...
    (src/jarvis_cli/terminal.py:76) — before any autouse fixture can fire. Source inspection
    ...
    assert '"default_toolsets"' not in source, (
        ...
        "Do not re-add it to src/jarvis_cli/terminal.py's CLI_CONFIG default dict; "
        "use src/tools/delegate_tool.py's DEFAULT_TOOLSETS module constant or "
