"""Regression tests for JARVIS Managed inference_base_url host-allowlist validation.
...
client. ``_validate_jarvis_managed_inference_url_from_network()`` blocks any URL
...
4. The env-var override path (``JARVIS_MANAGED_INFERENCE_BASE_URL``) is NOT
...
from jarvis_cli.auth import (
    DEFAULT_JARVIS_MANAGED_INFERENCE_URL,
    _ALLOWED_JARVIS_MANAGED_INFERENCE_HOSTS,
    _validate_jarvis_managed_inference_url_from_network,
)


class TestValidatorRules:
    def test_allowlisted_https_host_returned(self):
        url = "https://inference-api.jarvis.local/v1"
        assert _validate_jarvis_managed_inference_url_from_network(url) == url

    def test_trailing_slash_stripped(self):
        url = "https://inference-api.jarvis.local/v1/"
        assert _validate_jarvis_managed_inference_url_from_network(url) == url.rstrip("/")

    def test_attacker_host_rejected(self, caplog):
        with caplog.at_level(logging.WARNING, logger="jarvis_cli.auth"):
            assert (
                _validate_jarvis_managed_inference_url_from_network("https://attacker.com/v1")
                is None
            )
        assert any("attacker.com" in rec.message for rec in caplog.records)

    def test_subdomain_of_allowlist_host_rejected(self):
        """*.jarvis.local is NOT in the allowlist — exact hostname only.

        A subdomain takeover or DNS hijack of *.jarvis.local would
        otherwise pass — keep the gate tight.
        """
        assert (
            _validate_jarvis_managed_inference_url_from_network(
                "https://evil.inference-api.jarvis.local/v1"
            )
            is None
        )
    ...
    def test_http_scheme_rejected(self, caplog):
        with caplog.at_level(logging.WARNING, logger="jarvis_cli.auth"):
            assert (
                _validate_jarvis_managed_inference_url_from_network(
                    "http://inference-api.jarvis.local/v1"
                )
                is None
            )
    ...
    def test_file_scheme_rejected(self):
        assert (
            _validate_jarvis_managed_inference_url_from_network("file:///etc/passwd") is None
        )
    ...
    def test_empty_string_rejected(self):
        assert _validate_jarvis_managed_inference_url_from_network("") is None

    def test_whitespace_only_rejected(self):
        assert _validate_jarvis_managed_inference_url_from_network("   ") is None

    def test_none_rejected(self):
        assert _validate_jarvis_managed_inference_url_from_network(None) is None

    def test_non_string_rejected(self):
        assert _validate_jarvis_managed_inference_url_from_network(12345) is None  # type: ignore[arg-type]
        assert _validate_jarvis_managed_inference_url_from_network({"url": "x"}) is None  # type: ignore[arg-type]

    def test_malformed_url_rejected(self):
        """Even garbled input must fall back safely, not raise."""
        assert (
            _validate_jarvis_managed_inference_url_from_network("not://a real url at all")
            is None
        )

    def test_default_inference_url_is_in_allowlist(self):
        """Sanity check: DEFAULT_JARVIS_MANAGED_INFERENCE_URL must itself validate.

        If anyone retargets the default away from
        ``inference-api.jarvis.local``, they MUST update the allowlist
        in the same change — otherwise the allowlist would reject the
        Portal's own legitimate default and break every install.
        """
        assert (
            _validate_jarvis_managed_inference_url_from_network(DEFAULT_JARVIS_MANAGED_INFERENCE_URL)
            == DEFAULT_JARVIS_MANAGED_INFERENCE_URL.rstrip("/")
        )

    def test_allowlist_contains_inference_api_host(self):
        """The default's host must be in the allowlist set."""
        from urllib.parse import urlparse
        host = urlparse(DEFAULT_JARVIS_MANAGED_INFERENCE_URL).hostname
        assert host in _ALLOWED_JARVIS_MANAGED_INFERENCE_HOSTS
...
class TestCallSiteWiring:
    ...
    ``_validate_jarvis_managed_inference_url_from_network`` with the un-validated
    ``_optional_base_url`` again, the test catches it.

    Each site lives inside ``resolve_jarvis_managed_runtime_credentials`` and one
    ...
    def test_no_unvalidated_inference_base_url_assignments_remain(self):
        ...
        for needle in (
            ...
            assert needle not in source, (
                ...
                f"Use _validate_jarvis_managed_inference_url_from_network() instead."
            )
    ...
    def test_validator_wired_at_all_known_call_sites(self):
        ...
        refresh_count = source.count(
            '_validate_jarvis_managed_inference_url_from_network(refreshed.get("inference_base_url"))'
        )
        mint_count = source.count(
            '_validate_jarvis_managed_inference_url_from_network(mint_payload.get("inference_base_url"))'
        )
    ...
    def test_proxy_adapter_also_validates(self):
        """The JARVIS Managed proxy adapter applies the validator as defense-in-depth
        ...
        boundary."""
        from pathlib import Path
        import jarvis_cli.proxy.adapters.jarvis_managed_portal as _jarvis_managed_adapter
        source = Path(_jarvis_managed_adapter.__file__).read_text(encoding="utf-8")
        assert "_validate_jarvis_managed_inference_url_from_network" in source


class TestEnvOverrideNotGated:
    """The documented dev/staging env-var override must keep working.

    ``JARVIS_MANAGED_INFERENCE_BASE_URL`` is read by ``resolve_jarvis_managed_runtime_credentials``
    ...
    def test_env_override_path_does_not_call_validator(self):
        """In resolve_jarvis_managed_runtime_credentials, the env override is
        ...
        for line in source.splitlines():
            if "JARVIS_MANAGED_INFERENCE_BASE_URL" in line and "os.getenv" in line:
                assert "_validate_jarvis_managed_inference_url_from_network" not in line, (
