    ...
    def test_reads_new_list_format(self):
        ...
        cfg = {
            "fallback_providers": [
                ...
                {"provider": "jarvis_managed", "model": "Jarvis-4-Llama-3.1-405B"},
            ]
        }
    ...
    def test_skips_incomplete_entries(self):
        ...
        cfg = {
            "fallback_providers": [
                ...
                {"provider": "jarvis_managed", "model": "foo"},  # valid
                "not-a-dict",                          # noise
            ]
        }
        assert _read_chain(cfg) == [{"provider": "jarvis_managed", "model": "foo"}]

    def test_returns_copies_not_aliases(self):
        from jarvis_cli.fallback_cmd import _read_chain
        cfg = {"fallback_providers": [{"provider": "jarvis_managed", "model": "foo"}]}
        result = _read_chain(cfg)
        result[0]["provider"] = "mutated"
        assert cfg["fallback_providers"][0]["provider"] == "jarvis_managed"
...
class TestListCommand:
    ...
    def test_list_with_entries(self, isolated_home, capsys):
        _write_config(isolated_home, {
            ...
            "fallback_providers": [
                {"provider": "openrouter", "model": "anthropic/claude-sonnet-4.6"},
                {"provider": "jarvis_managed", "model": "Jarvis-4"},
            ],
        })
        from jarvis_cli.fallback_cmd import cmd_fallback_list
        cmd_fallback_list(types.SimpleNamespace())
...
class TestRemoveCommand:
    ...
    def test_remove_selected_entry(self, isolated_home, capsys):
        ...
        # Picker returns index 1 (the middle entry, "jarvis_managed / Jarvis-4")
