...
from jarvis_cli.models import (
    ...
    is_jarvis_managed_free_tier, partition_jarvis_managed_models_by_tier,
    check_jarvis_managed_free_tier, _FREE_TIER_CACHE_TTL,
...
class TestFetchOpenRouterModels:
    ...
    def test_permissive_when_supported_parameters_missing(self, monkeypatch):
        ...
        Some OpenRouter-compatible gateways (JARVIS Managed, private mirrors, older
...
class TestDetectProviderForModel:
    ...
    def test_aggregator_not_suggested(self):
        """jarvis_managed/openrouter should never be auto-suggested as target provider."""
        with patch("jarvis_cli.models.fetch_openrouter_models", return_value=LIVE_OPENROUTER_MODELS):
            result = detect_provider_for_model("claude-opus-4-6", "openai-codex")
        assert result is not None
        assert result[0] not in {"jarvis_managed",}  # jarvis_managed has claude models but shouldn't be suggested


class TestIsJarvisManagedFreeTier:
    """Tests for is_jarvis_managed_free_tier — account tier detection."""

    def test_paid_plus_tier(self):
        assert is_jarvis_managed_free_tier({"subscription": {"plan": "Plus", "tier": 2, "monthly_charge": 20}}) is False

    def test_free_tier_by_charge(self):
        assert is_jarvis_managed_free_tier({"subscription": {"plan": "Free", "tier": 0, "monthly_charge": 0}}) is True

    def test_no_charge_field_not_free(self):
        """Missing monthly_charge defaults to not-free (don't block users)."""
        assert is_jarvis_managed_free_tier({"subscription": {"plan": "Free", "tier": 0}}) is False

    def test_plan_name_alone_not_free(self):
        """Plan name alone is not enough — monthly_charge is required."""
        assert is_jarvis_managed_free_tier({"subscription": {"plan": "free"}}) is False

    def test_empty_subscription_not_free(self):
        """Empty subscription dict defaults to not-free (don't block users)."""
        assert is_jarvis_managed_free_tier({"subscription": {}}) is False

    def test_no_subscription_not_free(self):
        """Missing subscription key returns False."""
        assert is_jarvis_managed_free_tier({}) is False
...
class TestPartitionJarvisManagedModelsByTier:
    """Tests for partition_jarvis_managed_models_by_tier — free vs paid tier model split."""
    ...
    def test_paid_tier_all_selectable(self):
        ...
        sel, unav = partition_jarvis_managed_models_by_tier(models, pricing, free_tier=False)
        assert sel == models
        assert unav == []
    ...
    def test_free_tier_splits_correctly(self):
        ...
        sel, unav = partition_jarvis_managed_models_by_tier(models, pricing, free_tier=True)
    ...
    def test_no_pricing_returns_all(self):
        ...
        sel, unav = partition_jarvis_managed_models_by_tier(models, {}, free_tier=True)
        assert sel == models
        assert unav == []
...
class TestUnionWithPortalFreeRecommendations:
    ...
    def test_adds_portal_free_model_missing_from_curated(self):
        ...
        with patch(
            "jarvis_cli.models.fetch_jarvis_managed_recommended_models",
    ...
    def test_then_partition_keeps_portal_free_model(self):
        ...
        with patch(
            ...
            ids, p = union_with_portal_free_recommendations(curated, pricing, "")
        sel, unav = partition_jarvis_managed_models_by_tier(ids, p, free_tier=True)
    ...
    def test_empty_payload_returns_inputs_unchanged(self):
        ...
        with patch("jarvis_cli.models.fetch_jarvis_managed_recommended_models", return_value={}):
            ids, p = union_with_portal_free_recommendations(curated, pricing, "")
        assert ids == curated
        assert p == pricing
...
class TestUnionWithPortalPaidRecommendations:
    ...
    def test_does_not_synthesize_pricing_for_paid_models(self):
        ...
        :func:`partition_jarvis_managed_models_by_tier` into treating them as free;
...
class TestCheckJarvisManagedFreeTierCache:
    """Tests for the TTL cache on check_jarvis_managed_free_tier()."""
    ...
    @patch("jarvis_cli.models.fetch_jarvis_managed_account_tier")
    @patch("jarvis_cli.models.is_jarvis_managed_free_tier", return_value=True)
    def test_result_is_cached(self, mock_is_free, mock_fetch):
        ...
        with patch("jarvis_cli.auth.get_provider_auth_state", return_value={"access_token": "tok"}), \
             patch("jarvis_cli.auth.resolve_jarvis_managed_runtime_credentials"):
            result1 = check_jarvis_managed_free_tier()
            result2 = check_jarvis_managed_free_tier()
    ...
    @patch("jarvis_cli.models.fetch_jarvis_managed_account_tier")
    @patch("jarvis_cli.models.is_jarvis_managed_free_tier", return_value=False)
...
class TestJarvisManagedRecommendedModels:
    """Tests for fetch_jarvis_managed_recommended_models + get_jarvis_managed_recommended_aux_model."""
    ...
    def setup_method(self):
        _models_mod._jarvis_managed_recommended_cache.clear()
    ...
    def test_fetch_caches_per_portal_url(self):
        from jarvis_cli.models import fetch_jarvis_managed_recommended_models
        mock_cm = self._mock_urlopen(self._SAMPLE_PAYLOAD)
        with patch("urllib.request.urlopen", return_value=mock_cm) as mock_urlopen:
            a = fetch_jarvis_managed_recommended_models("https://portal.example.com")
            b = fetch_jarvis_managed_recommended_models("https://portal.example.com")
    ...
    def test_fetch_cache_is_keyed_per_portal(self):
        ...
        mock_cm = self._mock_urlopen(self._SAMPLE_PAYLOAD)
        with patch("urllib.request.urlopen", return_value=mock_cm) as mock_urlopen:
            fetch_jarvis_managed_recommended_models("https://portal.example.com")
            fetch_jarvis_managed_recommended_models("https://portal.staging-jarvis.local")
    ...
    def test_fetch_returns_empty_on_network_failure(self):
        ...
        with patch("urllib.request.urlopen", side_effect=OSError("boom")):
            result = fetch_jarvis_managed_recommended_models("https://portal.example.com")
        assert result == {}
    ...
    def test_fetch_force_refresh_bypasses_cache(self):
        ...
        with patch("urllib.request.urlopen", return_value=mock_cm) as mock_urlopen:
            fetch_jarvis_managed_recommended_models("https://portal.example.com")
            fetch_jarvis_managed_recommended_models("https://portal.example.com", force_refresh=True)
    ...
    def test_get_aux_model_returns_vision_recommendation(self):
        ...
        with patch(
            ...
            model = get_jarvis_managed_recommended_aux_model(vision=True, free_tier=True)
    ...
    def test_get_aux_model_returns_compaction_recommendation(self):
        ...
        with patch(
            ...
            model = get_jarvis_managed_recommended_aux_model(vision=False, free_tier=True)
    ...
    def test_get_aux_model_returns_none_on_empty_payload(self):
        from jarvis_cli.models import get_jarvis_managed_recommended_aux_model
        with patch("jarvis_cli.models.fetch_jarvis_managed_recommended_models", return_value={}):
            assert get_jarvis_managed_recommended_aux_model(vision=False, free_tier=True) is None
            assert get_jarvis_managed_recommended_aux_model(vision=True, free_tier=False) is None
    ...
    def test_paid_tier_prefers_paid_recommendation(self):
        ...
        with patch("jarvis_cli.models.fetch_jarvis_managed_recommended_models", return_value=payload):
            text = get_jarvis_managed_recommended_aux_model(vision=False, free_tier=False)
            vision = get_jarvis_managed_recommended_aux_model(vision=True, free_tier=False)
    ...
    def test_auto_detects_tier_when_not_supplied(self):
        """Default behaviour: call check_jarvis_managed_free_tier() to pick the tier."""
        from jarvis_cli.models import get_jarvis_managed_recommended_aux_model
        payload = {
        ...
        with (
            patch("jarvis_cli.models.fetch_jarvis_managed_recommended_models", return_value=payload),
            patch("jarvis_cli.models.check_jarvis_managed_free_tier", return_value=True),
        ):
            assert get_jarvis_managed_recommended_aux_model(vision=False) == "free-model"
        with (
            patch("jarvis_cli.models.fetch_jarvis_managed_recommended_models", return_value=payload),
            patch("jarvis_cli.models.check_jarvis_managed_free_tier", return_value=False),
        ):
            assert get_jarvis_managed_recommended_aux_model(vision=False) == "paid-model"
    ...
    def test_tier_detection_error_defaults_to_paid(self):
        ...
        with (
            patch("jarvis_cli.models.fetch_jarvis_managed_recommended_models", return_value=payload),
            patch("jarvis_cli.models.check_jarvis_managed_free_tier", side_effect=RuntimeError("boom")),
        ):
            assert get_jarvis_managed_recommended_aux_model(vision=False) == "paid-model"
