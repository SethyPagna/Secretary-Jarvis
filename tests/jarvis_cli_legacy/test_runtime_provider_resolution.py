...
def test_named_custom_provider_does_not_shadow_builtin_provider(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda **kwargs: {
            ...
            "api_key": "jarvis_managed-runtime-key",
            "source": "portal",
            "expires_at": None,
        },
    )

    resolved = rp.resolve_runtime_provider(requested="jarvis_managed")

    assert resolved["provider"] == "jarvis_managed"
    assert resolved["base_url"] == "https://inference-api.jarvis.local/v1"
    assert resolved["api_key"] == "jarvis_managed-runtime-key"
    assert resolved["requested_provider"] == "jarvis_managed"
...
def test_named_custom_provider_skipped_for_canonical_built_in(monkeypatch):
    """Companion to the test above: ``jarvis_managed`` is a canonical provider name
    (``resolve_provider('jarvis_managed') == 'jarvis_managed'``), so a custom entry with that name
    ...
    entry = rp._get_named_custom_provider("jarvis_managed")
...
def test_resolve_requested_provider_precedence(monkeypatch):
    ...
    monkeypatch.setattr(rp, "_get_model_config", lambda: {})
    assert rp.resolve_requested_provider() == "jarvis_managed"
...
def test_auto_detected_jarvis_managed_auth_failure_falls_through_to_openrouter(monkeypatch):
    """When auto-detect picks JARVIS Managed but credentials are revoked, fall through to OpenRouter."""
    from jarvis_cli.auth import AuthError
    ...
    # resolve_provider returns "jarvis_managed" (stale active_provider in auth.json)
    monkeypatch.setattr(rp, "resolve_provider", lambda *a, **k: "jarvis_managed")
    ...
    # JARVIS Managed credential resolution fails with revoked token
    monkeypatch.setattr(
        rp, "resolve_jarvis_managed_runtime_credentials",
        lambda **kw: (_ for _ in ()).throw(
            AuthError("Refresh session has been revoked",
                      provider="jarvis_managed", code="invalid_grant", relogin_required=True)
        ),
    )
...
def test_explicit_jarvis_managed_auth_failure_still_raises(monkeypatch):
    """When user explicitly requests JARVIS Managed and auth fails, the error should propagate."""
    from jarvis_cli.auth import AuthError
    import pytest
    ...
    # With explicit "jarvis_managed", should raise — don't silently switch providers
    with pytest.raises(AuthError, match="Refresh session has been revoked"):
        rp.resolve_runtime_provider(requested="jarvis_managed")
