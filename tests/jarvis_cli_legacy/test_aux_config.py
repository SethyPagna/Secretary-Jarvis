...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"provider": "jarvis_managed", "model": "gemini-3-flash"}, "jarvis_managed · gemini-3-flash"),
        (
...
def test_save_aux_choice_preserves_timeout(tmp_path, monkeypatch):
    ...
    _save_aux_choice("vision", provider="jarvis_managed", model="gemini-3-flash")
...
def test_save_aux_choice_creates_missing_task_entry(tmp_path, monkeypatch):
    ...
    _save_aux_choice("vision", provider="jarvis_managed", model="gemini-3-flash")
    cfg = load_config()
    assert cfg["auxiliary"]["vision"]["provider"] == "jarvis_managed"
...
def test_reset_aux_to_auto_clears_routing_preserves_timeouts(tmp_path, monkeypatch):
    ...
    _save_aux_choice("compression", provider="jarvis_managed", model="gemini-3-flash")
    from jarvis_cli.config import save_config
