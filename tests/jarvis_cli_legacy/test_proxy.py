...
def test_registry_lists_jarvis_managed():
    assert "jarvis_managed" in ADAPTERS
...
def test_get_adapter_returns_instance():
    adapter = get_adapter("jarvis_managed")
    assert isinstance(adapter, JarvisManagedPortalAdapter)
...
def test_get_adapter_case_insensitive():
    assert isinstance(get_adapter("jarvis_managed"), JarvisManagedPortalAdapter)
    assert isinstance(get_adapter("  JARVIS Managed  "), JarvisManagedPortalAdapter)
...
# ---------------------------------------------------------------------------
# JarvisManagedPortalAdapter
# ---------------------------------------------------------------------------


def _write_auth_store(jarvis_home: Path, jarvis_managed_state: Dict[str, Any]) -> Path:
    """Write an auth.json with the given jarvis_managed state into a hermetic JARVIS_HOME."""
    auth_path = jarvis_home / "auth.json"
    auth_path.write_text(json.dumps({
        "version": 1,
        "providers": {"jarvis_managed": jarvis_managed_state},
    }))
    return auth_path


def test_jarvis_managed_adapter_metadata():
    adapter = JarvisManagedPortalAdapter()
    assert adapter.name == "jarvis_managed"
    assert adapter.display_name == "JARVIS Managed"
...
def test_jarvis_managed_adapter_not_authenticated_when_no_auth_file(tmp_path, monkeypatch):
...
def test_jarvis_managed_adapter_not_authenticated_when_provider_missing(tmp_path, monkeypatch):
    monkeypatch.setenv("JARVIS_HOME", str(tmp_path))
    (tmp_path / "auth.json").write_text(json.dumps({
        "version": 1,
        "providers": {},
    }))
    assert not JarvisManagedPortalAdapter().is_authenticated()


def test_jarvis_managed_adapter_authenticated_with_agent_key(tmp_path, monkeypatch):
    monkeypatch.setenv("JARVIS_HOME", str(tmp_path))
    _write_auth_store(tmp_path, {
        "agent_key": "ov-test-key",
        "agent_key_expires_at": "2099-01-01T00:00:00Z",
        "inference_base_url": "https://inference-api.jarvis.local/v1",
    })
    assert JarvisManagedPortalAdapter().is_authenticated()


def test_jarvis_managed_adapter_authenticated_with_refresh_token_only(tmp_path, monkeypatch):
...
def test_jarvis_managed_adapter_get_credential_uses_runtime_resolver(tmp_path, monkeypatch):
    monkeypatch.setenv("JARVIS_HOME", str(tmp_path))
    _write_auth_store(tmp_path, {
        "access_token": "access-tok",
        "refresh_token": "refresh-tok",
    ...
    with patch(
        "jarvis_cli.proxy.adapters.jarvis_managed_portal.resolve_jarvis_managed_runtime_credentials",
    ...
    assert cred.base_url == "https://inference-api.jarvis.local/v1"
    assert cred.expires_at == "2099-01-01T00:00:00Z"
    assert cred.token_type == "Bearer"


def test_jarvis_managed_adapter_retry_credential_forces_legacy_mint(tmp_path, monkeypatch):
    monkeypatch.setenv("JARVIS_HOME", str(tmp_path))
    _write_auth_store(tmp_path, {
...
def test_jarvis_managed_adapter_retry_credential_skips_opaque_bearer(tmp_path, monkeypatch):
    monkeypatch.setenv("JARVIS_HOME", str(tmp_path))
    _write_auth_store(tmp_path, {
        "access_token": "jwt-access",
        "refresh_token": "refresh-tok",
...
def test_jarvis_managed_adapter_get_credential_raises_when_not_logged_in(tmp_path, monkeypatch):
    monkeypatch.setenv("JARVIS_HOME", str(tmp_path))
    adapter = JarvisManagedPortalAdapter()
    with pytest.raises(RuntimeError, match="jarvis login jarvis_managed"):
        adapter.get_credential()


def test_jarvis_managed_adapter_get_credential_raises_on_refresh_failure(tmp_path, monkeypatch):
    monkeypatch.setenv("JARVIS_HOME", str(tmp_path))
    _write_auth_store(tmp_path, {
        "access_token": "access-tok",
        "refresh_token": "refresh-tok",
...
def test_jarvis_managed_adapter_quarantines_terminal_refresh_failure(tmp_path, monkeypatch):
    ...
    jarvis_managed_state = stored["providers"]["jarvis_managed"]
    ...
    assert stored.get("credential_pool", {}).get("jarvis_managed") == []


def test_jarvis_managed_adapter_get_credential_raises_when_no_agent_key_returned(tmp_path, monkeypatch):
...
def test_jarvis_managed_adapter_concurrent_refresh_serialized(tmp_path, monkeypatch):
...
def test_cmd_proxy_status_runs(capsys, tmp_path, monkeypatch):
    ...
    assert "jarvis_managed" in out
    assert "JARVIS Managed" in out
...
def test_cmd_proxy_start_refuses_when_unauthenticated(capsys, tmp_path, monkeypatch):
    ...
    args = MagicMock()
    args.provider = "jarvis_managed"
    args.host = None
    args.port = None
    rc = cmd_proxy_start(args)
    assert rc == 2
    err = capsys.readouterr().err
    assert "jarvis login jarvis_managed" in err
