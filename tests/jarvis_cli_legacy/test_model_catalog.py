...
def _valid_manifest() -> dict:
    return {
        ...
        "providers": {
            ...
            "jarvis_managed": {
                "metadata": {"display_name": "JARVIS Managed"},
                "models": [
...
class TestCuratedAccessors:
    ...
    def test_jarvis_managed_returns_ids(self, isolated_home):
        from jarvis_cli import model_catalog
        with patch.object(
            model_catalog, "_fetch_manifest", return_value=_valid_manifest()
        ):
            result = model_catalog.get_curated_jarvis_managed_models()
    ...
    def test_jarvis_managed_returns_none_when_catalog_empty(self, isolated_home):
        from jarvis_cli import model_catalog
        with patch.object(model_catalog, "_fetch_manifest", return_value=None):
            assert model_catalog.get_curated_jarvis_managed_models() is None
...
class TestIntegrationWithModelsModule:
    ...
    def test_curated_jarvis_managed_ids_falls_back_to_hardcoded_on_empty_catalog(
        self, isolated_home
    ):
        from jarvis_cli import model_catalog
        from jarvis_cli.models import get_curated_jarvis_managed_model_ids, _PROVIDER_MODELS

        with patch.object(model_catalog, "_fetch_manifest", return_value=None):
            result = get_curated_jarvis_managed_model_ids()

        assert result == list(_PROVIDER_MODELS["jarvis_managed"])

    def test_curated_jarvis_managed_ids_prefers_manifest(self, isolated_home):
        from jarvis_cli import model_catalog
        from jarvis_cli.models import get_curated_jarvis_managed_model_ids
    ...
    def test_picker_jarvis_managed_row_uses_manifest(self, tmp_path, monkeypatch):
        """The /model picker must surface the manifest's jarvis_managed list, not the
        in-repo _PROVIDER_MODELS["jarvis_managed"] snapshot. Regression: before this
        ...
        the slash-command picker until the next Jarvis release.
        """
        ...
        try:
            ...
            (active_home / "auth.json").write_text(
                json.dumps(
                    {
                        "providers": {"jarvis_managed": {"access_token": "fake"}},
                        "credential_pool": {},
                    }
                )
            )
            ...
            with patch.object(
                ...
                picker = list_picker_providers(
                    current_provider="jarvis_managed", max_models=99
                )
        finally:
            model_catalog.reset_cache()

        jarvis_managed_row = next((r for r in picker if r["slug"] == "jarvis_managed"), None)
        assert jarvis_managed_row is not None, "jarvis_managed row must appear when authed"
        assert jarvis_managed_row["models"] == [
