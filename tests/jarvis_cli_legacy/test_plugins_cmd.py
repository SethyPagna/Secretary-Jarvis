...
class TestDiscoverAllPlugins:
    """Exercise the recursive scan that powers ``jarvis plugins list``.
    ...
    def _entries_by_key(self, tmp_path, monkeypatch) -> dict:
        ...
        user.mkdir()
        monkeypatch.setattr(
            "jarvis_cli.plugins.get_bundled_plugins_dir", lambda: bundled
        )
...
class TestCopyExampleFiles:
    ...
    def test_copies_example_files(self, tmp_path):
        from jarvis_cli.plugins_cmd import _copy_example_files
