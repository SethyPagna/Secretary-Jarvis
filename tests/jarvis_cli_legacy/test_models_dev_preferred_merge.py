...
These guard the contract:
  ...
  * OpenRouter and JARVIS Managed are NEVER merged — they keep their curated
    (OpenRouter) or live-Portal (JARVIS Managed) semantics.
...
class TestProviderModelIdsPreferred:
    ...
    def test_opencode_go_offline_falls_back_to_curated(self):
        ...
        with patch("agent.models_dev.list_agentic_models", return_value=[]):
            out = provider_model_ids("opencode-go")
        # Curated floor (see src/jarvis_cli/models.py _PROVIDER_MODELS["opencode-go"])
        assert "mimo-v2-pro" in out
        assert "kimi-k2.6" in out
...
class TestOpenRouterAndJarvisManagedUnchanged:
    """Per Teknium: openrouter and jarvis_managed are NEVER merged with models.dev."""

    def test_openrouter_not_in_preferred_set(self):
        assert "openrouter" not in _MODELS_DEV_PREFERRED

    def test_jarvis_managed_not_in_preferred_set(self):
        assert "jarvis_managed" not in _MODELS_DEV_PREFERRED
