...
(e.g. orphan merge-conflict markers in ``src/jarvis_cli/config.py``), the CLI
...
markers in ``src/jarvis_cli/config.py``; users who ran ``jarvis update`` in
...
def test_validate_critical_files_syntax_detects_conflict_markers(tmp_path):
    ...
    _populate_critical_tree(tmp_path, broken_file="src/jarvis_cli/config.py")

    ok, failing_path, error = jarvis_main._validate_critical_files_syntax(tmp_path)

    assert ok is False
    assert failing_path is not None and failing_path.endswith("src/jarvis_cli/config.py")
...
def test_validate_critical_files_syntax_detects_break_in_main_py(tmp_path):
    _populate_critical_tree(tmp_path, broken_file="src/jarvis_cli/main.py")

    ok, failing_path, _ = jarvis_main._validate_critical_files_syntax(tmp_path)

    assert ok is False
    assert failing_path is not None and failing_path.endswith("src/jarvis_cli/main.py")
...
def test_validate_critical_files_syntax_tolerates_missing_files(tmp_path):
    ...
    # Populate everything except src/jarvis_cli/constants.py
    for relpath in jarvis_main._UPDATE_CRITICAL_FILES:
        if relpath == "src/jarvis_cli/constants.py":
            continue
