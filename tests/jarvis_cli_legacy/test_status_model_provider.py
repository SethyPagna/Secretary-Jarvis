...
def _patch_common_status_deps(monkeypatch, status_mod, tmp_path, *, openai_base_url=""):
    ...
    monkeypatch.setattr(auth_mod, "get_jarvis_managed_auth_status", lambda: {}, raising=False)
...
def test_show_status_reports_managed_jarvis_managed_features(monkeypatch, capsys, tmp_path):
    monkeypatch.setattr("jarvis_cli.status.managed_jarvis_managed_tools_enabled", lambda: True)
    from jarvis_cli import status as status_mod
    ...
    monkeypatch.setattr(
        status_mod,
        "load_config",
        lambda: {"model": {"default": "claude-opus-4-6", "provider": "jarvis_managed"}},
        raising=False,
    )
    monkeypatch.setattr(status_mod, "resolve_requested_provider", lambda requested=None: "jarvis_managed", raising=False)
    monkeypatch.setattr(status_mod, "resolve_provider", lambda requested=None, **kwargs: "jarvis_managed", raising=False)
    monkeypatch.setattr(status_mod, "provider_label", lambda provider: "JARVIS Managed", raising=False)
    monkeypatch.setattr(
        status_mod,
        "get_jarvis_managed_subscription_features",
        lambda config: JarvisManagedSubscriptionFeatures(
            subscribed=True,
    ...
    out = capsys.readouterr().out
    assert "JARVIS Tool Gateway" in out
    assert "Browser automation" in out
    assert "active via JARVIS Managed subscription" in out


def test_show_status_hides_jarvis_managed_subscription_section_when_feature_flag_is_off(monkeypatch, capsys, tmp_path):
    monkeypatch.setattr("jarvis_cli.status.managed_jarvis_managed_tools_enabled", lambda: False)
    from jarvis_cli import status as status_mod
    ...
    out = capsys.readouterr().out
    assert "JARVIS Tool Gateway" not in out
