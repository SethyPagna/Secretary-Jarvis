"""Regression tests for JARVIS Managed OAuth refresh + agent-key mint interactions."""
...
from jarvis_cli.auth import AuthError, get_provider_auth_state, resolve_jarvis_managed_runtime_credentials
...
def _setup_jarvis_managed_auth(
    jarvis_home: Path,
    *,
...
def test_resolve_jarvis_managed_runtime_credentials_prefers_invoke_jwt_and_mirrors(
    tmp_path,
    monkeypatch,
):
    ...
    _setup_jarvis_managed_auth(
        jarvis_home,
        access_token=token,
        scope=auth_mod.DEFAULT_JARVIS_MANAGED_SCOPE,
    ...
    creds = auth_mod.resolve_jarvis_managed_runtime_credentials(min_key_ttl_seconds=300)

    assert creds["api_key"] == token
    assert creds["source"] == auth_mod.JARVIS_MANAGED_AUTH_PATH_INVOKE_JWT
    assert creds["auth_path"] == auth_mod.JARVIS_MANAGED_AUTH_PATH_INVOKE_JWT

    payload = json.loads((jarvis_home / "auth.json").read_text())
    singleton = payload["providers"]["jarvis_managed"]
    assert singleton["agent_key"] == token
    assert datetime.fromisoformat(singleton["agent_key_expires_at"]).timestamp() > time.time() + 300

    pool_entries = payload["credential_pool"]["jarvis_managed"]
    assert len(pool_entries) == 1
    assert pool_entries[0]["agent_key"] == token
    assert pool_entries[0]["source"] == auth_mod.JARVIS_MANAGED_DEVICE_CODE_SOURCE


def test_resolve_jarvis_managed_runtime_credentials_invoke_jwt_is_idempotent(
    tmp_path,
    monkeypatch,
):
    import jarvis_cli.auth as auth_mod
    ...
    monkeypatch.setattr(auth_mod, "_mint_agent_key", _unexpected_mint)
    monkeypatch.setattr(auth_mod, "_write_shared_jarvis_managed_state", _unexpected_shared_write)
    monkeypatch.setattr(
        auth_mod,
    ...
    assert (
        payload["providers"]["jarvis_managed"]["agent_key_obtained_at"]
        == original_obtained_at
    )


def test_resolve_jarvis_managed_runtime_credentials_trusts_invoke_jwt_exp_over_stale_metadata(
    tmp_path,
    monkeypatch,
):
    import jarvis_cli.auth as auth_mod
...
def test_resolve_jarvis_managed_runtime_credentials_does_not_apply_legacy_ttl_to_invoke_jwt(
    tmp_path,
    monkeypatch,
):
    import jarvis_cli.auth as auth_mod
    ...
    creds = auth_mod.resolve_jarvis_managed_runtime_credentials(min_key_ttl_seconds=1800)

    assert creds["api_key"] == token
    assert creds["source"] == auth_mod.JARVIS_MANAGED_AUTH_PATH_INVOKE_JWT
    payload = json.loads((jarvis_home / "auth.json").read_text())
    assert payload["providers"]["jarvis_managed"]["agent_key"] == token
    assert payload["credential_pool"]["jarvis_managed"][0]["agent_key"] == token
...
def test_legacy_auth_mode_bypasses_usable_invoke_jwt(tmp_path, monkeypatch):
    ...
    creds = auth_mod.resolve_jarvis_managed_runtime_credentials(
        min_key_ttl_seconds=300,
        inference_auth_mode=auth_mod.JARVIS_MANAGED_INFERENCE_AUTH_MODE_LEGACY,
    )

    assert mint_calls == [token]
    assert creds["api_key"] == "legacy-after-jwt-401"
    assert creds["auth_path"] == auth_mod.JARVIS_MANAGED_AUTH_PATH_LEGACY_SESSION_KEY_MINT
    payload = json.loads((jarvis_home / "auth.json").read_text())
    assert payload["providers"]["jarvis_managed"]["agent_key"] == "legacy-after-jwt-401"


def test_resolve_jarvis_managed_runtime_credentials_falls_back_when_invoke_scope_missing(
    tmp_path,
    monkeypatch,
):
    import jarvis_cli.auth as auth_mod
    ...
    _setup_jarvis_managed_auth(
        jarvis_home,
        access_token=token,
        scope=auth_mod.JARVIS_MANAGED_LEGACY_AGENT_KEY_SCOPE,
        expires_at=_future_iso(3600),
        expires_in=3600,
    )
    monkeypatch.setenv("JARVIS_HOME", str(jarvis_home))
    ...
    payload = json.loads((jarvis_home / "auth.json").read_text())
    assert payload["providers"]["jarvis_managed"]["agent_key"] == "opaque-agent-key"
    assert payload["credential_pool"]["jarvis_managed"][0]["agent_key"] == "opaque-agent-key"


def test_jarvis_managed_device_code_login_retries_legacy_scope_when_invoke_refused(monkeypatch):
    import jarvis_cli.auth as auth_mod
    ...
    monkeypatch.setattr(auth_mod, "refresh_jarvis_managed_oauth_from_state", _fake_refresh)

    result = auth_mod._jarvis_managed_device_code_login(
        portal_base_url="https://portal.example.com",
        inference_base_url="https://inference.example.com/v1",
        open_browser=False,
        timeout_seconds=1,
    ...
    assert scopes == [auth_mod.DEFAULT_JARVIS_MANAGED_SCOPE, auth_mod.JARVIS_MANAGED_LEGACY_AGENT_KEY_SCOPE]
    assert result["scope"] == auth_mod.JARVIS_MANAGED_LEGACY_AGENT_KEY_SCOPE
...
def test_forced_legacy_env_skips_invoke_scope_and_jwt_storage(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("JARVIS_HOME", str(jarvis_home))
    monkeypatch.setenv(auth_mod.JARVIS_MANAGED_LEGACY_SESSION_KEYS_ENV, "true")
    ...
    payload = json.loads((jarvis_home / "auth.json").read_text())
    assert payload["providers"]["jarvis_managed"]["agent_key"] == "forced-legacy-key"
    ...
    auth_mod._jarvis_managed_device_code_login(
        portal_base_url="https://portal.example.com",
        inference_base_url="https://inference.example.com/v1",
        open_browser=False,
        timeout_seconds=1,
    ...
    assert requested_scopes == [auth_mod.JARVIS_MANAGED_LEGACY_AGENT_KEY_SCOPE]


def test_jarvis_managed_inference_auth_logs_do_not_include_secret_values(
    tmp_path,
    monkeypatch,
    caplog,
):
    ...
    auth_mod.resolve_jarvis_managed_runtime_credentials(min_key_ttl_seconds=300)
...
def test_get_jarvis_managed_auth_status_checks_credential_pool(tmp_path, monkeypatch):
    """get_jarvis_managed_auth_status() should find JARVIS Managed credentials in the pool
    even when the auth store has no JARVIS Managed provider entry — this is the
    ...
    jarvis_home = tmp_path / "jarvis"
    jarvis_home.mkdir(parents=True, exist_ok=True)
    # Empty auth store — no JARVIS Managed provider entry
    (jarvis_home / "auth.json").write_text(json.dumps({
        "version": 1, "providers": {},
    }))
    monkeypatch.setenv("JARVIS_HOME", str(jarvis_home))

    # Seed the credential pool with a JARVIS Managed entry
    from agent.credential_pool import PooledCredential, load_pool
    pool = load_pool("jarvis_managed")
    entry = PooledCredential.from_dict("jarvis_managed", {
...
def test_get_jarvis_managed_auth_status_auth_store_fallback(tmp_path, monkeypatch):
    """get_jarvis_managed_auth_status() falls back to auth store when credential
    pool is empty.
    """
    from jarvis_cli.auth import get_jarvis_managed_auth_status

    jarvis_home = tmp_path / "jarvis"
    _setup_jarvis_managed_auth(jarvis_home, access_token="at-123")
    monkeypatch.setenv("JARVIS_HOME", str(jarvis_home))
    monkeypatch.setattr(
        "jarvis_cli.auth.resolve_jarvis_managed_runtime_credentials",
...
def test_get_jarvis_managed_auth_status_prefers_runtime_auth_store_over_stale_pool(tmp_path, monkeypatch):
    from jarvis_cli.auth import get_jarvis_managed_auth_status
    from agent.credential_pool import PooledCredential, load_pool

    jarvis_home = tmp_path / "jarvis"
    _setup_jarvis_managed_auth(jarvis_home, access_token="at-fresh")
    monkeypatch.setenv("JARVIS_HOME", str(jarvis_home))

    pool = load_pool("jarvis_managed")
    stale = PooledCredential.from_dict("jarvis_managed", {
...
def test_get_jarvis_managed_auth_status_reports_revoked_refresh_session(tmp_path, monkeypatch):
    from jarvis_cli.auth import get_jarvis_managed_auth_status
    ...
    def _boom(min_key_ttl_seconds=60):
        raise AuthError("Refresh session has been revoked", provider="jarvis_managed", relogin_required=True)

    monkeypatch.setattr("jarvis_cli.auth.resolve_jarvis_managed_runtime_credentials", _boom)
...
def test_get_jarvis_managed_auth_status_empty_returns_not_logged_in(tmp_path, monkeypatch):
    """get_jarvis_managed_auth_status() returns logged_in=False when both pool
...
def test_refresh_token_persisted_when_mint_returns_insufficient_credits(tmp_path, monkeypatch):
    jarvis_home = tmp_path / "jarvis"
    _setup_jarvis_managed_auth(jarvis_home, refresh_token="refresh-old")
    monkeypatch.setenv("JARVIS_HOME", str(jarvis_home))
    ...
    def _fake_mint_agent_key(*, client, portal_base_url, access_token, min_ttl_seconds):
        ...
        if mint_calls["count"] == 1:
            raise AuthError("credits exhausted", provider="jarvis_managed", code="insufficient_credits")
    ...
    monkeypatch.setattr("jarvis_cli.auth._mint_agent_key", _fake_mint_agent_key)

    with pytest.raises(AuthError) as exc:
        resolve_jarvis_managed_runtime_credentials(min_key_ttl_seconds=300)
    assert exc.value.code == "insufficient_credits"

    state_after_failure = get_provider_auth_state("jarvis_managed")
    assert state_after_failure is not None
    assert state_after_failure["refresh_token"] == "refresh-1"
    assert state_after_failure["access_token"] == "access-1"

    creds = resolve_jarvis_managed_runtime_credentials(min_key_ttl_seconds=300)
...
def test_terminal_refresh_failure_quarantines_tokens(
    ...
    """A revoked/invalid JARVIS Managed refresh token must not be replayed forever."""
    ...
    assert load_pool("jarvis_managed").select() is not None
    ...
    auth_mod._write_shared_jarvis_managed_state(shared_state)
    ...
    state_after_failure = auth_mod.get_provider_auth_state("jarvis_managed")
    assert state_after_failure is not None
    ...
    assert auth_mod._read_shared_jarvis_managed_state() is None
    payload = json.loads((jarvis_home / "auth.json").read_text())
    assert payload.get("credential_pool", {}).get("jarvis_managed") == []
...
def test_managed_access_token_refresh_failure_quarantines_tokens(
    ...
    with pytest.raises(AuthError, match="Invalid refresh token"):
        auth_mod.resolve_jarvis_managed_access_token()
...
def test_mint_retry_uses_latest_rotated_refresh_token(tmp_path, monkeypatch):
    ...
    def _fake_mint_agent_key(*, client, portal_base_url, access_token, min_ttl_seconds):
        mint_calls["count"] += 1
        if mint_calls["count"] == 1:
            raise AuthError("stale access token", provider="jarvis_managed", code="invalid_token")
...
# =============================================================================
# _login_jarvis_managed: "Skip (keep current)" must preserve prior provider + model
# =============================================================================


class TestLoginJarvisManagedSkipKeepsCurrent:
    """When a user runs `jarvis model` → JARVIS Managed → Skip (keep current) after
    ...
    unconditionally after login, which flipped model.provider to "jarvis_managed" while
    ...
    def _patch_login_internals(self, monkeypatch, *, prompt_returns):
        """Patch OAuth + model-list + prompt so _login_jarvis_managed doesn't hit network."""
        import jarvis_cli.auth as auth_mod
        import jarvis_cli.models as models_mod
        import jarvis_cli.jarvis_managed_subscription as ns

        fake_auth_state = {
            "access_token": "fake-jarvis_managed-token",
            "agent_key": "fake-agent-key",
            "inference_base_url": "https://inference-api.jarvis.local",
        ...
        monkeypatch.setattr(models_mod, "check_jarvis_managed_free_tier", lambda: None)
        monkeypatch.setattr(
            models_mod, "partition_jarvis_managed_models_by_tier",
    ...
    def test_skip_keep_current_preserves_provider_and_model(self, tmp_path, monkeypatch):
        """User picks Skip → config.yaml untouched, JARVIS Managed creds still saved."""
        import argparse
        import yaml
        from jarvis_cli.auth import PROVIDER_REGISTRY, _login_jarvis_managed
        ...
        _login_jarvis_managed(args, PROVIDER_REGISTRY["jarvis_managed"])
        ...
        # auth.json: active_provider restored to openrouter, but JARVIS Managed creds saved
        auth_after = json.loads(auth_path.read_text())
        assert auth_after["active_provider"] == "openrouter"
        assert "jarvis_managed" in auth_after["providers"]
        assert auth_after["providers"]["jarvis_managed"]["access_token"] == "fake-jarvis_managed-token"
        # Existing openrouter creds still intact
        assert auth_after["providers"]["openrouter"]["api_key"] == "sk-or-fake"

    def test_picking_model_switches_to_jarvis_managed(self, tmp_path, monkeypatch):
        """User picks a JARVIS Managed model → provider flips to jarvis_managed with that model."""
        import argparse
        import yaml
        from jarvis_cli.auth import PROVIDER_REGISTRY, _login_jarvis_managed
        ...
        cfg_after = yaml.safe_load(config_path.read_text())
        assert cfg_after["model"]["provider"] == "jarvis_managed"
        assert cfg_after["model"]["default"] == "xiaomi/mimo-v2-pro"

        auth_after = json.loads(auth_path.read_text())
        assert auth_after["active_provider"] == "jarvis_managed"
    ...
    def test_skip_with_no_prior_active_provider_clears_it(self, tmp_path, monkeypatch):
        ...
        instead of leaving it as jarvis_managed."""
        import argparse
        import yaml
        from jarvis_cli.auth import PROVIDER_REGISTRY, _login_jarvis_managed
        ...
        auth_path = jarvis_home / "auth.json"
        auth_after = json.loads(auth_path.read_text())
        # active_provider should NOT be set to "jarvis_managed" after Skip
        assert auth_after.get("active_provider") in {None, ""}
        # But JARVIS Managed creds are still saved
        assert "jarvis_managed" in auth_after.get("providers", {})


# =============================================================================
# persist_jarvis_managed_credentials: shared helper for CLI + web dashboard login paths
# =============================================================================


def _full_state_fixture() -> dict:
    """Shape of the dict returned by _jarvis_managed_device_code_login /
    refresh_jarvis_managed_oauth_from_state. Used as helper input."""
    return {
        "portal_base_url": "https://portal.example.com",
        "inference_base_url": "https://inference.example.com/v1",
        "client_id": "jarvis-cli",
...
def test_persist_jarvis_managed_credentials_writes_both_pool_and_providers(tmp_path, monkeypatch):
    """Helper must populate BOTH credential_pool.jarvis_managed AND providers.jarvis_managed.
    ...
    401-recovery path in run_agent.py called resolve_jarvis_managed_runtime_credentials
    ...
    """
    from jarvis_cli.auth import persist_jarvis_managed_credentials, JARVIS_MANAGED_DEVICE_CODE_SOURCE
    ...
    entry = persist_jarvis_managed_credentials(_full_state_fixture())

    assert entry is not None
    assert entry.provider == "jarvis_managed"
    assert entry.source == JARVIS_MANAGED_DEVICE_CODE_SOURCE

    payload = json.loads((jarvis_home / "auth.json").read_text())

    # providers.jarvis_managed populated with the full state (new behaviour)
    singleton = payload["providers"]["jarvis_managed"]
    ...
    # credential_pool.jarvis_managed has exactly one canonical device_code entry
    pool_entries = payload["credential_pool"]["jarvis_managed"]
    assert len(pool_entries) == 1, pool_entries
    pool_entry = pool_entries[0]
    assert pool_entry["source"] == JARVIS_MANAGED_DEVICE_CODE_SOURCE
    assert pool_entry["agent_key"] == "agent-key-value"
    assert pool_entry["inference_base_url"] == "https://inference.example.com/v1"


def test_persist_jarvis_managed_credentials_allows_recovery_from_401(tmp_path, monkeypatch):
    ...
    This is the exact path that run_agent.py's `_try_refresh_jarvis_managed_client_credentials`
    ...
    from jarvis_cli.auth import (
        ...
        persist_jarvis_managed_credentials,
    ...
    persist_jarvis_managed_credentials(_full_state_fixture())
    ...
    # doesn't raise "Jarvis is not logged into JARVIS Managed".
    def _fake_refresh_access_token(*, client, portal_base_url, client_id, refresh_token):
        return {
...
def test_persist_jarvis_managed_credentials_idempotent_no_duplicate_pool_entries(tmp_path, monkeypatch):
    ...
    ``load_pool()``. The helper now writes providers.jarvis_managed and lets seeding
    ...
    persist_jarvis_managed_credentials(first)

    second = _full_state_fixture()
    second["access_token"] = "access-second"
    second["agent_key"] = "agent-key-second"
    persist_jarvis_managed_credentials(second)

    payload = json.loads((jarvis_home / "auth.json").read_text())

    # providers.jarvis_managed reflects the latest write (singleton semantics)
    assert payload["providers"]["jarvis_managed"]["access_token"] == "access-second"
    assert payload["providers"]["jarvis_managed"]["agent_key"] == "agent-key-second"

    # credential_pool.jarvis_managed has exactly one entry, carrying the latest agent_key
    pool_entries = payload["credential_pool"]["jarvis_managed"]
    assert len(pool_entries) == 1, pool_entries
    assert pool_entries[0]["source"] == JARVIS_MANAGED_DEVICE_CODE_SOURCE
...
def test_persist_jarvis_managed_credentials_reloads_pool_after_singleton_write(tmp_path, monkeypatch):
...
def test_persist_jarvis_managed_credentials_embeds_custom_label(tmp_path, monkeypatch):
    """User-supplied ``--label`` round-trips through providers.jarvis_managed and the pool.

    Previously `jarvis auth add jarvis_managed --type oauth --label <name>` silently
    dropped the label because persist_jarvis_managed_credentials() ignored it and
    _seed_from_singletons always auto-derived via label_from_token().  The
    fix stashes the label inside providers.jarvis_managed so seeding prefers it.
    """
    from jarvis_cli.auth import persist_jarvis_managed_credentials, JARVIS_MANAGED_DEVICE_CODE_SOURCE
    ...
    entry = persist_jarvis_managed_credentials(_full_state_fixture(), label="my-personal")
    assert entry is not None
    assert entry.source == JARVIS_MANAGED_DEVICE_CODE_SOURCE
    assert entry.label == "my-personal"

    # providers.jarvis_managed carries the label so re-seeding on the next load_pool
    # doesn't overwrite it with the auto-derived fingerprint.
    payload = json.loads((jarvis_home / "auth.json").read_text())
    assert payload["providers"]["jarvis_managed"]["label"] == "my-personal"


def test_persist_jarvis_managed_credentials_custom_label_survives_reseed(tmp_path, monkeypatch):
    ...
    persist_jarvis_managed_credentials(_full_state_fixture(), label="work-acct")
...
def test_persist_jarvis_managed_credentials_no_label_uses_auto_derived(tmp_path, monkeypatch):
    ...
    # No "label" key embedded in providers.jarvis_managed when the caller didn't supply one.
    payload = json.loads((jarvis_home / "auth.json").read_text())
    assert "label" not in payload["providers"]["jarvis_managed"]


def test_refresh_token_reuse_detection_surfaces_actionable_message():
    """Regression for #15099.

    When the JARVIS Managed server returns ``invalid_grant`` with
    ...
    assert "jarvis auth add jarvis_managed" in message.lower()
...
def test_refresh_token_reuse_error_code_is_terminal():
    """JARVIS Managed may return refresh_token_reused as the OAuth error code itself."""
    from jarvis_cli import auth as auth_mod
    ...
    assert exc_info.value.relogin_required is True
    assert auth_mod._is_terminal_jarvis_managed_refresh_error(exc_info.value) is True


def test_refresh_token_exchange_sends_refresh_token_header():
    """JARVIS Managed refresh tokens must be sent in a header so sandbox proxies can
    ...
    assert client.kwargs["headers"]["x-jarvis_managed-refresh-token"] == "refresh-1"
...
# =============================================================================
# Shared JARVIS Managed token store — cross-profile persistence (Codex-style auto-import)
# =============================================================================
...
def shared_store_env(tmp_path, monkeypatch):
    ...
    Required for every test that exercises the shared JARVIS Managed store — the
...
def test_shared_store_seat_belt_refuses_real_home_under_pytest(monkeypatch):
    ...
    """
    from jarvis_cli.auth import _jarvis_managed_shared_store_path

    monkeypatch.delenv("JARVIS_SHARED_AUTH_DIR", raising=False)

    with pytest.raises(RuntimeError, match="shared JARVIS Managed auth store"):
        _jarvis_managed_shared_store_path()


def test_shared_store_honors_env_override(tmp_path, monkeypatch):
    """JARVIS_SHARED_AUTH_DIR must redirect the path."""
    from jarvis_cli.auth import _jarvis_managed_shared_store_path, JARVIS_MANAGED_SHARED_STORE_FILENAME
    ...
    assert path == custom_dir / JARVIS_MANAGED_SHARED_STORE_FILENAME


def test_shared_store_read_missing_returns_none(shared_store_env):
    """Missing file → ``_read_shared_jarvis_managed_state()`` returns None."""
    from jarvis_cli.auth import _read_shared_jarvis_managed_state

    assert _read_shared_jarvis_managed_state() is None


def test_shared_store_read_malformed_returns_none(shared_store_env):
    """Unreadable / non-JSON file → None, not an exception."""
    from jarvis_cli.auth import _jarvis_managed_shared_store_path, _read_shared_jarvis_managed_state
...
def test_shared_store_write_and_read_roundtrip(shared_store_env):
    ...
    from jarvis_cli.auth import (
        _jarvis_managed_shared_store_path,
        _read_shared_jarvis_managed_state,
        _write_shared_jarvis_managed_state,
    )

    _write_shared_jarvis_managed_state(_full_state_fixture())
    ...
    loaded = _read_shared_jarvis_managed_state()
...
def test_shared_store_write_skips_when_refresh_token_missing(shared_store_env):
    ...
    from jarvis_cli.auth import _jarvis_managed_shared_store_path, _write_shared_jarvis_managed_state
    ...
    assert not _jarvis_managed_shared_store_path().is_file()


def test_persist_jarvis_managed_credentials_mirrors_to_shared_store(
    tmp_path, monkeypatch, shared_store_env,
):
    """persist_jarvis_managed_credentials must populate BOTH per-profile auth.json
    AND the shared store, so a future profile's `jarvis auth add jarvis_managed
    ...
    payload = json.loads((jarvis_home / "auth.json").read_text())
    assert "jarvis_managed" in payload.get("providers", {})
    ...
    assert str(_jarvis_managed_shared_store_path()).startswith(str(shared_store_env))
...
def test_try_import_shared_returns_none_when_store_missing(shared_store_env):
    ...
    from jarvis_cli.auth import _try_import_shared_jarvis_managed_state

    assert _try_import_shared_jarvis_managed_state() is None
...
def test_try_import_shared_returns_none_on_refresh_failure(
    ...
    portal down), _try_import_shared_jarvis_managed_state must return None so the
    login flow falls back to a fresh device-code run.
    """
    from jarvis_cli import auth as auth_mod

    # Seed the shared store
    auth_mod._write_shared_jarvis_managed_state(_full_state_fixture())
    ...
    monkeypatch.setattr(auth_mod, "refresh_jarvis_managed_oauth_from_state", _boom)

    assert auth_mod._try_import_shared_jarvis_managed_state() is None
    assert auth_mod._read_shared_jarvis_managed_state() is None
...
def test_try_import_shared_persists_rotated_token_when_mint_fails(
    ...
    shared_after = auth_mod._read_shared_jarvis_managed_state()
...
def test_try_import_shared_rehydrates_on_success(shared_store_env, monkeypatch):
    ...
    every field persist_jarvis_managed_credentials() needs.
    """
    from jarvis_cli import auth as auth_mod
    ...
    def _fake_refresh(state, **kwargs):
        ...
        assert (
            ...
            == auth_mod.JARVIS_MANAGED_INFERENCE_AUTH_MODE_FRESH
        )
        return {
            **state,
    ...
    result = auth_mod._try_import_shared_jarvis_managed_state()
...
def test_shared_store_survives_across_profile_switch(
    ...
    auth_mod.persist_jarvis_managed_credentials(_full_state_fixture())

    # Profile A's auth.json has jarvis_managed
    a_payload = json.loads((profile_a / "auth.json").read_text())
    assert "jarvis_managed" in a_payload.get("providers", {})

    # Profile B: fresh JARVIS_HOME, no auth yet, but the shared store
    # persists — _read_shared_jarvis_managed_state() must still return the tokens.
    ...
    # B's own auth.json has no jarvis_managed
    b_payload = json.loads((profile_b / "auth.json").read_text())
    assert "jarvis_managed" not in b_payload.get("providers", {})
    ...
    # And a successful rehydrate + persist lands jarvis_managed into profile B
    def _fake_refresh(state, **kwargs):
        return {
            **state,
    ...
    auth_mod.persist_jarvis_managed_credentials(result)

    b_payload = json.loads((profile_b / "auth.json").read_text())
    assert "jarvis_managed" in b_payload.get("providers", {})
    assert b_payload["providers"]["jarvis_managed"]["refresh_token"] == "b-refresh-tok"
...
def test_runtime_refresh_uses_newer_shared_token_before_local_stale_token(
    ...
    """A sibling profile may rotate the single-use JARVIS Managed refresh token.
    ...
    creds = auth_mod.resolve_jarvis_managed_runtime_credentials(
        min_key_ttl_seconds=300,
        inference_auth_mode=auth_mod.JARVIS_MANAGED_INFERENCE_AUTH_MODE_FRESH,
    )

    assert creds["api_key"] == "agent-key-from-shared-token"
    assert minted_with == ["shared-fresh-access"]

    profile_state = auth_mod.get_provider_auth_state("jarvis_managed")
...
def test_managed_gateway_access_token_uses_newer_shared_token(
    ...
    assert auth_mod.resolve_jarvis_managed_access_token() == "shared-fresh-access"
