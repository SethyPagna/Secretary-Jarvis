...
def test_setup_syncs_jarvis_managed_from_disk(tmp_path, monkeypatch):
    """JARVIS Managed OAuth writes config to disk; wizard config dict must pick it up."""
    monkeypatch.setenv("JARVIS_HOME", str(tmp_path))
    _clear_provider_env(monkeypatch)
    _stub_tts(monkeypatch)
    ...
    def fake_select():
        _write_model_config(tmp_path, "jarvis_managed", "https://inference.example.com/v1", "gemini-3-flash")
    ...
    reloaded = load_config()
    assert isinstance(reloaded["model"], dict)
    assert reloaded["model"]["provider"] == "jarvis_managed"
...
def test_modal_setup_can_use_jarvis_managed_subscription_without_modal_creds(tmp_path, monkeypatch, capsys):
    monkeypatch.setattr("jarvis_cli.setup.managed_jarvis_managed_tools_enabled", lambda: True)
    monkeypatch.setenv("JARVIS_HOME", str(tmp_path))
    config = load_config()
    ...
    monkeypatch.setattr(
        ...
        lambda config: type("Features", (), {"jarvis_managed_auth_present": True})(),
    )
    monkeypatch.setitem(
        sys.modules,
