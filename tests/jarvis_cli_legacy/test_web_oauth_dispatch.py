...
def _fake_jarvis_managed_device_data():
    return {
        "device_code": "device-code",
        "user_code": "JARVIS-1234",
        "verification_uri": "https://portal.jarvis.local/device",
        "verification_uri_complete": (
            "https://portal.jarvis.local/device?user_code=JARVIS-1234"
        ),
        "expires_in": 600,
        "interval": 5,
    }
...
def _invoke_scope_refusal():
    request = httpx.Request("POST", "https://portal.jarvis.local/oauth/device/code")
...
def test_jarvis_managed_dashboard_device_flow_honors_legacy_scope_override(monkeypatch):
    ...
    def fake_request_device_code(**kwargs):
        ...
        return _fake_jarvis_managed_device_data()
    ...
    monkeypatch.setattr(ws, "_jarvis_managed_poller", lambda sid: None)

    result = asyncio.run(ws._start_device_code_flow("jarvis_managed"))
    try:
        assert requested_scopes == [auth_mod.JARVIS_MANAGED_LEGACY_AGENT_KEY_SCOPE]
        assert result["flow"] == "device_code"
        assert result["user_code"] == "JARVIS-1234"
        assert (
            ws._oauth_sessions[result["session_id"]]["scope"]
            == auth_mod.JARVIS_MANAGED_LEGACY_AGENT_KEY_SCOPE
        )
    finally:
        ws._oauth_sessions.pop(result["session_id"], None)


def test_jarvis_managed_dashboard_device_flow_retries_legacy_scope_on_invoke_refusal(monkeypatch):
    from jarvis_cli import auth as auth_mod
    from jarvis_cli import web_server as ws
    ...
    monkeypatch.delenv(auth_mod.JARVIS_MANAGED_LEGACY_SESSION_KEYS_ENV, raising=False)
    monkeypatch.setattr(auth_mod, "_request_device_code", fake_request_device_code)
    monkeypatch.setattr(ws, "_jarvis_managed_poller", lambda sid: None)
    ...
    try:
        assert requested_scopes == [
            auth_mod.DEFAULT_JARVIS_MANAGED_SCOPE,
            auth_mod.JARVIS_MANAGED_LEGACY_AGENT_KEY_SCOPE,
        ]
        assert (
            ws._oauth_sessions[result["session_id"]]["scope"]
...
def test_jarvis_managed_dashboard_poller_preserves_effective_scope_when_token_omits_scope(monkeypatch):
    from jarvis_cli import auth as auth_mod
    from jarvis_cli import web_server as ws

    session_id = "jarvis_managed-effective-scope-test"
    ...
    def fake_refresh_jarvis_managed_oauth_from_state(state, **kwargs):
    ...
    monkeypatch.setattr(
        ...
        fake_refresh_jarvis_managed_oauth_from_state,
    )
    monkeypatch.setattr(auth_mod, "persist_jarvis_managed_credentials", lambda state: None)

    try:
        ws._jarvis_managed_poller(session_id)
        assert captured_state["scope"] == auth_mod.JARVIS_MANAGED_LEGACY_AGENT_KEY_SCOPE
