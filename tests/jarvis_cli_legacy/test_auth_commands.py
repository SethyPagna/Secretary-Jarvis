...
def test_auth_add_anthropic_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "agent.anthropic_adapter.run_jarvis_oauth_login_pure",
        lambda: {
            "access_token": token,
...
def test_auth_add_jarvis_managed_oauth_persists_pool_entry(tmp_path, monkeypatch):
    monkeypatch.setenv("JARVIS_HOME", str(tmp_path / "jarvis"))
    _write_auth_store(tmp_path, {"version": 1, "providers": {}})
    token = _jwt_with_email("jarvis_managed@example.com")
    monkeypatch.setattr(
        "jarvis_cli.auth._jarvis_managed_device_code_login",
        lambda **kwargs: {
    ...
    entries = payload["credential_pool"]["jarvis_managed"]
    device_code_entries = [
    ...
    # `jarvis auth add jarvis_managed` must also populate providers.jarvis_managed so the
    # 401-recovery path (resolve_jarvis_managed_runtime_credentials) can mint a fresh
    # agent_key when the 24h TTL expires. If this mirror is missing, recovery
    # raises "Jarvis is not logged into JARVIS Managed" and the agent dies.
...
def test_auth_add_jarvis_managed_oauth_honors_custom_label(tmp_path, monkeypatch):
    """`jarvis auth add jarvis_managed --type oauth --label <name>` must preserve the
    custom label end-to-end — it was silently dropped in the first cut of the
    persist_jarvis_managed_credentials helper because `--label` wasn't threaded through.
    """
    monkeypatch.setenv("JARVIS_HOME", str(tmp_path / "jarvis"))
    _write_auth_store(tmp_path, {"version": 1, "providers": {}})
    token = _jwt_with_email("jarvis_managed@example.com")
    ...
    pool_entry = payload["credential_pool"]["jarvis_managed"][0]
    assert pool_entry["source"] == "device_code"
    assert pool_entry["label"] == "my-jarvis_managed"

    # … and survives in providers.jarvis_managed so a subsequent load_pool() re-seeds
    # it without reverting to the auto-derived fingerprint.
    assert payload["providers"]["jarvis_managed"]["label"] == "my-jarvis_managed"
...
def test_seed_from_singletons_respects_jarvis_managed_suppression(tmp_path, monkeypatch):
    """jarvis_managed device_code must not re-seed from auth.json when suppressed."""
    jarvis_home = tmp_path / "jarvis"
    jarvis_home.mkdir(parents=True, exist_ok=True)
    monkeypatch.setenv("JARVIS_HOME", str(jarvis_home))
    ...
    (jarvis_home / "auth.json").write_text(json.dumps({
        "version": 1,
        "providers": {"jarvis_managed": {"access_token": "tok", "refresh_token": "r", "expires_at": 9999999999}},
        "suppressed_sources": {"jarvis_managed": ["device_code"]},
    }))

    from agent.credential_pool import _seed_from_singletons
    entries = []
    changed, active = _seed_from_singletons("jarvis_managed", entries)
    assert changed is False
    assert entries == []
    assert active == set()
...
def test_credential_sources_registry_has_expected_steps():
    ...
    required = {
        ...
        "auth.json providers.jarvis_managed",
