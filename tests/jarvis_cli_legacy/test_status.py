...
def test_show_status_reports_jarvis_managed_auth_error(monkeypatch, capsys, tmp_path):
    from jarvis_cli import status as status_mod
    import jarvis_cli.auth as auth_mod
    import jarvis_cli.gateway as gateway_mod
    ...
    output = capsys.readouterr().out
    assert "JARVIS Managed   ✗ not logged in (run: jarvis auth add jarvis_managed --type oauth)" in output
...
class TestShowStatusXaiOAuth:
    ...
    def test_import_failure_does_not_break_other_oauth_providers(self, monkeypatch, capsys, tmp_path):
        """JARVIS Managed/Codex/MiniMax rows must still appear when xAI import fails."""
        import jarvis_cli.auth as auth_mod
        status_mod = _base_xai_mocks(monkeypatch, tmp_path)
        monkeypatch.setattr(auth_mod, "get_jarvis_managed_auth_status",
