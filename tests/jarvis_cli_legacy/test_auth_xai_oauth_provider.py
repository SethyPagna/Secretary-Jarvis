...
def test_resolve_credentials_quarantines_dead_tokens_on_terminal_refresh_failure(
    ...
    """
    jarvis_home = tmp_path / "jarvis"
    _seed_xai_oauth_state(jarvis_home, dict(_STALE_XAI_OAUTH_STATE), active_provider="jarvis_managed")
    monkeypatch.setenv("JARVIS_HOME", str(jarvis_home))
    ...
    assert raw["active_provider"] == "jarvis_managed"
...
def test_pool_refresh_adopts_singleton_tokens_when_consumed_elsewhere(tmp_path, monkeypatch):
    ...
    Mirrors the proactive sync codex/jarvis_managed already perform for the same
...
def test_pool_exhausted_xai_entry_recovers_after_singleton_refresh(tmp_path, monkeypatch):
    ...
    cooldown elapses.  Mirrors the codex/jarvis_managed self-heal pattern."""
...
def test_auxiliary_client_routes_xai_oauth_through_responses_api(tmp_path, monkeypatch):
    ...
    JARVIS Managed, etc.).  Users on xAI Grok OAuth would then see surprise charges
...
def test_pool_sync_back_preserves_active_provider(tmp_path, monkeypatch):
    ...
    using it on the sync-back path means every xAI/Codex/JARVIS Managed refresh in a
    ...
    assert raw_after["active_provider"] == "openrouter", (
        ...
        "JARVIS Managed token rotations silently take over multi-provider users' "
