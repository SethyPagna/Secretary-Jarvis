...
# ---------------------------------------------------------------------------
# JARVIS Managed shared-credential store write (inside _write_shared_jarvis_managed_state)
# ---------------------------------------------------------------------------


def test_shared_jarvis_managed_store_writes_0o600_with_0o700_parent(tmp_path, monkeypatch):
    """The JARVIS Managed shared-credential store must land at 0o600 / parent 0o700."""
    monkeypatch.setenv("JARVIS_HOME", str(tmp_path))
    # _jarvis_managed_shared_store_path() refuses to touch the real shared store during
    ...
    try:
        ...
        state = {
            "access_token": "jarvis_managed-access-xxx",
            "refresh_token": "jarvis_managed-refresh-xxx",
            "token_type": "Bearer",
        ...
        auth_mod._write_shared_jarvis_managed_state(state)
        path = auth_mod._jarvis_managed_shared_store_path()
    finally:
        os.umask(old_umask)

    assert path.exists(), "shared JARVIS Managed store was not written"
    mode = stat.S_IMODE(path.stat().st_mode)
    parent_mode = stat.S_IMODE(path.parent.stat().st_mode)

    assert mode == 0o600, (
        f"JARVIS Managed shared store mode 0o{mode:o} != 0o600 — TOCTOU race regressed"
    )
    assert parent_mode == 0o700, (
        f"JARVIS Managed shared store parent dir mode 0o{parent_mode:o} != 0o700"
    )

    data = json.loads(path.read_text())
    assert data["refresh_token"] == "jarvis_managed-refresh-xxx"
