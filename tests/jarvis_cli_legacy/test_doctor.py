...
class TestDoctorMemoryProviderSection:
    ...
    def _run_doctor_and_capture(self, monkeypatch, tmp_path, provider=""):
        ...
        try:
            ...
            monkeypatch.setattr(_auth_mod, "get_jarvis_managed_auth_status", lambda: {})
...
class TestGitHubTokenCheck:
    ...
    def test_no_token_and_not_gh_authenticated_shows_warn(self, monkeypatch, tmp_path):
        ...
        from jarvis_cli.doctor import run_doctor, _DHH
        import io, contextlib
...
def _run_doctor_with_healthy_oauth_fallback(
    ...
    *,
    ...
    monkeypatch.setattr(_auth_mod, "get_jarvis_managed_auth_status", lambda: {"logged_in": True})
    monkeypatch.setattr(_auth_mod, "get_codex_auth_status", lambda: {})
...
class TestDoctorXaiOAuthStatus:
    ...
    failure (or runtime exception) cannot silence the JARVIS Managed / Codex / Gemini /
    ...
    def _run(self, monkeypatch, tmp_path, *, xai_auth_fn) -> str:
        ...
        from jarvis_cli import auth as _auth_mod
        monkeypatch.setattr(_auth_mod, "get_jarvis_managed_auth_status", lambda: {"logged_in": False})
    ...
    def test_import_failure_does_not_affect_other_providers(self, monkeypatch, tmp_path):
        """JARVIS Managed / Codex / Gemini / MiniMax rows must survive an xAI import failure."""
        home = tmp_path / ".jarvis"
        home.mkdir(parents=True, exist_ok=True)
        (home / "config.yaml").write_text("memory: {}\n", encoding="utf-8")
        project = tmp_path / "project"
        ...
        with contextlib.redirect_stdout(buf):
            doctor_mod.run_doctor(Namespace(fix=False))
        out = buf.getvalue()
        assert "JARVIS Managed auth" in out
        assert "logged in" in out
