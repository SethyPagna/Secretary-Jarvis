...
class TestParseModelInput:
    ...
    def test_provider_colon_model_switches_provider(self):
        provider, model = parse_model_input("openrouter:anthropic/claude-sonnet-4.5", "jarvis_managed")
        assert provider == "openrouter"
        assert model == "anthropic/claude-sonnet-4.5"
    ...
    def test_jarvis_managed_provider_switch(self):
        provider, model = parse_model_input("jarvis_managed:jarvis-3", "openrouter")
        assert provider == "jarvis_managed"
        assert model == "jarvis-3"

    def test_empty_model_after_colon_keeps_current(self):
        provider, model = parse_model_input("openrouter:", "jarvis_managed")
        assert provider == "jarvis_managed"
...
class TestProviderModelIds:
    ...
    def test_copilot_falls_back_to_curated_defaults_without_stale_opus(self):
        with patch("jarvis_cli.models._resolve_copilot_catalog_api_key", return_value="gh-token"), \
             patch("jarvis_cli.models._fetch_github_models", return_value=None):
