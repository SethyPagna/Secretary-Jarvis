...
def test_save_platform_tools_preserves_mcp_server_names():
    ...
    Regression test for https://github.com/JARVISProject/jarvis-agent/issues/1247
    """
    config = {
        "platform_toolsets": {
...
def test_visible_providers_include_jarvis_managed_subscription_when_logged_in(monkeypatch):
    monkeypatch.setattr("jarvis_cli.tools_config.managed_jarvis_managed_tools_enabled", lambda: True)
    config = {"model": {"provider": "jarvis_managed"}}

    monkeypatch.setattr(
        "jarvis_cli.jarvis_managed_subscription.get_jarvis_managed_auth_status",
        lambda: {"logged_in": True},
    )

    providers = _visible_providers(TOOL_CATEGORIES["browser"], config)

    assert providers[0]["name"].startswith("JARVIS Managed")


def test_visible_providers_hide_jarvis_managed_subscription_when_feature_flag_is_off(monkeypatch):
    monkeypatch.setattr("jarvis_cli.tools_config.managed_jarvis_managed_tools_enabled", lambda: False)
    config = {"model": {"provider": "jarvis_managed"}}
    ...
    assert all(not provider["name"].startswith("JARVIS Managed") for provider in providers)
...
def test_first_install_jarvis_managed_auto_configures_managed_defaults(monkeypatch):
    monkeypatch.setattr("jarvis_cli.tools_config.managed_jarvis_managed_tools_enabled", lambda: True)
    monkeypatch.setattr("jarvis_cli.jarvis_managed_subscription.managed_jarvis_managed_tools_enabled", lambda: True)
    config = {
    ...
    # apply_jarvis_managed_managed_defaults per platform; a second iteration sees values
...
def test_numeric_mcp_server_name_does_not_crash_sorted():
    ...
    Regression test for https://github.com/JARVISProject/jarvis-agent/issues/6901
    """
    config = {
...
class TestImagegenBackendRegistry:
    ...
    def test_image_gen_providers_tagged_with_fal_backend(self):
        """Both JARVIS Managed and FAL.ai providers must carry the
...
def test_get_platform_tools_recovers_non_configurable_toolsets_from_composite():
    ...
    with mock_patch("jarvis_cli.tools_config.PLATFORMS", {**PLATFORMS, **test_platforms}):
        with mock_patch("tools.toolsets.TOOLSETS", fake_toolsets):
...
@pytest.mark.parametrize("provider,config_key,expected", [
    ...
    ({"name": "T", "tts_provider": "elevenlabs", "managed_jarvis_managed_feature": "tts", "env_vars": []}, "tts", True),
    ({"name": "B", "browser_provider": "browserbase", "managed_jarvis_managed_feature": "browser", "env_vars": []}, "browser", True),
    ({"name": "W", "web_backend": "tavily", "managed_jarvis_managed_feature": "web", "env_vars": []}, "web", True),
