"""Tests for JARVIS Managed subscription feature detection."""

from jarvis_cli import jarvis_managed_subscription as ns


def test_get_jarvis_managed_subscription_features_recognizes_direct_exa_backend(monkeypatch):
    env = {"EXA_API_KEY": "exa-test"}

    monkeypatch.setattr(ns, "get_env_value", lambda name: env.get(name, ""))
    monkeypatch.setattr(ns, "get_jarvis_managed_auth_status", lambda: {})
    monkeypatch.setattr(ns, "managed_jarvis_managed_tools_enabled", lambda: False)
    monkeypatch.setattr(ns, "_toolset_enabled", lambda config, key: key == "web")
    monkeypatch.setattr(ns, "_has_agent_browser", lambda: False)
    monkeypatch.setattr(ns, "resolve_openai_audio_api_key", lambda: "")
    monkeypatch.setattr(ns, "has_direct_modal_credentials", lambda: False)

    features = ns.get_jarvis_managed_subscription_features({"web": {"backend": "exa"}})

    assert features.web.available is True
    assert features.web.active is True
    assert features.web.managed_by_jarvis_managed is False
    assert features.web.direct_override is True
    assert features.web.current_provider == "exa"


def test_get_jarvis_managed_subscription_features_prefers_managed_modal_in_auto_mode(monkeypatch):
    monkeypatch.setattr("tools.tool_backend_helpers.managed_jarvis_managed_tools_enabled", lambda: True)
    monkeypatch.setattr(ns, "get_env_value", lambda name: "")
    monkeypatch.setattr(ns, "get_jarvis_managed_auth_status", lambda: {"logged_in": True})
    monkeypatch.setattr(ns, "managed_jarvis_managed_tools_enabled", lambda: True)
    ...
    features = ns.get_jarvis_managed_subscription_features(
        {"terminal": {"backend": "modal", "modal_mode": "auto"}}
    )

    assert features.modal.available is True
    assert features.modal.active is True
    assert features.modal.managed_by_jarvis_managed is True
    assert features.modal.direct_override is False


def test_get_jarvis_managed_subscription_features_marks_browser_use_as_managed_when_gateway_ready(monkeypatch):
    monkeypatch.setattr(ns, "get_env_value", lambda name: "")
    monkeypatch.setattr(ns, "get_jarvis_managed_auth_status", lambda: {"logged_in": True})
    monkeypatch.setattr(ns, "managed_jarvis_managed_tools_enabled", lambda: True)
    ...
    assert features.browser.managed_by_jarvis_managed is True
    assert features.browser.direct_override is False
    assert features.browser.current_provider == "Browser Use"


def test_get_jarvis_managed_subscription_features_uses_direct_browserbase_when_no_managed_gateway(monkeypatch):
    ...
    assert features.browser.available is True
    assert features.browser.active is True
    assert features.browser.managed_by_jarvis_managed is False
    assert features.browser.direct_override is True
    assert features.browser.current_provider == "Browserbase"


def test_get_jarvis_managed_subscription_features_prefers_camofox_over_managed_browser_use(monkeypatch):
...
def test_get_jarvis_managed_subscription_features_requires_agent_browser_for_browserbase(monkeypatch):
    env = {
        "BROWSERBASE_API_KEY": "bb-key",
        "BROWSERBASE_PROJECT_ID": "bb-project",
    }
...
def test_get_jarvis_managed_subscription_features_does_not_treat_quoted_false_as_gateway_opt_in(monkeypatch):
    env = {"EXA_API_KEY": "exa-test"}
