...
class TestUpdateConfigForProviderClearsStaleCustomFields:
    ...
    def test_switching_to_jarvis_managed_clears_stale_api_mode(self):
        _seed_custom_provider_config()
        _update_config_for_provider("jarvis_managed", "https://inference-api.jarvis.local/v1")
        model_cfg = _read_model_cfg()
        assert model_cfg.get("provider") == "jarvis_managed"
        assert "api_mode" not in model_cfg
        assert "api_key" not in model_cfg
