...
def _make_jarvis_tree(root: Path) -> None:
    ...
    (root / "jarvis_cli.session_state.db").write_bytes(b"fake-state")
    ...
    (root / "jarvis-agent" / "run_agent.py").write_text("# big file\n")
...
class TestShouldExclude:
    def test_excludes_jarvis_agent(self):
        ...
        assert _should_exclude(Path("jarvis-agent/run_agent.py"))
...
class TestBackup:
    ...
    def test_excludes_jarvis_agent(self, tmp_path, monkeypatch):
        """Backup does NOT include jarvis-agent/ directory."""
        jarvis_home = tmp_path / ".jarvis"
        jarvis_home.mkdir()
        _make_jarvis_tree(jarvis_home)
...
class TestValidateBackupZip:
    ...
    def test_old_wrong_db_name_fails(self, tmp_path):
        """A zip with only session_state.db (old wrong name) is rejected."""
        from jarvis_cli.backup import _validate_backup_zip
        zip_path = tmp_path / "old.zip"
        self._make_zip(zip_path, ["jarvis_cli.session_state.db", "memory_store.db"])
        with zipfile.ZipFile(zip_path, "r") as zf:
            ok, reason = _validate_backup_zip(zf)
        assert not ok
...
class TestImport:
    ...
    def test_restores_secret_files_with_0600_perms(self, tmp_path, monkeypatch):
        ...
        self._make_backup_zip(zip_path, {
            ...
            "auth.json": '{"providers": {"jarvis_managed": "token"}}',
...
class TestPreUpdateBackup:
    ...
    def test_backup_contents_match_full_backup(self, jarvis_home):
        ...
        assert not any(n.startswith("jarvis-agent/") for n in names)
...
class TestRunPreUpdateBackup:
    ...
    def test_default_disabled_is_silent(self, jarvis_home, capsys):
