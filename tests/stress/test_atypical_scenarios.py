...
def _(home, kb):
    ...
    app = FastAPI()
    app.include_router(kanban_router, prefix="/api/src/plugins/kanban")
    client = TestClient(app)

    # Empty title
    r = client.post("/api/src/plugins/kanban/tasks", json={"title": ""})
    assert r.status_code in {400, 422}, f"empty title should 4xx, got {r.status_code}"

    # Title only
    r = client.post("/api/src/plugins/kanban/tasks", json={"title": "x"})
    assert r.status_code == 200, r.text

    # Huge title
    r = client.post("/api/src/plugins/kanban/tasks", json={"title": "x" * 10000})
    # Should succeed — kernel doesn't cap title length
    assert r.status_code == 200

    # Unicode + emoji
    r = client.post("/api/src/plugins/kanban/tasks", json={
        "title": "📋 deploy 🚀 to 生产",
        "body": "日本語 body",
        "assignee": "deploy-bot",
    })
    ...
    # Priority as non-int
    r = client.post("/api/src/plugins/kanban/tasks", json={"title": "prio", "priority": "high"})
    assert r.status_code == 422, f"string priority should 422, got {r.status_code}"

    # PATCH with empty body (no changes requested)
    r = client.patch(f"/api/src/plugins/kanban/tasks/{tid}", json={})
