...
def fake_mcp_server(populated_sessions_dir, mock_session_db, monkeypatch):
    import jarvis_cli.mcp_server as mcp_serve
...
class TestContentExtraction:
    def test_text(self):
        from jarvis_cli.mcp_server import _extract_message_content
...
class TestAttachmentExtraction:
    def test_image_url_block(self):
        from jarvis_cli.mcp_server import _extract_attachments
        msg = {"content": [
...
class TestEventBridge:
    def test_create(self):
        from jarvis_cli.mcp_server import EventBridge
        b = EventBridge()
        assert b._cursor == 0
        assert b._queue == []

    def test_enqueue_and_poll(self):
        from jarvis_cli.mcp_server import EventBridge, QueueEvent
        b = EventBridge()
    ...
    def test_queue_limit(self):
        from jarvis_cli.mcp_server import EventBridge, QueueEvent, QUEUE_LIMIT
        b = EventBridge()
...
class TestE2EEventsPoll:
    ...
    def test_poll_with_events(self, mcp_server_e2e, _event_loop):
        from jarvis_cli.mcp_server import QueueEvent
        server, bridge = mcp_server_e2e
...
class TestEventBridgePollE2E:
    ...
    def test_poll_interval_is_200ms(self):
        ...
        from jarvis_cli.mcp_server import POLL_INTERVAL
