"""Tests for logging_config — centralized logging setup."""
...
from jarvis_cli import logging_config
...
def _reset_logging_state():
    ...
    logging_config._install_session_record_factory()
    yield
    ...
    for h in list(root.handlers):
        if h not in pre_existing:
            ...
            h.close()
    logging_config._logging_initialized = False
    logging_config.clear_session_context()
...
class TestSetupLogging:
    ...
    def test_creates_log_directory(self, jarvis_home):
        log_dir = logging_config.setup_logging(jarvis_home=jarvis_home)
        assert log_dir == jarvis_home / "logs"
        assert log_dir.is_dir()

    def test_creates_agent_log_handler(self, jarvis_home):
        logging_config.setup_logging(jarvis_home=jarvis_home)
        root = logging.getLogger()
    ...
    def test_idempotent_no_duplicate_handlers(self, jarvis_home):
        logging_config.setup_logging(jarvis_home=jarvis_home)
        logging_config.setup_logging(jarvis_home=jarvis_home)  # second call — should be no-op
    ...
    def test_force_reinitializes(self, jarvis_home):
        ...
        logging_config.setup_logging(jarvis_home=jarvis_home, force=True)
    ...
    def test_custom_log_level(self, jarvis_home):
        logging_config.setup_logging(jarvis_home=jarvis_home, log_level="DEBUG")
    ...
    def test_writes_to_agent_log(self, jarvis_home):
        ...
        test_logger = logging.getLogger("test_logging_config.write_test")
        ...
        agent_log = jarvis_home / "logs" / "agent.log"
    ...
    def test_warnings_appear_in_both_logs(self, jarvis_home):
        ...
        test_logger = logging.getLogger("test_logging_config.warning_test")
    ...
    def test_info_not_in_errors_log(self, jarvis_home):
        ...
        test_logger = logging.getLogger("test_logging_config.info_test")
    ...
    def test_explicit_params_override_config(self, jarvis_home):
        ...
        logging_config.setup_logging(jarvis_home=jarvis_home, log_level="WARNING")
...
class TestGatewayMode:
    ...
    def test_gateway_log_created(self, jarvis_home):
        logging_config.setup_logging(jarvis_home=jarvis_home, mode="gateway")
        root = logging.getLogger()
    ...
    def test_gateway_log_not_created_in_cli_mode(self, jarvis_home):
        logging_config.setup_logging(jarvis_home=jarvis_home, mode="cli")
        root = logging.getLogger()
...
class TestSessionContext:
    ...
    def test_session_tag_in_log_output(self, jarvis_home):
        ...
        logging_config.setup_logging(jarvis_home=jarvis_home)
        logging_config.set_session_context("abc123")
    ...
    def test_clear_session_context(self, jarvis_home):
        ...
        logging_config.setup_logging(jarvis_home=jarvis_home)
        logging_config.set_session_context("xyz789")
        logging_config.clear_session_context()
    ...
    def test_session_context_thread_isolated(self, jarvis_home):
        ...
        def thread_a():
            logging_config.set_session_context("thread_a_session")
            logging.getLogger("test.thread_a").info("from thread A")
            for h in logging.getLogger().handlers:
                h.flush()

        def thread_b():
            logging_config.set_session_context("thread_b_session")
...
class TestRecordFactory:
    ...
    def test_tag_with_context(self):
        logging_config.set_session_context("sess_42")
        factory = logging.getLogRecordFactory()
        record = factory("test", logging.INFO, "", 0, "msg", (), None)
    ...
    def test_works_with_any_handler(self):
        ...
        logging_config.set_session_context("any_handler_test")
...
class TestComponentFilter:
    ...
    def test_passes_matching_prefix(self):
        f = logging_config._ComponentFilter(("gateway",))
    ...
    def test_multiple_prefixes(self):
        f = logging_config._ComponentFilter(("agent", "run_agent", "model_tools"))
...
class TestComponentPrefixes:
    ...
    def test_gateway_prefix(self):
        assert "gateway" in logging_config.COMPONENT_PREFIXES
        ...
        # under that prefix).
        assert ("gateway", "jarvis_plugins") == logging_config.COMPONENT_PREFIXES["gateway"]

    def test_agent_prefix(self):
        prefixes = logging_config.COMPONENT_PREFIXES["agent"]
        assert "agent" in prefixes
        assert "run_agent" in prefixes
        assert "model_tools" in prefixes

    def test_tools_prefix(self):
        assert ("tools",) == logging_config.COMPONENT_PREFIXES["tools"]

    def test_cli_prefix(self):
        prefixes = logging_config.COMPONENT_PREFIXES["cli"]
        assert "jarvis_cli" in prefixes
        assert "cli" in prefixes

    def test_cron_prefix(self):
        assert ("cron",) == logging_config.COMPONENT_PREFIXES["cron"]
...
class TestSetupVerboseLogging:
    ...
    def test_idempotent(self, jarvis_home):
        logging_config.setup_logging(jarvis_home=jarvis_home)
        logging_config.setup_verbose_logging()
        logging_config.setup_verbose_logging()  # second call
...
class TestAddRotatingHandler:
    ...
    def test_creates_directory(self, tmp_path):
        ...
        logging_config._add_rotating_handler(
            logger, log_path,
    ...
    def test_log_filter_attached(self, tmp_path):
        ...
        formatter = logging.Formatter("%(message)s")
        component_filter = logging_config._ComponentFilter(("test",))
    ...
    def test_no_session_filter_on_handler(self, tmp_path):
        ...
        logging_config.set_session_context("factory_test")
...
class TestReadLoggingConfig:
    ...
    def test_returns_none_when_no_config(self, jarvis_home):
        level, max_size, backup = logging_config._read_logging_config()
