...
def _load_release_module(monkeypatch, tmp_root: Path):
    ...
    monkeypatch.setattr(
        module, "ACP_REGISTRY_MANIFEST", tmp_root / "ops" / "acp_registry" / "agent.json"
    )
    return module


def _write_manifest(root: Path, version: str) -> None:
    manifest_dir = root / "ops" / "acp_registry"
...
def test_update_acp_registry_versions_bumps_manifest_and_pin(monkeypatch, tmp_path):
    ...
    manifest = json.loads(
        (tmp_path / "ops" / "acp_registry" / "agent.json").read_text(encoding="utf-8")
    )
