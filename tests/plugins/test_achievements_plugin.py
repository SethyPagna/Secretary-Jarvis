...
``src/plugins/jarvis-achievements/tests/`` covering the achievement engine
...
def plugin_api(tmp_path, monkeypatch):
    ...
    # swap ``sys.modules['jarvis_cli.session_state']`` with auto-restoration. Without
    # this, a raw ``sys.modules[...] = fake`` assignment would leak the
    # fake into later tests in the same xdist worker — breaking every
    # test that does ``from jarvis_cli.session_state import SessionDB``.
    module._test_monkeypatch = monkeypatch
    yield module


class _FakeSessionDB:
    """Stand-in for session_state.SessionDB that records scan calls."""
...
def _install_fake_session_db(plugin_api, fake_db):
    ...
    ``sys.modules['jarvis_cli.session_state']`` swap is auto-restored at test teardown
    and cannot leak into unrelated tests in the same xdist worker.
    """
    fake_module = type(sys)("jarvis_cli.session_state")
    fake_module.SessionDB = lambda: fake_db
    plugin_api._test_monkeypatch.setitem(sys.modules, "jarvis_cli.session_state", fake_module)
