...
class DesktopHomeContractTests(unittest.TestCase):
    ...
    def test_home_voice_records_transcribes_and_dispatches_to_live_terminal(self) -> None:
        ...
        self.assertIn("VOICE_AUDIO_CONSTRAINTS", source)
        ...
        self.assertIn("audio: VOICE_AUDIO_CONSTRAINTS", source)
        self.assertIn("encodePcm16Wav", source)
        self.assertIn('new Blob([buffer], { type: "audio/wav" })', source)
        self.assertIn("createVoiceWavBlob()", source)
        ...
        self.assertIn("const agentPrompt = cleanPrompt", source)
        self.assertNotIn("Spoken user message:", source)
        self.assertNotIn("SpeechRecognition", source)
        self.assertNotIn("webkitSpeechRecognition", source)
        self.assertNotIn("liveTranscriptFresh", source)
        self.assertNotIn("runDesktopAgentTurn(liveTranscript", source)
        ...
        self.assertNotIn("VOICE_MAX_EMPTY_RETRIES", source)
        self.assertIn("Voice is live. Listening again.", source)
        self.assertNotIn("Voice is paused until the microphone has a clear signal", source)
        self.assertIn("scheduleVoiceRetry", source)
        self.assertIn("Voice is live. Waiting for speech.", source)
        self.assertIn("voiceSilentRetryScheduledRef", source)
        self.assertIn("Voice capture is live; Whisper is still warming", source)
        self.assertIn("whisper warming", source)
        self.assertNotIn("Using browser live transcription while local Whisper finishes warming.", source)
        self.assertNotIn("STT is not ready yet. Waiting for the Whisper model.", source)
        self.assertIn("const VOICE_SPEECH_THRESHOLD = 0.018", source)
        self.assertIn("const VOICE_MIN_SPEECH_MS = 520", source)
        self.assertIn("voiceSpeechStartedAtRef", source)
        self.assertIn("const VOICE_AUTO_STOP_SILENCE_MS = 1050", source)
        self.assertIn("const VOICE_EMPTY_RETRY_DELAY_MS = 1_600", source)
        self.assertIn("const VOICE_MAX_NO_SPEECH_MS = 30_000", source)
        self.assertIn("const VOICE_SILENT_MONITOR_RESTART_MS = 1_800", source)
        self.assertIn("const VOICE_LIVE_TRANSCRIBE_INTERVAL_MS = 2_400", source)
        self.assertIn("const hadSpeech = voiceHadSpeechRef.current", source)
        self.assertIn("if (!hadSpeech) {", source)
        self.assertIn('scheduleVoiceRetry("silent")', source)
        ...
        self.assertIn("if (autoVoiceArmed || listening || voiceBusy || speaking || ttsQueueBusy) {", source)
        self.assertIn("setAutoVoiceArmed(false)", source)
        self.assertIn("Voice loop paused.", source)
        self.assertIn("setAutoVoiceArmed(true)", source)
        self.assertNotIn("setAutoVoiceArmed(true);\n                void toggleMic()", source)
        ...
        self.assertNotIn("getUserMedia({ audio: true })", source)
    ...
    def test_home_voice_output_synthesizes_live_assistant_output(self) -> None:
        ...
        self.assertIn("activeTurnSoulRef", home_source)
        self.assertIn("activeTurnSoulRef.current = nextSoul", home_source)
        self.assertIn("soul: activeTurnSoulRef.current ?? stats?.active_soul ?? \"jarvis\"", home_source)
        ...
        self.assertIn("speechQueuePendingRef", home_source)
        self.assertIn("ttsQueueBusy", home_source)
        self.assertIn("speechQueuePendingRef.current += 1", home_source)
        self.assertIn("speechQueuePendingRef.current = Math.max(0, speechQueuePendingRef.current - 1)", home_source)
        self.assertIn("if (!autoVoiceArmed || listening || voiceBusy || speaking || ttsQueueBusy) return", home_source)
        self.assertIn("if (autoVoiceArmed || listening || voiceBusy || speaking || ttsQueueBusy)", home_source)
    ...
    def test_api_client_exposes_voice_stt_and_tts_endpoints(self) -> None:
        ...
        self.assertIn("skills_summary?: SkillsRuntimeSummary", source)
        self.assertIn("interface SkillsRuntimeSummary", source)
        self.assertIn("memory_context?: MemoryContextSummary", source)
        self.assertIn("interface MemoryContextSummary", source)
    ...
    def test_orb_and_home_use_unframed_cosmic_scene(self) -> None:
        ...
        self.assertIn("skillsSummary", stats_source)
        self.assertIn("setup_names.join", stats_source)
        self.assertIn("memorySummary", stats_source)
        self.assertIn("Memory", stats_source)
