"""Generic managed-tool gateway helpers for JARVIS Managed-hosted vendor passthroughs."""
...
_JARVIS_MANAGED_ACCESS_TOKEN_REFRESH_SKEW_SECONDS = 120
...
def _read_jarvis_managed_provider_state() -> Optional[dict]:
    try:
        path = auth_json_path()
        if not path.is_file():
            return None
        ...
        if not isinstance(providers, dict):
            return None
        jarvis_managed_provider = providers.get("jarvis_managed", {})
        if isinstance(jarvis_managed_provider, dict):
            return jarvis_managed_provider
    except Exception:
        pass
    return None
...
def read_jarvis_managed_access_token() -> Optional[str]:
    """Read a JARVIS Managed Subscriber OAuth access token from auth store or env override."""
    explicit = os.getenv("TOOL_GATEWAY_USER_TOKEN")
    if isinstance(explicit, str) and explicit.strip():
        return explicit.strip()

    jarvis_managed_provider = _read_jarvis_managed_provider_state() or {}
    access_token = jarvis_managed_provider.get("access_token")
    ...
    if cached_token and not _access_token_is_expiring(
        jarvis_managed_provider.get("expires_at"),
        _JARVIS_MANAGED_ACCESS_TOKEN_REFRESH_SKEW_SECONDS,
    ):
        return cached_token

    try:
        from jarvis_cli.auth import resolve_jarvis_managed_access_token

        refreshed_token = resolve_jarvis_managed_access_token(
            refresh_skew_seconds=_JARVIS_MANAGED_ACCESS_TOKEN_REFRESH_SKEW_SECONDS,
        )
        if isinstance(refreshed_token, str) and refreshed_token.strip():
            return refreshed_token.strip()
    except Exception as exc:
        logger.debug("JARVIS Managed access token refresh failed: %s", exc)
...
def resolve_managed_tool_gateway(
    ...
    resolved_token_reader = token_reader or read_jarvis_managed_access_token

    gateway_origin = resolved_gateway_builder(vendor)
    jarvis_managed_user_token = resolved_token_reader()
    if not gateway_origin or not jarvis_managed_user_token:
        return None

    return ManagedToolGatewayConfig(
        vendor=vendor,
        gateway_origin=gateway_origin,
        jarvis_managed_user_token=jarvis_managed_user_token,
        managed_mode=True,
    )
...
def is_managed_tool_gateway_ready(
    ...
    """Return True when gateway URL and JARVIS Managed access token are available."""
